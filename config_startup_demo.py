...
def main():
    ...
    try:
        ...
        if missing_secrets:
            logger.warning(
                f"Missing secrets (non-critical in {settings.ENVIRONMENT}): {missing_secrets}"
            )
        else:
        ...
        logger.info(f"\n=== Type Safety Demo ===")
        logger.info(
            f"API Port (int): {settings.API_PORT} (type: {type(settings.API_PORT).__name__})"
        )
        logger.info(f"Debug Mode (bool): {settings.DEBUG} (type: {type(settings.DEBUG).__name__})")
        logger.info(
            f"Risk Percentage (float): {settings.RISK_PERCENTAGE} (type: {type(settings.RISK_PERCENTAGE).__name__})"
        )
