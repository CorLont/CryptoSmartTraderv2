...
class RealisticExecutionEngine:
    ...
    def execute_order(
        ...
        executed_size = (
            order_size * (0.5 + np.random.random() * 0.5) if partial_fill else order_size
        )
...
class PortfolioBacktestEngine:
    ...
    def backtest_strategy(
        self, signals_df: pd.DataFrame, initial_capital: float = 100000
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_slippage_bps": np.mean([trade["slippage_bps"] for trade in trades])
            if trades
            else 0,
