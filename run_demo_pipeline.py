...
def create_demo_data():
    ...
    crypto_coins = [
        'BTC', 'ETH', 'ADA', 'DOT', 'SOL', 'AVAX', 'NEAR', 'FTM', 'MATIC', 'ATOM',
        'LINK', 'UNI', 'AAVE', 'COMP', 'MKR', 'SNX', 'YFI', 'SUSHI', 'CRV', 'LUNA',
        'ALGO', 'XTZ', 'EGLD', 'ONE', 'HBAR', 'VET', 'ENJ', 'ZIL', 'BAT', 'ZRX',
        'ICX', 'ONT', 'QTUM', 'WAVES', 'LSK', 'ARK', 'STRAT', 'XEM', 'MONA', 'DCR'
    ]
...
def generate_predictions_with_ml_features(models, metadata, df):
    ...
    for idx, row in df.iterrows():
        ...
        expected_return_pct = predicted_return * 100 if predicted_direction == 1 else -abs(predicted_return) * 100
...
def apply_confidence_gate_demo(predictions_df, min_confidence=80.0):
    ...
    logger.info(f"Confidence gate: {filtered_count}/{original_count} predictions passed (≥{min_confidence}%)")
