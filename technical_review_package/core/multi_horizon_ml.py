...
class MultiHorizonMLSystem:
    ...
    def __init__(self, container):
        ...
        self.logger.info(
            f"Multi-horizon ML system initialized with {len(self.horizons)} time horizons"
        )
    ...
    def prepare_training_data(self, lookback_days: int = 30) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if len(validated_coins) < 10:
                self.logger.warning(
                    f"Insufficient coins with complete data: {len(validated_coins)}"
                )
                return None
            ...
            self.logger.info(
                f"Training data prepared: {final_rows}/{initial_rows} valid samples from {len(validated_coins)} coins"
            )

            return (
                combined_df
                if len(combined_df) >= self.training_config["min_training_samples"]
                else None
            )
    ...
    def _extract_coin_historical_data(
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["volatility"] = (
                df.groupby("coin")["price_momentum"]
            ...
            df["price_vs_24h_low"] = df["close"] / df.groupby("coin")["low"].rolling(
                window=24, min_periods=1
        ...
        try:
            ...
            for horizon_name in self.horizons.keys():
                ...
                if len(X_valid) < self.training_config["min_training_samples"]:
                    self.logger.warning(
                        f"Insufficient valid samples for {horizon_name}: {len(X_valid)}"
                    )
                    continue

                # Train-test split
                X_train, X_test, y_train, y_test = train_test_split(
                    X_valid, y_valid, test_size=self.training_config["test_size"], random_state=42
                )
                ...
                self.logger.info(
                    f"{horizon_name} model trained - MAE: {test_mae:.4f}, Samples: {len(X_valid)}"
                )
                    ...
                    for i, (pred, conf) in enumerate(zip(y_pred, confidence_scores)):
                        if i < len(features_df):
                            ...
                            result = {
                                ...
                                "meets_threshold": conf
                                >= self.training_config["confidence_threshold"],
    ...
    def _calculate_prediction_confidence(
        self, model, X: np.ndarray, horizon_name: str
    ) -> np.ndarray:
    ...
    def get_alpha_opportunities(
        self, min_confidence: float = 0.80, min_return_30d: float = 1.0
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for coin in all_coins:
                ...
                if (
                    coin_opportunity["overall_confidence"] >= min_confidence
                    and coin_opportunity["max_return_30d"] >= min_return_30d
                ):
