...
class OrderPipeline:
    ...
    def __init__(self, 
        ...
        self.logger.info("OrderPipeline initialized with hard ExecutionPolicy wire-up",
        ...
        with self._lock:
            try:
                ...
                if client_order_id in self.client_order_registry:
                    ...
                    self.logger.info("Duplicate order detected - returning existing result",
                ...
                if not execution_decision['approved']:
                    ...
                    self.logger.warning("Order rejected by ExecutionPolicy",
                                      client_order_id=client_order_id,
                ...
                self.logger.info("Order successfully processed through pipeline",
                               client_order_id=client_order_id,
                               symbol=order_request.symbol,
                ...
                try:
                    if 'order_result' in locals() and hasattr(locals()['order_result'], 'client_order_id'):
    ...
    def _manual_policy_check(self, market_context: Dict[str, Any], order_request: OrderRequest) -> Dict[str, Any]:
        ...
        if orderbook_depth > 0 and (order_value / orderbook_depth) > 0.1:  # Max 10% of depth
            ...
            rejection_reason = f"Order size {order_value/orderbook_depth*100:.1f}% of depth > 10%"
        ...
        return {
            ...
            'market_conditions': {
                ...
                'order_impact_pct': (order_value / orderbook_depth * 100) if orderbook_depth > 0 else 0
            }
        }

    def _validate_slippage_budget(self, order_request: OrderRequest, execution_decision: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_time_in_force(self, order_request: OrderRequest) -> Dict[str, Any]:
        ...
        if order_request.time_in_force == TimeInForce.POST_ONLY:
            if order_request.order_type != OrderType.LIMIT and order_request.order_type != OrderType.POST_ONLY:
                return {
                    'approved': False,
        ...
        if order_request.expires_at and order_request.expires_at <= datetime.now():
            return {
                'approved': False,
                'rejection_reason': "Order already expired"
            }
        
        return {'approved': True}

    async def _execute_order(self, order_request: OrderRequest, execution_decision: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _finalize_order_result(self, order_result: OrderResult, pipeline_start_time: float) -> None:
        ...
        # Move to history
        if order_result.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.FAILED, OrderStatus.REJECTED]:
        ...
        # Record metrics
        symbol_part = order_result.client_order_id.split('_')[1] if len(order_result.client_order_id.split('_')) > 1 else 'unknown'
    ...
    def _cleanup_dedupe_cache(self) -> None:
        ...
        # Remove old entries
        expired_keys = [
            key for key, timestamp in self.order_dedupe_cache.items()
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_slippage_bps': (
                self.pipeline_stats['total_slippage_bps'] / max(self.pipeline_stats['orders_filled'], 1)
            ),
            'recent_orders': [
                {
                    'client_order_id': result.client_order_id,
                    'status': result.status.value,
                    'symbol': result.client_order_id.split('_')[1].upper() if len(result.client_order_id.split('_')) > 1 else 'unknown',
