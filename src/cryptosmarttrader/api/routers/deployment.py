...
from cryptosmarttrader.deployment.go_live_system import (
    GoLiveManager, SLOMonitor, ChaosTestRunner, 
    DeploymentStage, HealthStatus, DeploymentMetrics
)
...
class DeploymentRequest(BaseModel):
    ...
    risk_budget_percentage: Optional[float] = Field(1.0, description="Risk budget for canary deployment")
...
@router.post("/chaos/test",
            ...
            summary="Run chaos engineering tests")
...
@router.post("/staging/validate",
            summary="Validate staging environment")
...
async def deploy_canary(request: DeploymentRequest, background_tasks: BackgroundTasks):
    ...
    """
    try:
        logger.info(f"Starting canary deployment with {request.risk_budget_percentage}% risk budget")
        
        # Validate request parameters
        if request.risk_budget_percentage > 5.0:
            raise HTTPException(
                status_code=400,
                detail="Risk budget cannot exceed 5% for canary deployment"
            )
        
        if request.duration_hours < 24 or request.duration_hours > 168:  # 1-7 days
            raise HTTPException(
                status_code=400,
                detail="Canary duration must be between 24 and 168 hours"
            )
...
async def deploy_production(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if last_deployment.get('success_rate', 0) < 0.95:
            raise HTTPException(
                status_code=409,
                detail="Canary success rate too low for production deployment"
            )
...
@router.post("/go-live",
            summary="Execute complete go-live sequence")
async def execute_go_live_sequence(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not slo_results['overall_compliance']:
            raise HTTPException(
                status_code=409,
                detail="SLO compliance violations prevent go-live sequence"
            )
        ...
        if chaos_results['overall_success_rate'] < 0.8:
            raise HTTPException(
                status_code=409,
                detail="Chaos test failures prevent go-live sequence"
            )
