...
class MetricsCollector:
    ...
    def _init_trading_metrics(self):
        ...
        # Portfolio equity
        self.equity = Gauge(
            "cryptotrader_equity_usd", "Current portfolio equity in USD", registry=self.registry
        )
        ...
        self.daily_pnl = Gauge(
            "cryptotrader_daily_pnl_usd", "Daily profit and loss in USD", registry=self.registry
        )
    ...
    def _init_system_metrics(self):
        ...
        self.system_uptime = Gauge(
            "cryptotrader_uptime_seconds", "System uptime in seconds", registry=self.registry
        )
    ...
    def update_equity(self, equity_usd: float):
        ...
        self.logger.info(
            "Equity updated",
            extra={"extra_fields": {"equity_usd": equity_usd, "metric": "equity_update"}},
        )
    ...
    def update_drawdown(self, drawdown_pct: float):
        ...
        if drawdown_pct > 10.0:  # 10% drawdown threshold
            self._trigger_alert(
                ...
                {"drawdown_percent": drawdown_pct, "threshold": 10.0},
            )

        self.logger.info(
            "Drawdown updated",
            extra={"extra_fields": {"drawdown_percent": drawdown_pct, "metric": "drawdown_update"}},
        )
    ...
    def _check_api_error_rate(self):
        ...
        if error_rate > 0.10:  # 10% threshold
            self._trigger_alert(
                ...
                {"error_rate_percent": error_rate * 100, "threshold_percent": 10.0},
            )
    ...
    def check_no_signals_alert(self):
        ...
        if time_since_last_signal > 1800:  # 30 minutes
            self._trigger_alert(
                "NoSignals",
                AlertSeverity.WARNING,
                {"minutes_since_last_signal": time_since_last_signal / 60, "threshold_minutes": 30},
            )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            "timestamp": datetime.utcnow().isoformat(),
            "orders": {
                "total_sent": self._get_counter_value(
                    self.orders_total, {"state": OrderState.SENT.value}
                ),
                "total_filled": self._get_counter_value(
                    self.orders_total, {"state": OrderState.FILLED.value}
                ),
                "total_errors": self._get_counter_value(self.order_errors),
            },
            "trading": {
                "equity_usd": self.equity._value._value
                if hasattr(self.equity._value, "_value")
                else 0,
                "drawdown_percent": self.drawdown_pct._value._value
                if hasattr(self.drawdown_pct._value, "_value")
                else 0,
                "daily_pnl_usd": self.daily_pnl._value._value
                if hasattr(self.daily_pnl._value, "_value")
                else 0,
            },
            "system": {
