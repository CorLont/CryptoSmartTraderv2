...
class StrictConfidenceGate:
    ...
    def apply_gate(
        ...
        try:
            ...
            if not all(col in predictions_df.columns for col in required_columns):
                ...
                return pd.DataFrame(), self._create_gate_report(
                    gate_id, original_count, 0, "missing_columns"
                )
            ...
            if not confidence_columns:
                ...
                self.rejection_counts["missing_data"] += original_count
                return pd.DataFrame(), self._create_gate_report(
                    gate_id, original_count, 0, "no_confidence"
                )
            ...
            if passed_count == 0:
                self.logger.warning(
                    f"STRICT GATE CLOSED: {gate_id} - only {passed_count}/{original_count} candidates passed (minimum: 1)"
                )
                self.logger.info(
                    "No reliable opportunities available - confidence threshold not met"
                )
            else:
                self.logger.info(
                    f"STRICT GATE PASSED: {gate_id} - {passed_count}/{original_count} candidates passed"
                )
    ...
    def log_empty_state(self, context: str = "dashboard"):
        ...
        self.logger.info(
            f"No reliable opportunities available in {context} - strict confidence gate active"
        )
...
def apply_strict_confidence_filter(
    predictions_df: pd.DataFrame, threshold: float = 0.80, gate_id: Optional[str] = None
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
