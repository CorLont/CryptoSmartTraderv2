...
def validate_production_outputs():
    ...
    if predictions_file.exists():
        try:
            ...
            validation_results["predictions_valid"] = all(col in pred_df.columns for col in required_cols)
            validation_results["prediction_count"] = len(pred_df)
            validation_results["unique_coins"] = pred_df["coin"].nunique() if "coin" in pred_df.columns else 0
            
            print(f"✅ predictions.csv: {len(pred_df)} predictions, {validation_results['unique_coins']} coins")
    ...
    # Overall validation
    all_valid = (validation_results["predictions_csv_exists"] and 
                validation_results["summary_json_exists"] and
                validation_results["predictions_valid"] and
                validation_results["summary_valid"])
