...
class ReplayBuffer:
    ...
    def sample(
        self, batch_size: int, strategy: str = "uniform"
        ...
        if strategy == "uniform":
            indices = np.random.choice(
                len(self.samples), size=min(batch_size, len(self.samples)), replace=False
            )
            ...
            indices = np.random.choice(
                len(self.samples), size=min(batch_size, len(self.samples)), replace=False, p=weights
            )
...
class EWCRegularizer:
    ...
    def compute_fisher_information(self, dataloader, device: str = "cpu") -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Computed Fisher information for {len(self.fisher_information)} parameters"
            )
...
class FineTuneScheduler:
    ...
    def create_fine_tune_job(
        ...
        self.logger.info(
            f"Created fine-tuning job {job.job_id} for {model_name} "
    ...
    def setup_ewc_for_model(
        self, model_name: str, model, training_dataloader, device: str = "cpu"
    ) -> None:
    ...
    def run_fine_tuning_job(
        self, job: FineTuneJob, model, optimizer, train_dataloader, device: str = "cpu"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for epoch in range(job.max_epochs):
                ...
                self.logger.info(
                    f"Job {job.job_id} - Epoch {epoch + 1}/{job.max_epochs}: "
            ...
            self.logger.info(
                f"Fine-tuning job {job.job_id} completed successfully. "
    ...
    def _prepare_mixed_dataloader(self, job: FineTuneJob, new_dataloader):
        ...
        try:
            ...
            replay_features, replay_targets, replay_weights = replay_buffer.sample(
                batch_size=replay_batch_size * len(new_dataloader), strategy="importance_weighted"
            )
            ...
            self.logger.debug(
                f"Created mixed dataloader with {len(mixed_batches)} batches "
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        recent_completed = [
            job
            for job in self.completed_jobs
            if job.completed_at and (datetime.now() - job.completed_at).total_seconds() < 86400
        ]
        ...
        status = {
            ...
            "replay_buffers": {
                name: buffer.get_statistics() for name, buffer in self.replay_buffers.items()
            },
