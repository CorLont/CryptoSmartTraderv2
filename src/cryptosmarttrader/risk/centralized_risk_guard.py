...
class CentralizedRiskGuard:
    ...
    def __init__(self,
        ...
        self.logger.info("CentralizedRiskGuard initialized with hard wire-up",
        ...
        with self._lock:
            try:
                ...
                if self.kill_switch_active:
                    ...
                    self.logger.warning("Operation blocked by kill-switch",
                ...
                if day_loss_check['violation']:
                    ...
                    if day_loss_check['action'] in ['block', 'kill_switch']:
                        result.action = RiskAction.BLOCK if day_loss_check['action'] == 'block' else RiskAction.KILL_SWITCH
                        result.approved = False
                        
                        if day_loss_check['action'] == 'kill_switch':
                            self._trigger_kill_switch(f"Daily loss limit exceeded: {request.daily_pnl:.2f}%")
                ...
                if drawdown_check['violation']:
                    ...
                    if drawdown_check['action'] in ['block', 'kill_switch']:
                        result.action = RiskAction.BLOCK if drawdown_check['action'] == 'block' else RiskAction.KILL_SWITCH
                        result.approved = False
                        
                        if drawdown_check['action'] == 'kill_switch':
                            self._trigger_kill_switch(f"Max drawdown exceeded: {request.total_drawdown_pct:.2f}%")
                ...
                self.logger.error("Risk check failed - blocking operation",
                                operation_type=request.operation_type.value,
                                symbol=request.symbol,
                                error=str(e),
                                check_time_ms=check_time)
    ...
    def _calculate_risk_budget_usage(self, request: RiskCheckRequest) -> float:
        ...
        daily_risk_used = abs(request.daily_pnl) / self.max_daily_loss_pct * 100 if request.daily_pnl < 0 else 0
    ...
    def _log_risk_check(self, request: RiskCheckRequest, result: RiskCheckResult) -> None:
        ...
        if result.approved:
            self.logger.info("Risk check approved",
                           operation_type=request.operation_type.value,
                           symbol=request.symbol,
            ...
            self.logger.warning("Risk check blocked",
                              operation_type=request.operation_type.value,
                              symbol=request.symbol,
                              quantity=request.quantity,
    ...
    def _update_risk_tracking(self, request: RiskCheckRequest, result: RiskCheckResult) -> None:
        ...
        self.daily_operations = [op for op in self.daily_operations if op['timestamp'] > cutoff_time]
        ...
        try:
            if state_file.exists():
                ...
                if state.get('kill_switch_timestamp'):
                    self.kill_switch_timestamp = datetime.fromisoformat(state['kill_switch_timestamp'])
                
                self.logger.info("Loaded persistent kill-switch state",
                               kill_switch_active=self.kill_switch_active,
