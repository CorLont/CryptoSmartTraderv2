...
class MarketRegimeDetector:
    ...
    def _score_regime(self, features: Dict[str, float], regime: str) -> float:
        ...
        if regime == 'bull':
            ...
            score += max(0, (0.4 - features['return_volatility']) * 1.5)  # High but capped volatility
...
class MLUncertaintyEngine:
    ...
    def train_uncertainty_models(self, training_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={'samples': len(features), 'features': features.shape[1] if hasattr(features, 'shape') else 0}
            )
    ...
    def update_performance_metrics(self, actual_values: List[float], predictions: List[float], 
