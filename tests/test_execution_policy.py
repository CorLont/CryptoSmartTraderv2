...
class TestExecutionPolicy:
    ...
    def test_slippage_budget_validation(self, execution_policy):
        ...
        within_budget, slippage = execution_policy.validate_slippage_budget(
            params,
            50060.0,
            conditions,  # 50 bps slippage
        )
        assert not within_budget
        assert slippage > params.max_slippage_bps

    @pytest.mark.asyncio
    async def test_successful_order_execution(
        self, execution_policy, good_market_conditions, standard_order_params
    ):
        """Test successful order execution."""
        execution = await execution_policy.execute_order(
            standard_order_params, good_market_conditions
        )
    ...
    @pytest.mark.asyncio
    async def test_rejected_order_tradability_gates(
        self, execution_policy, bad_market_conditions, standard_order_params
    ):
        """Test order rejection due to tradability gates."""
        execution = await execution_policy.execute_order(
            standard_order_params, bad_market_conditions
        )
    ...
    async def test_duplicate_order_rejection(self, execution_policy, good_market_conditions):
        ...
        params = ExecutionParams(
            symbol="BTC-USD", side="buy", quantity=0.1, client_order_id="TEST_DUPLICATE_ORDER"
        )
    ...
    def test_execution_strategy_optimization(self, execution_policy):
        ...
        large_order_params = ExecutionParams(
            symbol="BTC-USD", side="buy", quantity=1.0, order_type=OrderType.LIMIT
        )
        ...
        small_order_params = ExecutionParams(
            symbol="BTC-USD", side="buy", quantity=0.1, order_type=OrderType.LIMIT
        )
...
class TestExecutionScenarios:
    ...
    async def test_large_order_optimization_scenario(self, execution_system):
        ...
        execution = await execution_system.execute_order(
            large_order_params, high_liquidity_conditions
        )
    ...
    async def test_basic_execution():
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            twap_params = ExecutionParams(
                symbol="BTC-USD", side="buy", quantity=1.0, order_type=OrderType.TWAP
            )
