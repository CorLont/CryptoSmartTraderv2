...
class SyntaxErrorFixer:
    ...
    def __init__(self):
        self.exclude_patterns = [
            "*.git*", "*__pycache__*", "*.cache*", "*venv*", "*node_modules*",
            "*mlartifacts*", "*models*", "*exports*", "*logs*", "*.pyc",
            "*dist*", "*build*", "*.egg-info*", "*experiments*"
        ]
    ...
    def apply_common_fixes(self, content: str, error_msg: str) -> str:
        ...
        if "invalid syntax" in error_msg:
            ...
            for i, line in enumerate(lines):
                stripped = line.strip()
                if (stripped.startswith(('if ', 'for ', 'while ', 'def ', 'class ', 'try', 'except ', 'finally', 'with ')) 
