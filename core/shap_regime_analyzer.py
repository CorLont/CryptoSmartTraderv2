...
class SHAPRegimeAnalyzer:
    ...
    def analyze_regime_specific_importance(
        self, data: pd.DataFrame, target_column: str, regime_column: Optional[str] = None
    ) -> Dict[MarketRegime, SHAPAnalysisResult]:
        ...
        with self._lock:
            try:
                ...
                if regime_column is None:
                    ...
                    current_regime = (
                        regime_data.iloc[-1] if not regime_data.empty else MarketRegime.SIDEWAYS
                    )
                ...
                # Prepare features
                feature_columns = [
                    col for col in data.columns if col != target_column and col != regime_column
                ]
                ...
                for regime in MarketRegime:
                    ...
                    if len(regime_X) < self.config.min_samples_per_regime:
                        self.logger.debug(
                            f"Insufficient samples for regime {regime.value}: {len(regime_X)}"
                        )
                        continue
    ...
    def _analyze_regime_shap(
        self, regime: MarketRegime, X: pd.DataFrame, y: pd.Series
    ) -> Optional[SHAPAnalysisResult]:
    ...
    def _create_regime_feature_set(
        self, regime: MarketRegime, analysis_result: SHAPAnalysisResult
    ) -> RegimeFeatureSet:
        """Create optimized feature set for specific regime"""
        try:
            # Sort features by importance
            sorted_features = sorted(
                analysis_result.feature_importance.items(), key=lambda x: x[1], reverse=True
            )
            ...
            for feature, importance in sorted_features:
                if importance > self.config.feature_importance_threshold:
                    ...
                    if len(selected_features) >= int(
                        self.config.max_features_to_analyze * self.config.adaptive_feature_ratio
                    ):
                        break
    ...
    def _calculate_regime_stability_score(
        self, regime: MarketRegime, current_features: List[str]
    ) -> float:
        ...
        try:
            ...
            for historical_set in historical_features:
                overlap = len(set(current_features) & set(historical_set)) / max(
                    len(current_features), 1
                )
    ...
    def _adapt_features_to_regime(self, current_regime: MarketRegime):
        ...
        try:
            ...
            if (
                hasattr(self, "_last_adapted_regime")
                and self._last_adapted_regime != current_regime
            ):
                self.logger.info(
                    f"Regime change detected: {self._last_adapted_regime} -> {current_regime}"
                )
    ...
    def explain_prediction(
        self, features: pd.DataFrame, target_value: float, regime: Optional[MarketRegime] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            model_features = (
                model.feature_names_in_ if hasattr(model, "feature_names_in_") else features.columns
            )
...
def analyze_features_for_regime(
