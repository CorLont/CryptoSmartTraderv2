...
async def demonstrate_risk_management():
    ...
    print(f"   Daily loss limits: {risk_guard.limits.daily_loss_warning:.1%} warn, {risk_guard.limits.daily_loss_critical:.1%} critical, {risk_guard.limits.daily_loss_emergency:.1%} emergency")
    print(f"   Drawdown limits: {risk_guard.limits.max_drawdown_warning:.1%} warn, {risk_guard.limits.max_drawdown_critical:.1%} critical, {risk_guard.limits.max_drawdown_emergency:.1%} emergency")
    print(f"   Position limits: {risk_guard.limits.max_position_size:.1%} max size, {risk_guard.limits.max_total_positions} max count")
    ...
    for equity, description in drawdown_scenarios:
        ...
        print(f"      Kill Switch: {'🚨 ACTIVE' if risk_guard.kill_switch_active else '✅ Inactive'}")
    ...
    for i in range(20):
        ...
        if i % 5 == 0:
            ...
            print(f"      API Success Rate: {current_rate:.1%} ({risk_guard.api_success_count}/{risk_guard.api_total_count})")
    ...
    for i in range(10):
        ...
        if i % 3 == 0:
            avg_latency = sum(risk_guard.recent_latencies[-5:]) / min(5, len(risk_guard.recent_latencies))
            print(f"      Average Latency: {avg_latency:.0f}ms (limit: {risk_guard.limits.max_latency_ms}ms)")
    ...
    for event in risk_guard.risk_events[-5:]:  # Show last 5 events
        print(f"   {event.timestamp.strftime('%H:%M:%S')} [{event.severity.value.upper()}] {event.description}")
