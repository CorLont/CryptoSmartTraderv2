...
class TrainingEnforcer:
    ...
    def _get_status_message(self, complete: bool, weeks: float) -> str:
        ...
        if complete:
            return (
                f"✅ Training complete ({weeks:.1f} weeks) - Confidence scores and trades ENABLED"
            )
        else:
    ...
    def get_dashboard_status(self) -> Dict[str, Any]:
        ...
        dashboard_info = {
            "training_progress_percent": min(
                100, (status["weeks_trained"] / self.required_weeks) * 100
            ),
            "status_color": "green" if status["training_complete"] else "orange",
            "status_text": status["message"],
            "features_blocked": []
            if status["training_complete"]
            else ["Confidence scores", "Trade execution", "Advanced ML features"],
...
def main():
    ...
    print(
        f"Confidence scores: {'✅ ENABLED' if gate_config['confidence_scores_enabled'] else '🚫 BLOCKED'}"
    )
