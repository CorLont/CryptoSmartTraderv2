...
class AnalysisControlDashboard:
    ...
    async def __init__(
        ...
        self.daily_scheduler = DailyAnalysisScheduler(
            self.config_manager, self.cache_manager, self.health_monitor
        )
    ...
    async def _render_daily_analysis_overview(
        ...
        with col1:
            ...
            if st.button(
                ...
                if not daily_status["scheduler_running"]
                else "Stop Daily Analysis"
            ):
    ...
    async def _render_service_status(
        ...
        with col1:
            ...
            status_color = (
                "🟢" if ml_status == "running" else "🔴" if ml_status == "error" else "🟡"
            )
        ...
        with col2:
            ...
            status_color = (
                "🟢" if social_status == "running" else "🔴" if social_status == "error" else "🟡"
            )
        ...
        with col3:
            ...
            status_color = (
                "🟢" if tech_status == "running" else "🔴" if tech_status == "error" else "🟡"
            )
        ...
        with col4:
            ...
            status_color = (
                "🟢"
                if sentiment_status == "running"
                else "🔴"
                if sentiment_status == "error"
                else "🟡"
            )
    ...
    async def _render_analysis_results(
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["ML Predictions", "Social Sentiment", "Technical Signals", "Market Sentiment"]
        )
    ...
    async def _start_ml_analysis(
        ...
        try:
            ...
            with st.spinner("Starting ML Analysis..."):
                ...
                st.success(
                    f"Started ML analysis for {num_coins} coins with {len(horizons)} prediction horizons"
                )
    ...
    async def _start_social_scraping(
        ...
        try:
            ...
            with st.spinner("Starting Social Media Scraping..."):
                ...
                st.success(
                    f"Started social media scraping for {', '.join(platforms)} with {interval} updates"
                )
    ...
    async def _start_all_background_services(
        ...
        try:
            ...
            if os.name == "nt":
                ...
                st.warning(
                    "Background service scripts are designed for Windows. Use the Python scripts directly on other platforms."
                )
