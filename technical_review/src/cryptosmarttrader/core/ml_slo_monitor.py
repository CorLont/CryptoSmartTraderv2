...
class SLOMonitor:
    ...
    def _calculate_violation_severity(self, slo_metric: SLOMetric, current_value: float) -> float:
        ...
        if slo_metric.metric_type == 'lower_is_better':
            if current_value <= slo_metric.critical_threshold:
                ...
                severity = min(1.0, (current_value - slo_metric.critical_threshold) /
            ...
            if current_value >= slo_metric.critical_threshold:
                ...
                severity = min(1.0, (slo_metric.critical_threshold - current_value) /
    ...
    def _trigger_remediation(self, violations: List[SLOViolation], horizon: Optional[str]):
        ...
        if critical_violations:
            ...
            accuracy_violations = [v for v in critical_violations
    ...
    def _trigger_retraining(self, horizon: Optional[str], reason: str) -> Optional[Dict[str, Any]]:
        ...
        recent_retrains = [
            ...
            and (datetime.now() - datetime.fromisoformat(a['timestamp'])).total_seconds() < 3600  # 1 hour
        ]
    ...
    def _trigger_monitoring_alert(self, violations: List[SLOViolation], horizon: Optional[str]) -> Dict[str, Any]:
    ...
    def get_slo_dashboard_data(self) -> Dict[str, Any]:
        ...
        for horizon in ['1h', '24h']:
            recent_performance = [
                s for s in self.performance_history[-50:]
            ...
            if recent_performance:
                ...
                dashboard_data['performance_trend'][f'mae_{horizon}'] = {
                    ...
                    'trend': 'improving' if len(mae_values) > 1 and mae_values[-1] < mae_values[-2] else 'degrading'
                }

        # Coverage stats
        total_metrics = len(self.slo_metrics)
        healthy_metrics = len([s for s in self.current_slo_status.values() if s == SLOStatus.HEALTHY])
