    ...
    async def run_tests():
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "health_monitor", "core/system_health_monitor.py"
            )
            ...
            if threshold_hierarchy_valid:
                ...
                print(
                    f"   Healthy: {thresholds.get('healthy_threshold')}%, Warning: {thresholds.get('warning_threshold')}%"
                )
            else:
            ...
            if "minimum_score" in go_nogo_config:
                print(
                    f"   ✅ PASSED: GO/NO-GO threshold properly configured: {go_nogo_config['minimum_score']}%"
                )
            else:
            ...
            try:
                ...
                if isinstance(data_metrics, dict) and isinstance(ml_metrics, dict):
                    print(
                        "   ✅ PASSED: Component assessments handle missing dependencies gracefully"
                    )
                else:
            ...
            go_nogo_decision = monitor._make_go_nogo_decision(
                report.components, report.overall_score
            )

            if isinstance(go_nogo_decision, bool):
                print(
                    f"   ✅ PASSED: GO/NO-GO decision made: {'GO' if go_nogo_decision else 'NO-GO'}"
                )
                print(
                    f"   Overall score: {report.overall_score:.1f}%, Decision: {'GO' if go_nogo_decision else 'NO-GO'}"
                )
            else:
            ...
            for component in report.components:
                ...
                if component.score != 50.0 and component.metrics:
                    print(
                        f"   ✅ {component.name}: {component.score:.1f}% with {len(component.metrics)} authentic metrics"
                    )
                elif not component.metrics:
                    print(
                        f"   ⚠️ {component.name}: No metrics available (expected for missing components)"
                    )
                else:
