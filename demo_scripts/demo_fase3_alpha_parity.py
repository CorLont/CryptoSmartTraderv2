...
from src.cryptosmarttrader.deployment.canary_system import (
    CanaryDeploymentSystem, DeploymentPlan, CanaryStage
)
...
class Fase3Demo:
    ...
    def demo_strategy_switching(self, regime_results):
        ...
        for strategy, config in summary['allocations'].items():
            if config['weight'] > 0:
                print(f"   {strategy}: {config['weight']:.1%} (max pos: {config['max_position_size']:.1%})")
        ...
        for symbol, target in position_targets.items():
            print(f"   {symbol}: {target.target_weight:.2%} "
        ...
        for cluster_name, cluster_info in summary['cluster_limits'].items():
            ...
            print(f"   {cluster_name}: {cluster_info['current_weight']:.1%} / "
    ...
    async def demo_backtest_parity(self, position_targets):
        ...
        if 'total_slippage' in slippage_analysis:
            ...
            print(f"   Binnen budget: {'✅ JA' if slippage_analysis['within_budget'] else '❌ NEE'}")
    ...
    async def demo_canary_deployment(self):
        ...
        print(f"   Staging: {deployment_plan.staging_duration_hours}h @ {deployment_plan.staging_risk_percentage}% risk")
        print(f"   Prod Canary: {deployment_plan.prod_canary_duration_hours}h @ {deployment_plan.prod_canary_risk_percentage}% risk")
        print(f"   Auto-rollback: {'✅ ENABLED' if deployment_plan.auto_rollback_enabled else '❌ DISABLED'}")
    ...
    def demo_return_attribution(self, parity_metrics):
        ...
        print(f"📅 Periode: {attribution.period_start.strftime('%Y-%m-%d')} → {attribution.period_end.strftime('%Y-%m-%d')}")
        print(f"\n📊 Return Components:")
        print(f"   Totaal return: {attribution.total_return:.4f} ({attribution.total_return*100:.2f}%)")
        print(f"   Alpha return: {attribution.alpha_return:.4f} ({attribution.alpha_return*100:.2f}%)")
        print(f"   Market impact: {attribution.market_impact:.4f} ({attribution.market_impact*100:.2f}%)")
        print(f"   Fees impact: {attribution.fees_impact:.4f} ({attribution.fees_impact*10000:.1f} bps)")
        print(f"   Slippage impact: {attribution.slippage_impact:.4f} ({attribution.slippage_impact*10000:.1f} bps)")
        print(f"   Timing impact: {attribution.timing_impact:.4f} ({attribution.timing_impact*10000:.1f} bps)")
        print(f"   Sizing impact: {attribution.sizing_impact:.4f} ({attribution.sizing_impact*100:.2f}%)")
        print(f"   Confidence: {attribution.attribution_confidence:.2f}")
        
        # Attribution visualization
        total_costs = abs(attribution.fees_impact) + abs(attribution.slippage_impact) + abs(attribution.timing_impact)
        ...
        efficiency = (alpha_net / attribution.alpha_return * 100) if attribution.alpha_return != 0 else 0
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            if parity_metrics:
                ...
                print(f"✅ Backtest-Live Parity: {parity_metrics.tracking_error_bps:.1f} bps tracking error {'✅' if tracking_ok else '❌'}")
            else:
