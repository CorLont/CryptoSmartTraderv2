...
class SlippageModel:
    ...
    def calculate_slippage(
        ...
        volume_impact = params["volume_impact"] * np.log(
            1 + order_value / params["liquidity_threshold"]
        )
...
class BacktestingEngine:
    ...
    def execute_order(
        ...
        fees = self.fee_model.calculate_fee(
            order_value=executed_value, order_type=order_type, exchange=exchange, is_maker=is_maker
        )
        ...
        if side == OrderSide.BUY:
            ...
            if required_capital > self.current_capital:
                self.logger.warning(
                    f"Insufficient capital: need {required_capital}, have {self.current_capital}"
                )
                return None
    ...
    def _update_position(self, trade: Trade):
        ...
        if trade.side == OrderSide.BUY:
            ...
            if total_quantity > 0:
                total_cost = (
                    position.quantity * position.avg_price + trade.quantity * trade.executed_price
                )
    ...
    def update_health_score(self, health_score: float):
        ...
        if health_score < self.min_health_score:
            ...
            self.logger.warning(
                f"Trading disabled: health score {health_score} < {self.min_health_score}"
            )
    ...
    def update_drift_score(self, drift_score: float):
        ...
        if drift_score > self.max_drift_score:
            self.is_trading_enabled = False
            self.logger.warning(
                f"Trading disabled: drift score {drift_score} > {self.max_drift_score}"
            )
        ...
        for symbol, position in self.positions.items():
            if symbol in market_prices and position.quantity > 0:
                ...
                position.unrealized_pnl = (
                    market_prices[symbol] - position.avg_price
                ) * position.quantity
