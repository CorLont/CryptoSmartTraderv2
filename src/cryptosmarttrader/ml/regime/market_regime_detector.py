...
class HMMRegimeDetector:
    ...
    def fit(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(X) < 100:
            raise ValueError(
                "Insufficient data for regime detection (need at least 100 observations)"
            )
    ...
    def _interpret_regimes(
        self, X: np.ndarray, regime_states: np.ndarray
        ...
        for regime_id, chars in regime_characteristics.items():
            ...
            if (
                chars.get("returns", 0) > 0.001
                and chars.get("volatility", 0)
                < np.median([c.get("volatility", 0) for c in regime_characteristics.values()])
                and chars.get("price_vs_sma20", 0) > 0
            ):
                regime_mapping[regime_id] = MarketRegime.BULL_MARKET

            # Bear market: negative returns, increasing volatility
            elif chars.get("returns", 0) < -0.001 and chars.get("price_vs_sma20", 0) < 0:
                regime_mapping[regime_id] = MarketRegime.BEAR_MARKET

            # High volatility: high volatility regardless of direction
            elif chars.get("volatility", 0) > np.percentile(
                [c.get("volatility", 0) for c in regime_characteristics.values()], 75
            ):
                regime_mapping[regime_id] = MarketRegime.HIGH_VOLATILITY

            # Low volatility: low volatility, sideways movement
            elif chars.get("volatility", 0) < np.percentile(
                [c.get("volatility", 0) for c in regime_characteristics.values()], 25
            ):
...
class RuleBasedRegimeDetector:
    ...
    def _classify_regime(
        self, volatility: float, trend_strength: float, momentum: float, price_position: float
    ...
    def _estimate_transition_probability(
        self, volatility: float, trend_strength: float, momentum: float
    ) -> float:
...
class MarketRegimeDetector:
    ...
    def _combine_regime_predictions(
        self, regime_states: List[Tuple[str, RegimeState]]
    ) -> RegimeState:
    ...
    def _check_regime_transition(self, prev_regime: Dict, curr_regime: Dict):
        ...
        if prev_regime["regime"] != curr_regime["regime"]:
            ...
            self.logger.info(
                f"Regime transition detected: {prev_regime['regime'].value} → {curr_regime['regime'].value}"
            )
    ...
    def get_regime_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_transitions": [
                {
                    "from_address": t.from_regime.value,
...
def create_regime_detector(
    use_hmm: bool = True, use_rules: bool = True, hmm_regimes: int = 4
) -> MarketRegimeDetector:
    """Create configured market regime detector"""

    return MarketRegimeDetector(use_hmm=use_hmm, use_rules=use_rules, hmm_regimes=hmm_regimes)


def detect_market_regime(data: pd.DataFrame, method: str = "combined") -> RegimeState:
