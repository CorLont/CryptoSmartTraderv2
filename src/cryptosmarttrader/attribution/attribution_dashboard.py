...
from .return_attribution import ReturnAttributionAnalyzer, AttributionPeriod, ReturnAttributionReport
...
class AttributionDashboard:
            ...
            df_components = pd.DataFrame([
                {
                    ...
                    "% of Total": f"{comp[1]/report.total_return_bps*100:.1f}%" if report.total_return_bps != 0 else "0.0%"
                }
    ...
    def render_cost_analysis(self, report: ReturnAttributionReport):
        ...
        if report.fees_component.contribution_bps < -1:
            ...
            if fees_meta.get('maker_ratio', 0) < 0.7:
                st.warning("⚠️ Low maker ratio detected. Consider using more limit orders to reduce fees.")
    ...
    def generate_demo_attribution_report(self, period: AttributionPeriod) -> ReturnAttributionReport:
...
def main():
    """Main Streamlit app."""
    st.set_page_config(
        page_title="Return Attribution Dashboard",
        page_icon="🎯",
        layout="wide"
    )
