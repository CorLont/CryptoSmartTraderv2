...
from core.system_health_monitor import (
    get_system_health_monitor, HealthConfig, HealthStatus, ComponentStatus
)
...
async def run_system_health_check(detailed: bool = True):
    ...
    try:
        ...
        print(f"   {live_icon} Live Trading: {'AUTHORIZED' if health_report.live_trading_authorized else 'BLOCKED'}")
        print(f"   {paper_icon} Paper Trading: {'AUTHORIZED' if health_report.paper_trading_authorized else 'BLOCKED'}")
        ...
        if detailed:
            ...
            healthy_count = sum(1 for h in health_report.component_health.values() if h.status == ComponentStatus.HEALTHY)
            degraded_count = sum(1 for h in health_report.component_health.values() if h.status == ComponentStatus.DEGRADED)
            failed_count = sum(1 for h in health_report.component_health.values() if h.status == ComponentStatus.FAILED)
        ...
        if detailed:
            ...
            print(f"     WARNING (Paper Only): {config.warning_threshold:.0f}-{config.go_threshold:.0f}")
...
async def run_quick_health_check():
    ...
    try:
        ...
        print(f"📝 Paper Trading: {'✅ YES' if health_report.paper_trading_authorized else '❌ NO'}")
...
async def run_trading_authorization_check():
    ...
    try:
        ...
        print(f"   Live Trading: {'✅ AUTHORIZED' if auth_status['live_trading_authorized'] else '❌ BLOCKED'}")
        print(f"   Paper Trading: {'✅ AUTHORIZED' if auth_status['paper_trading_authorized'] else '❌ BLOCKED'}")
...
async def monitor_health_continuously(interval_minutes: int = 15):
    ...
    while True:
        try:
            ...
            print(f"   {icon} {health_report.overall_health_status.value.upper()} - Score: {health_report.overall_health_score:.1f}")
            print(f"   🚦 Live: {'✅' if health_report.live_trading_authorized else '❌'} | Paper: {'✅' if health_report.paper_trading_authorized else '❌'}")
...
Examples:
  ...
  python scripts/system_health_check.py monitor            # Continuous monitoring
    ...
    check_parser.add_argument(
        '--summary',
        action='store_true',
        help='Show summary view instead of detailed breakdown'
    )
