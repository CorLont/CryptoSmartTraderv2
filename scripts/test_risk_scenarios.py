...
class RiskScenarioTester:
    ...
    def test_daily_loss_scenarios(self) -> bool:
        ...
        for equity, expected_risk, expected_kill in test_cases:
            ...
            if guard.current_risk_level != expected_risk:
                print(
                    f"   ❌ Daily loss {equity}: Expected {expected_risk}, got {guard.current_risk_level}"
                )
                return False

            if guard.kill_switch_active != expected_kill:
                print(
                    f"   ❌ Kill switch {equity}: Expected {expected_kill}, got {guard.kill_switch_active}"
                )
                return False

            print(
                f"   ✅ Daily loss ${equity}: {expected_risk.value} + kill_switch={expected_kill}"
            )
    ...
    def test_drawdown_scenarios(self) -> bool:
        ...
        for equity, dd_pct, expected_risk, expected_kill in test_cases:
            ...
            if guard.current_risk_level != expected_risk:
                print(
                    f"   ❌ Drawdown {dd_pct}%: Expected {expected_risk}, got {guard.current_risk_level}"
                )
                return False

            if guard.kill_switch_active != expected_kill:
                print(
                    f"   ❌ Kill switch {dd_pct}%: Expected {expected_kill}, got {guard.kill_switch_active}"
                )
                return False
    ...
    def test_api_reliability_scenarios(self) -> bool:
        ...
        if not guard.kill_switch_active:
            print(
                f"   ❌ API reliability kill switch not activated (success rate: {guard.api_success_count / guard.api_total_count:.1%})"
            )
            return False

        print(
            f"   ✅ API reliability kill switch: ACTIVATED (success rate: {guard.api_success_count / guard.api_total_count:.1%})"
        )
        return True
    ...
    def test_flash_crash_scenario(self) -> bool:
        ...
        for i, equity in enumerate(decline_steps):
            ...
            print(
                f"   Step {i + 1}: ${equity} (loss: {loss_pct:.1%}, dd: {dd_pct:.1%}) -> {guard.current_risk_level.value}"
            )
    ...
    def test_edge_cases(self) -> bool:
        ...
        for equity, expected_level in test_cases:
            ...
            if guard.current_risk_level != expected_level:
                print(
                    f"   ❌ Threshold boundary ${equity}: Expected {expected_level}, got {guard.current_risk_level}"
                )
                return False
