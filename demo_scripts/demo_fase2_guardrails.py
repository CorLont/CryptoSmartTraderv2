...
class Fase2Demo:
    ...
    async def demo_simulation_scenarios(self):
        ...
        for name, scenario, intensity, duration in scenarios:
            ...
            print(
                f"   Kill switch: {'✅ GEACTIVEERD' if result.kill_switch_activated else '❌ NIET GEACTIVEERD'}"
            )
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            print(
                f"✅ Prometheus: {len(self.metrics.registry._collector_to_names)} metrics verzameld"
            )
            ...
            print(
                f"🔴 Kill Switch: {'ACTIEF' if self.risk_guard.kill_switch_active else 'INACTIEF'}"
            )
