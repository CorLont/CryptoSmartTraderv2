...
class DashboardGateChecker:
    ...
    def _check_health_gate(self, health_file: str) -> Dict[str, Any]:
        ...
        gate_result = {"gates_checked": [gate_name], "gates_passed": [], "gates_failed": []}
        ...
        try:
            ...
            if health_score >= 60:
                ...
                gate_result["gates_failed"].append(gate_name)
                gate_result["empty_state_reason"] = (
                    f"System health too low: {health_score:.1f} < 60.0"
                )
    ...
    def _check_confidence_gate(self, predictions_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(high_confidence_recommendations) >= 1:
                gate_result["gates_passed"].append(gate_name)
                self.logger.info(
                    f"Confidence gate passed: {len(high_confidence_recommendations)} high-confidence recommendations"
                )
            else:
                gate_result["gates_failed"].append(gate_name)
    ...
    def _check_completeness_gate(self, predictions_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if overall_completeness >= 0.90:
                ...
                gate_result["gates_failed"].append(gate_name)
                gate_result["empty_state_reason"] = (
                    f"Incomplete features: {overall_completeness:.1%} < 90%"
                )
    ...
    def _make_final_gate_decision(self, gate_results: Dict[str, Any]) -> None:
        ...
        if failed_gates == 0 and passed_gates == total_gates:
            ...
            self.logger.warning(
                f"Gates failed: {failed_gates}/{total_gates} - no recommendations will be displayed"
            )
...
def print_gate_status(gate_results: Dict[str, Any]) -> None:
    ...
    decision_icon = {"display": "✅", "blocked": "❌", "error": "🚨"}.get(decision, "❓")
    ...
    if "health_status" in gate_results:
        health_icon = {"GO": "🟢", "WARNING": "🟡", "NO-GO": "🔴"}.get(
            gate_results["health_status"], "❓"
        )
    ...
    if "high_confidence_count" in gate_results:
        print(
            f"🎯 High Confidence: {gate_results['high_confidence_count']}/{gate_results.get('total_predictions', 0)} predictions"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Dashboard Gate Checker - Pre-Display Validation")
