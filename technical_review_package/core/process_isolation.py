...
class AgentProcess:
    ...
    def __init__(self, config: AgentConfig):
        ...
        self.backoff = ExponentialBackoff(config.restart_delay_base, config.restart_delay_max)
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.process = mp.Process(
                target=self._run_agent, name=f"Agent_{self.config.name}", daemon=False
            )
    ...
    def restart(self) -> bool:
        ...
        if delay > 0:
            self.logger.info(
                f"Waiting {delay:.1f}s before restart (attempt {self.status.restart_count + 1})"
            )
            time.sleep(delay)
...
class ProcessIsolationManager:
    ...
    async def _monitor_agents(self) -> None:
        ...
        for name, agent in self.agents.items():
            try:
                ...
                if (
                    agent.process
                    and not agent.process.is_alive()
                    and agent.status.state == AgentState.RUNNING
                ):
                ...
                if should_restart and agent.config.restart_policy == "always":
                    if agent.status.restart_count < agent.config.max_restarts:
                        ...
                        self.logger.error(
                            f"Agent {name} exceeded max restarts ({agent.config.max_restarts})"
                        )
                        agent.status.state = AgentState.FAILED
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        running_agents = sum(
            1 for agent in self.agents.values() if agent.status.state == AgentState.RUNNING
        )
        failed_agents = sum(
            1 for agent in self.agents.values() if agent.status.state == AgentState.FAILED
        )
...
if __name__ == "__main__":
    ...
    async def test_process_isolation():
        ...
        for i in range(6):
            ...
            print(
                f"   Health check {i + 1}: {health['running_agents']}/{health['total_agents']} running "
                f"({health['health_percentage']:.1f}%)"
            )

            # Show agent status
            for name, status in manager.get_all_status().items():
                state_emoji = {"running": "🟢", "failed": "🔴", "restarting": "🟡"}.get(
                    status.state.value, "⚪"
                )
                print(
                    f"     {state_emoji} {name}: {status.state.value} (restarts: {status.restart_count})"
                )
