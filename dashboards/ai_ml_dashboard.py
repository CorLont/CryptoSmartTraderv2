...
class AIMLDashboard:
    ...
    def render(self):
        ...
        st.set_page_config(
            page_title="AI/ML Engine - CryptoSmartTrader V2",
            page_icon="🤖",
            layout="wide"
        )
    ...
    def _render_deep_learning_tab(self):
        ...
        with col1:
            selected_coin = st.selectbox("Select Coin", ["BTC", "ETH", "ADA", "SOL", "MATIC"], key="dl_coin")
        
        with col2:
            model_type = st.selectbox("Model Type", ["LSTM", "Transformer", "N-BEATS"], key="dl_model")
        ...
        try:
            ...
            with col1:
                st.metric("GPU Available", "✅ Yes" if training_status.get('gpu_available') else "❌ No")
            
            with col2:
                st.metric("Training Active", "🏃 Running" if training_status.get('is_training') else "⏸️ Idle")
            ...
            if training_status.get('model_performance'):
                ...
                st.info("No trained models available yet. Start training to see performance metrics.")
    ...
    def _render_automl_tab(self):
        ...
        with col1:
            selected_coin = st.selectbox("Select Coin", ["BTC", "ETH", "ADA", "SOL"], key="automl_coin")
        
        with col2:
            n_trials = st.number_input("Optimization Trials", min_value=10, max_value=200, value=50, key="automl_trials")
        ...
        try:
            ...
            with col1:
                st.metric("GPU Available", "✅ Yes" if automl_status.get('gpu_available') else "❌ No")
            ...
            if automl_status.get('available_models'):
                ...
                for i, model in enumerate(automl_status['available_models']):
                    with model_cols[i]:
                        gpu_icon = "⚡" if model in ['lightgbm', 'xgboost'] and automl_status['gpu_available'] else "🖥️"
    ...
    def _render_gpu_accelerator_tab(self):
        ...
        with col1:
            data_size = st.slider("Benchmark Data Size", min_value=1000, max_value=100000, value=10000, step=1000)
        ...
        try:
            ...
            with col2:
                st.metric("CuPy Available", "✅ Yes" if gpu_status.get('cupy_available') else "❌ No")
            
            with col3:
                st.metric("RAPIDS Available", "✅ Yes" if gpu_status.get('rapids_available') else "❌ No")
            
            with col4:
                st.metric("Numba Available", "✅ Yes" if gpu_status.get('numba_available') else "❌ No")
            ...
            if perf_stats:
                ...
                with stat_cols[3]:
                    total_ops = perf_stats.get('gpu_operations', 0) + perf_stats.get('cpu_fallbacks', 0)
    ...
    def _render_ml_performance_tab(self):
        ...
        try:
            ...
            fig_performance.add_trace(
                go.Scatter(x=hours, y=accuracy, mode='lines+markers', name='Accuracy', line=dict(color='green')),
                row=1, col=1
            )
            
            fig_performance.add_trace(
                go.Scatter(x=hours, y=inference_time, mode='lines+markers', name='Inference Time', line=dict(color='blue')),
                row=2, col=1
            )
            
            fig_performance.add_trace(
                go.Scatter(x=hours, y=gpu_usage, mode='lines+markers', name='GPU Usage', line=dict(color='red')),
                row=3, col=1
            )
            ...
            model_comparison = pd.DataFrame({
                'Model': ['LSTM', 'Transformer', 'N-BEATS', 'LightGBM', 'XGBoost', 'Random Forest'],
