...
class StrategyInterface:
    ...
    def generate_signals(
        self, market_data: pd.DataFrame, portfolio: Dict[str, Position], timestamp: datetime
    ) -> List[Dict[str, Any]]:
    ...
    def calculate_position_size(
        self, signal: Dict[str, Any], portfolio_value: float, risk_budget: float
    ) -> float:
...
class BacktestEngine:
            ...
            try:
                ...
                if portfolio_value < config.initial_capital * 0.5:  # 50% drawdown limit
                    self.logger.warning(
                        ...
                        extra={
                            "portfolio_value": portfolio_value,
                            "drawdown": (config.initial_capital - portfolio_value)
    ...
    def _update_portfolio_positions(
        self, portfolio: Dict[str, Position], market_row: pd.Series, cash: float
    ) -> float:
    ...
    def _simple_execute_order(
        self, order: Order, current_price: float, config: BacktestConfig
    ) -> Order:
    ...
    def _calculate_performance_metrics(
        self, portfolio_df: pd.DataFrame, config: BacktestConfig
    ) -> Dict[str, float]:
    ...
    def _calculate_risk_metrics(
        self, portfolio_df: pd.DataFrame, trades: List[BacktestTrade], config: BacktestConfig
    ) -> Dict[str, float]:
    ...
    def _record_slo_performance(self, results: BacktestResults, ml_predictions: pd.DataFrame):
                ...
                for horizon in ["1h", "24h"]:
                    if len(predictions) > 0 and len(actuals) > 0:
                        self.slo_monitor.record_performance(
                            ...
                            * min(100, len(predictions)),  # Default confidence
                        )

        except Exception as e:
            self.logger.warning(f"Failed to record SLO performance: {e}")

    def compare_backtests(self, backtest_results: List[BacktestResults]) -> Dict[str, Any]:
        ...
        comparison["ranking"] = [
            {"rank": rank + 1, "backtest_index": backtest_idx, "avg_ranking_score": score}
    ...
    def get_backtest_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_backtest": {
                "total_return": self.current_backtest.performance_metrics.get("total_return", 0)
                if self.current_backtest
                else 0,
                "sharpe_ratio": self.current_backtest.performance_metrics.get("sharpe_ratio", 0)
                if self.current_backtest
                else 0,
                "max_drawdown": self.current_backtest.risk_metrics.get("max_drawdown", 0)
                if self.current_backtest
                else 0,
