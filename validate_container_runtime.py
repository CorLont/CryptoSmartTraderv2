...
def validate_docker_configuration():
    ...
    if dockerfile.exists():
        ...
        print(f"   {'✅' if has_pinned_image else '❌'} Pinned base image (python:3.11.10-slim-bookworm)")
    ...
    if k8s_dir.exists():
        ...
        if deployment_file.exists():
            ...
            has_probes = all(probe in content for probe in ['livenessProbe:', 'readinessProbe:', 'startupProbe:'])
    ...
    for script in deployment_scripts:
        ...
        if exists and script.endswith('.sh'):
            ...
            print(f"   {'✅' if is_executable else '❌'} {script} (executable: {bool(is_executable)})")
        else:
