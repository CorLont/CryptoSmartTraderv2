...
def test_temporal_validator():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("temporal_validator", "core/temporal_integrity_validator.py")
        ...
        print(f"   String timestamps handled: {not result['is_valid'] and 'invalid timestamp' in str(result['violations'])}")
        ...
        utc_details = next((detail for detail in result.get('validation_details', []) 
        ...
        future_details = next((detail for detail in result.get('validation_details', []) 
