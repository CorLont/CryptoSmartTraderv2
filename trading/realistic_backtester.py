...
from trading.orderbook_simulator import OrderBookSimulator, ExchangeConfig, OrderSide, OrderType, OrderStatus
...
class RealisticBacktester:
    ...
    def run_backtest(
        ...
        for i, (timestamp, price_row) in enumerate(price_aligned.iterrows()):
            ...
            current_volatility = vol_aligned.loc[timestamp, 'volatility'] if timestamp in vol_aligned.index else 0.02
    ...
    def _process_trading_signal(
        ...
        if signal > 0.5:  # Buy signal
            if symbol not in self.positions or self.positions[symbol].quantity <= 0:
                self._execute_buy_order(symbol, current_price, current_volume, volatility, timestamp)
                
        elif signal < -0.5:  # Sell signal
            if symbol in self.positions and self.positions[symbol].quantity > 0:
                self._execute_sell_order(symbol, current_price, current_volume, volatility, timestamp)
        ...
        if order.status in [OrderStatus.FILLED, OrderStatus.PARTIAL]:
            ...
            if fills:
                ...
                if symbol in self.positions:
                    ...
                    total_cost_new = (existing_pos.quantity * existing_pos.avg_entry_price) + total_cost
                ...
                self.logger.debug(f"Buy executed: {total_quantity:.6f} @ {avg_price:.2f}, "
    ...
    def _execute_sell_order(
        ...
        if order.status in [OrderStatus.FILLED, OrderStatus.PARTIAL]:
            ...
            if fills:
                ...
                self.logger.debug(f"Sell executed: {total_quantity:.6f} @ {avg_price:.2f}, "
