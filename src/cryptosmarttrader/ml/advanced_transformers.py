...
class TemporalFusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.output_layers = nn.ModuleList([nn.Linear(hidden_size, output_size) for _ in quantiles])

        # Position encoding
        self.pos_encoding = PositionalEncoding(hidden_size, dropout)

    def forward(
        self, dynamic_inputs: torch.Tensor, static_inputs: torch.Tensor, decoder_length: int
        ...
        static_context_expanded = static_context.unsqueeze(1).expand(batch_size, seq_len, -1)
        ...
        for _ in range(decoder_length):
            ...
            decoder_input_with_context = torch.cat(
                [decoder_input, hidden.transpose(0, 1)[:, -1:, :]], dim=-1
            )
        ...
        return {
            ...
            "variable_selection": {"dynamic": selected_dynamic, "static": selected_static},
        }
...
class NBEATS(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, stacks: List[Dict[str, Any]] = None):
        super().__init__()
...
class AdvancedTransformerPredictor:
    ...
    def __init__(
        self, input_size: int, static_size: int = 10, output_horizons: List[int] = [1, 24, 168, 720]
    ):
        self.input_size = input_size
        self.static_size = static_size
        ...
        self.nbeats_model = NBEATS(input_size=input_size, output_size=max(output_horizons))
    ...
    def train_models(
        ...
        for epoch in range(epochs):
            ...
            nbeats_loss = criterion(
                nbeats_outputs[:, : len(self.output_horizons)], train_data["targets"]
            )
            ...
            if epoch % 10 == 0:
                ...
                print(
                    f"Epoch {epoch}: TFT: {tft_loss.item():.4f}, "
    ...
    def _validate(self, val_data: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            nbeats_loss = F.mse_loss(
                nbeats_outputs[:, : len(self.output_horizons)], val_data["targets"]
            )
    ...
    def predict(
        self, dynamic_inputs: torch.Tensor, static_inputs: torch.Tensor
    ) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            # TFT predictions
            tft_outputs = self.tft_model(dynamic_inputs, static_inputs, len(self.output_horizons))
...
if __name__ == "__main__":
    ...
    predictor = AdvancedTransformerPredictor(
        input_size=input_size, static_size=static_size, output_horizons=output_horizons
    )
    ...
    train_data = {"dynamic": dynamic_data, "static": static_data, "targets": targets}

    val_data = {"dynamic": dynamic_data[:16], "static": static_data[:16], "targets": targets[:16]}
