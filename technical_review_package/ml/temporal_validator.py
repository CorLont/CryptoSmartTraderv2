...
class TemporalValidator:
        ...
        try:
            ...
            if timestamp_col not in df.columns:
                return ValidationResult(
                    passed=False, message=f"Timestamp column '{timestamp_col}' not found"
                )
            ...
            passed = (
                not leakage_results["has_leakage"] and duplicates == 0 and len(high_nan_cols) == 0
            )
            ...
            return ValidationResult(
                passed=False, message=f"Validation error: {str(e)}", details={"error": str(e)}
            )

    def _check_feature_leakage(
        self, df: pd.DataFrame, timestamp_col: str, feature_cols: List[str], max_lag_minutes: int
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if high_future_corr:
                    ...
                    leakage_details[feature] = {
                        ...
                        "max_future_correlation": max(future_corrs.values(), key=abs)
                        if future_corrs
                        else 0,
                    }
        ...
        return {"has_leakage": has_leakage, "details": leakage_details}
    ...
    def create_time_series_splits(
        ...
        for i in range(n_splits):
            ...
            test_mask = (df_sorted[timestamp_col] >= test_start_date) & (
    ...
    def validate_feature_engineering(
        self, df: pd.DataFrame, feature_engineering_func, timestamp_col: str = "timestamp"
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            if not new_cols:
                return ValidationResult(
                    passed=True, message="No new features created", details={"new_features": []}
                )
        ...
        try:
            ...
            for feature in feature_cols:
                ...
                if pd.api.types.is_numeric_dtype(ref_values):
                    ...
                    feature_drift = (
                        sum(abs(ref_dist.get(val, 0) - curr_dist.get(val, 0)) for val in all_values)
                        / 2
                    )
