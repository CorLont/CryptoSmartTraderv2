    ...
    def register_secret(self, secret: str, identifier: str = None):
        ...
        if secret and len(secret) >= 8:  # Only register substantial secrets
            ...
            if self.enable_audit:
                self.logger.debug(f"Registered secret for sanitization: {identifier or secret_hash}")
    ...
    def _init_default_rules(self):
        ...
        self.sanitization_rules.extend([
            ...
            SanitizationRule(
                ...
                pattern=re.compile(r'(?:api[_-]?key|apikey|access[_-]?token)[\s]*[:=][\s]*["\']?([A-Za-z0-9+/]{20,})["\']?', re.IGNORECASE),
        ...
        self.sanitization_rules.extend([
            SanitizationRule(
                ...
                pattern=re.compile(r'(?:secret|signature)[\s]*[:=][\s]*["\']?([A-Za-z0-9+/]{20,})["\']?', re.IGNORECASE),
        ...
        self.sanitization_rules.extend([
            SanitizationRule(
                name="large_amounts",
                pattern=re.compile(r'\$[\d,]+\.?\d*(?:\s*million|\s*M|\s*billion|\s*B)?(?=\s|$)', re.IGNORECASE),
        ...
        self.sanitization_rules.extend([
            SanitizationRule(
                name="order_ids",
                pattern=re.compile(r'(?:order[_-]?id|orderId)[\s]*[:=][\s]*["\']?([A-Za-z0-9-]{10,})["\']?', re.IGNORECASE),
            ...
            SanitizationRule(
                name="user_ids",
                pattern=re.compile(r'(?:user[_-]?id|userId|customer[_-]?id)[\s]*[:=][\s]*["\']?([A-Za-z0-9-]{8,})["\']?', re.IGNORECASE),
    ...
    def _sanitize_unknown_secrets(self, text: str) -> str:
        ...
        for secret in self.known_secrets:
            if secret in sanitized_text:
                ...
                # Update stats
                self.sanitization_stats['known_secrets'] = self.sanitization_stats.get('known_secrets', 0) + 1
        
        return sanitized_text
    
    def _audit_sanitization(self, 
                          original: str, 
                          sanitized: str, 
...
class ExchangeComplianceMonitor:
    ...
    def _init_compliance_rules(self) -> Dict[str, Callable[[str], bool]]:
        ...
        return {
            ...
            'general_market_manipulation': lambda msg: self._check_market_manipulation_indicators(msg),
        }
    ...
    def _check_market_manipulation_indicators(self, msg: str) -> bool:
        ...
        manipulation_patterns = [
            r'pump.*dump',
            r'artificial.*volume',
            r'coordinated.*trading'
        ]
        
        return any(re.search(pattern, msg, re.IGNORECASE) for pattern in manipulation_patterns)


def create_log_sanitizer(config_path: Optional[str] = None, enable_audit: bool = True) -> LogSanitizer:
