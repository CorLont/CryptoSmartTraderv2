...
async def test_complete_orderbook_slippage_paper():
    ...
    tests = [
        (
            "L2 Orderbook Simulator (L2 depth + partial fills + fees + TIF + latency)",
...
async def test_orderbook_simulator():
    ...
    try:
        ...
        exchange_config = ExchangeConfig(
            name="test_exchange", maker_fee=0.001, taker_fee=0.0015, latency_mean_ms=30.0
        )

        simulator = OrderBookSimulator("BTC/USD", exchange_config)

        print("   📊 Generating L2 order book...")
        orderbook = simulator.generate_realistic_orderbook(
            50000.0, volatility=0.005, depth_levels=20
        )
        simulator.update_orderbook(orderbook)

        market_summary = simulator.get_market_summary()
        print(
            f"   Market: {market_summary['market_data']['bid_price']:.2f}/{market_summary['market_data']['ask_price']:.2f}"
        )
    ...
    try:
        ...
        for i in range(150):
            ...
            order_size = np.random.choice(
                [0.01, 0.05, 0.1, 0.5, 1.0, 2.0], p=[0.3, 0.2, 0.2, 0.15, 0.1, 0.05]
            )
...
async def test_paper_trading():
    ...
    try:
        ...
        engine = PaperTradingEngine(
            {"validation_criteria": validation_criteria, "starting_balance": 100000.0}
        )
        ...
        if final_status:
            ...
            complete_logging = (
                final_status["total_trades"] > 0
                and "final_metrics" in final_status
                and "average_slippage_bps" in final_status["final_metrics"]
                and "average_latency_ms" in final_status["final_metrics"]
            )
...
async def test_integration():
    ...
    try:
        ...
        print(
            f"      Baseline comparison: {'ABOVE' if precision_at_k > baseline_precision else 'BELOW'} baseline"
        )
