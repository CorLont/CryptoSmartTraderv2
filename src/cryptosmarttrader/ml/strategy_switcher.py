...
class StrategySwitcher:
    ...
    def select_strategy(self, regime_classification: RegimeClassification) -> Optional[RegimeStrategy]:
        ...
        try:
            ...
            if confidence < regime_strategy.confidence_threshold:
                logger.info(f"Regime confidence ({confidence:.2f}) below threshold ({regime_strategy.confidence_threshold:.2f})")
            ...
            if (regime_classification.regime_duration_minutes < self.min_regime_duration_minutes and 
                self.active_strategy and 
                self.active_strategy.regime != regime):
                
                logger.info(f"Regime duration ({regime_classification.regime_duration_minutes}min) too short for strategy switch")
    ...
    def calculate_position_size(self, 
        ...
        try:
            ...
            signal_adjusted = strategy_adjusted * min(signal_strength / params.entry_signal_strength, 1.0)
    ...
    def get_take_profit_levels(self, 
        ...
        try:
            ...
            for i, (tp_pct, scaling) in enumerate(zip(params.partial_profit_levels, params.profit_scaling)):
                if side.lower() == "buy":
    ...
    def update_strategy_performance(self, 
        ...
        try:
            ...
            strategy.turnover_rate = strategy.trades_executed / max(1, 
                (datetime.now() - (datetime.now() - timedelta(days=1))).total_seconds() / 86400
            )
    ...
    def _record_strategy_change(self, regime_classification: RegimeClassification, new_strategy: RegimeStrategy):
    ...
    def optimize_strategy_parameters(self, regime: MarketRegime, performance_window_days: int = 30):
        ...
        try:
            ...
            recent_trades = [
                p for p in self.performance_history 
                if p.get("regime") == regime.value and 
            ...
            if win_rate < 0.4:  # Low win rate, tighten stops
                ...
                logger.info(f"Tightened stops for {regime.value}: {strategy.primary_strategy.stop_loss_pct:.2f}%")
            elif win_rate > 0.7:  # High win rate, widen stops
                strategy.primary_strategy.stop_loss_pct *= 1.1
                logger.info(f"Widened stops for {regime.value}: {strategy.primary_strategy.stop_loss_pct:.2f}%")
            ...
            strategy.primary_strategy.stop_loss_pct = np.clip(strategy.primary_strategy.stop_loss_pct, 0.5, 5.0)
            strategy.primary_strategy.base_position_size = np.clip(strategy.primary_strategy.base_position_size, 0.1, 2.0)
    ...
    def get_strategy_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "strategy_type": self.active_strategy.primary_strategy.strategy_type.value if self.active_strategy else None,
            ...
            "current_parameters": {
                "position_size": self.active_strategy.primary_strategy.base_position_size if self.active_strategy else 0,
                "stop_loss_pct": self.active_strategy.primary_strategy.stop_loss_pct if self.active_strategy else 0,
                "take_profit_pct": self.active_strategy.primary_strategy.take_profit_pct if self.active_strategy else 0,
                "max_daily_trades": self.active_strategy.primary_strategy.max_daily_trades if self.active_strategy else 0
            }
        }