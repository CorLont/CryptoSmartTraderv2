...
class OrderBookSimulator:
    ...
    def update_orderbook(self, snapshot: OrderBookSnapshot) -> None:
        ...
        if snapshot.bids and snapshot.asks:
            ...
            self.logger.debug(
                f"Updated order book: bid={self.bid_price:.6f}, "
                f"ask={self.ask_price:.6f}, spread={self.spread:.6f}"
            )

    def generate_realistic_orderbook(
        self, base_price: float, volatility: float = 0.01, depth_levels: int = 20
    ) -> OrderBookSnapshot:
    ...
    def submit_order(
        ...
        self.logger.info(
            f"Order submitted: {order_id} - {side.value} {quantity} {self.symbol} "
    ...
    def _validate_order(self, order: Order) -> bool:
        ...
        try:
            ...
            if order.quantity < self.exchange_config.min_order_size:
                self.logger.warning(
                    f"Order size {order.quantity} below minimum "
            ...
            if order.price is not None:
                ...
                if price_decimals > self.exchange_config.price_precision:
                    self.logger.warning(
                        f"Price precision {price_decimals} exceeds maximum "
            ...
            if qty_decimals > self.exchange_config.quantity_precision:
                self.logger.warning(
                    f"Quantity precision {qty_decimals} exceeds maximum "
    ...
    def _process_order_immediately(self, order: Order) -> None:
        ...
        try:
            ...
            book_levels = (
                self.current_book.asks if order.side == OrderSide.BUY else self.current_book.bids
            )
            ...
            self.logger.info(
                f"Order processed: {order.order_id} - {order.status.value}, "
    ...
    def _simulate_latency(self) -> float:
        ...
        if np.random.random() < 0.05:  # 5% chance of high latency
            ...
            latency = np.random.normal(
                self.exchange_config.latency_mean_ms, self.exchange_config.latency_std_ms
            )
    ...
    def _calculate_fee(self, notional_value: float, is_taker: bool = True) -> float:
        ...
        fee_rate = self.exchange_config.taker_fee if is_taker else self.exchange_config.maker_fee
...
if __name__ == "__main__":
    ...
    async def test_orderbook_simulator():
        ...
        exchange_config = ExchangeConfig(
            name="test_exchange", maker_fee=0.001, taker_fee=0.0015, latency_mean_ms=25.0
        )
        ...
        for order_id in [buy_order_id, sell_order_id, ioc_order_id]:
            ...
            if status:
                ...
                if status["filled_quantity"] > 0:
                    print(
                        f"      Avg price: ${status['average_fill_price']:.2f}, "
        ...
        for fill in fills:
            print(
                f"   {fill['side']} {fill['quantity']:.6f} at ${fill['price']:.2f} "
                f"(fee: ${fill['fee']:.4f})"
            )
