...
class DailyDashboardIntegrator:
            ...
            flattened.update(
                {
                    ...
                    "validation_accuracy_score": health.get("component_scores", {}).get(
                        "validation_accuracy", 0
                    ),
                    "sharpe_norm_score": health.get("component_scores", {}).get("sharpe_norm", 0),
                    "feedback_success_score": health.get("component_scores", {}).get(
                        "feedback_success", 0
                    ),
                    "error_ratio_score": health.get("component_scores", {}).get("error_ratio", 0),
                    "data_completeness_score": health.get("component_scores", {}).get(
                        "data_completeness", 0
                    ),
                    "tuning_freshness_score": health.get("component_scores", {}).get(
                        "tuning_freshness", 0
                    ),
                }
            )
    ...
    def generate_health_trend_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        if df.empty or "health_score" not in df.columns:
            return go.Figure().add_annotation(
                text="No health data available", xref="paper", yref="paper", x=0.5, y=0.5
            )
        ...
        # Add threshold lines
        fig.add_hline(
            y=85, line_dash="dash", line_color="green", annotation_text="GO Threshold (85)"
        )
        fig.add_hline(
            y=60, line_dash="dash", line_color="orange", annotation_text="Warning Threshold (60)"
        )
    ...
    def generate_performance_metrics_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        if df.empty:
            return go.Figure().add_annotation(
                text="No performance data available", xref="paper", yref="paper", x=0.5, y=0.5
            )
        ...
        # Add target lines
        fig.add_hline(
            y=60, line_dash="dash", line_color="blue", annotation_text="Precision Target (60%)"
        )
        fig.add_hline(
            y=55, line_dash="dash", line_color="green", annotation_text="Hit Rate Target (55%)"
        )
    ...
    def generate_coverage_trend_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        if df.empty or "coverage_percentage" not in df.columns:
            return go.Figure().add_annotation(
                text="No coverage data available", xref="paper", yref="paper", x=0.5, y=0.5
            )
        ...
        fig.add_hline(y=95, line_dash="dash", line_color="orange", annotation_text="Minimum (95%)")
    ...
    def generate_operational_status_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        if df.empty or "operational_status" not in df.columns:
            return go.Figure().add_annotation(
                text="No operational status data available",
                xref="paper",
                yref="paper",
                x=0.5,
                y=0.5,
    ...
    def generate_component_scores_heatmap(self, df: pd.DataFrame) -> go.Figure:
        ...
        if df.empty or not available_cols:
            return go.Figure().add_annotation(
                text="No component score data available", xref="paper", yref="paper", x=0.5, y=0.5
            )
        ...
        if "precision_at_k" in df.columns:
            ...
            if not precision_data.empty:
                summary["performance"] = {
                    ...
                    "current_precision": round(precision_data.iloc[-1], 3)
                ...
                summary.setdefault("performance", {}).update(
                    {
                        ...
                        "current_hit_rate": round(hit_rate_data.iloc[-1], 3)
        ...
        if "coverage_percentage" in df.columns:
            ...
            if not coverage_data.empty:
                summary["coverage"] = {
                    ...
                    "current_coverage": round(coverage_data.iloc[-1], 1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Daily Dashboard Integration - Historical Metrics")

    parser.add_argument("--days", type=int, default=30, help="Number of days to load (default: 30)")

    parser.add_argument("--export-charts", action="store_true", help="Export charts as HTML files")
