...
class GeneticFeatureEvolver:
    ...
    def evolve_features(
        self, parent_features: List[FeatureCandidate], performance_scores: Dict[str, float]
    ...
    def _select_elite(
        self, features: List[FeatureCandidate], performance_scores: Dict[str, float]
    ) -> List[FeatureCandidate]:
    ...
    def _select_parents(
        self, features: List[FeatureCandidate], performance_scores: Dict[str, float]
    ) -> Tuple[FeatureCandidate, FeatureCandidate]:
        """Select parents using tournament selection"""
        try:

            def tournament_select(tournament_size: int = 3) -> FeatureCandidate:
                tournament = np.random.choice(
                    features, size=min(tournament_size, len(features)), replace=False
                )
                best = max(
    ...
    def _crossover(
        self, parent1: FeatureCandidate, parent2: FeatureCandidate
...
class FeatureDiscoveryEngine:
        ...
        with self._lock:
            try:
                ...
                if self.discovery_mode == DiscoveryMode.EXPLORATORY:
                    new_candidates = self._exploratory_discovery(
                        data, target_column, baseline_features
                    )
                elif self.discovery_mode == DiscoveryMode.EXPLOITATIVE:
                    new_candidates = self._exploitative_discovery(
                        data, target_column, baseline_features
                    )
                elif self.discovery_mode == DiscoveryMode.REGIME_ADAPTIVE:
                    new_candidates = self._regime_adaptive_discovery(
                        data, target_column, baseline_features
                    )
                else:  # PERFORMANCE_DRIVEN
                    new_candidates = self._performance_driven_discovery(
                        data, target_column, baseline_features
                    )

                # Evaluate candidates
                evaluated_candidates = self._evaluate_candidates(
                    new_candidates, data, target_column
                )
                ...
                self.logger.info(
                    f"Discovery completed: {len(evaluated_candidates)} new candidates, "
    ...
    def _exploratory_discovery(
        self, data: pd.DataFrame, target_column: str, baseline_features: pd.DataFrame
    ) -> List[FeatureCandidate]:
        ...
        try:
            ...
            for _ in range(self.config.max_candidates_per_iteration // 2):
                ...
                selected_features = np.random.choice(
                    feature_cols, size=min(3, len(feature_cols)), replace=False
                )
    ...
    def _regime_adaptive_discovery(
        self, data: pd.DataFrame, target_column: str, baseline_features: pd.DataFrame
    ) -> List[FeatureCandidate]:
        ...
        try:
            ...
            for feature_name in list(regime_features)[:10]:  # Limit to top 10
                if feature_name in baseline_features.columns:
                    ...
                    regime_adaptations = [
                        source_data.rolling(self.current_regime.duration)
                        .mean()
                        .fillna(source_data),
            ...
            if remaining_slots > 0:
                exploratory_candidates = self._exploratory_discovery(
                    data, target_column, baseline_features
                )
    ...
    def _performance_driven_discovery(
        self, data: pd.DataFrame, target_column: str, baseline_features: pd.DataFrame
    ) -> List[FeatureCandidate]:
    ...
    def _evaluate_candidates(
        self, candidates: List[FeatureCandidate], data: pd.DataFrame, target_column: str
    ) -> List[FeatureCandidate]:
        ...
        try:
            for candidate in candidates:
                ...
                if self.current_regime:
                    candidate.regime_performance[self.current_regime.regime_id] = (
    ...
    def _promote_candidates_to_active(self):
        ...
        try:
            ...
            sorted_candidates = sorted(
                self.feature_candidates.values(), key=lambda c: c.performance_score, reverse=True
            )
            ...
            for candidate in sorted_candidates:
                ...
                # Promotion criteria
                if (
                    candidate.performance_score > self.config.min_performance_improvement
                    and candidate.stability_score > self.config.min_stability_score
                    and candidate.status == FeatureStatus.CANDIDATE
                ):
                    # Promote to active
                    ...
                    if self.current_regime:
                        self.regime_feature_mapping[self.current_regime.regime_id].add(
                            candidate.name
                        )

                    promotion_count += 1
                    self.logger.info(
                        f"Promoted feature to active: {candidate.name} (score: {candidate.performance_score:.4f})"
                    )
    ...
    def _genetic_evolution_step(
        ...
        try:
            ...
            performance_scores = {
                feature.name: feature.performance_score for feature in parent_features
            }

            # Evolve features
            evolved_features = self.genetic_evolver.evolve_features(
                parent_features, performance_scores
            )
    ...
    def _create_performance_combinations(
        self, baseline_features: pd.DataFrame, target_column: str, max_features: int
    ) -> List[FeatureCandidate]:
            ...
            for i, feature1 in enumerate(top_features):
                for j, feature2 in enumerate(top_features):
                    if i < j and len(candidates) < max_features:  # Avoid duplicates
                        if (
                            feature1 in baseline_features.columns
                            and feature2 in baseline_features.columns
                        ):
                            # Create combination
    ...
    def _cleanup_old_candidates(self):
        ...
        try:
            ...
            candidates_by_age = sorted(
                self.feature_candidates.items(), key=lambda x: x[1].created_at
            )
        ...
        with self._lock:
            return {
                ...
                "regime_feature_mapping": {
                    regime: len(features)
                    for regime, features in self.regime_feature_mapping.items()
                },
...
def get_feature_discovery_engine(
...
def discover_features_for_regime(
