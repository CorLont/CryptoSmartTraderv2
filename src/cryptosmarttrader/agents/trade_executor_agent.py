...
class TradeExecutorAgent:
    ...
    def _execution_loop(self):
        ...
        while self.active:
            try:
                ...
                interval = self.config_manager.get("agents", {}).get("trade_executor", {}).get("update_interval", 120)
    ...
    def _analyze_symbol_for_trading(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            signal = self._combine_signals(
                ...
                ml_prediction, backtest_results
            )
    ...
    def _combine_signals(self, symbol: str, technical: Dict, sentiment: Dict,
                        ml_pred: Dict, backtest: Dict) -> Dict[str, Any]:
    ...
    def _process_trading_signal(self, symbol: str, signal: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.trading_signals[symbol] = [
                s for s in self.trading_signals[symbol]
    ...
    def _evaluate_portfolio_risk(self):
        ...
        try:
            with self._lock:
                ...
                total_exposure = sum(pos.get('value', 0) for pos in self.portfolio_positions.values())
    ...
    def _check_position_sizes(self):
        ...
        for symbol, position in list(self.portfolio_positions.items()):
            try:
                ...
                portfolio_value = sum(pos.get('value', 0) for pos in self.portfolio_positions.values())
                ...
                if portfolio_value > 0:
                    ...
                    if position_weight > self.max_position_size:
                        ...
                        self._generate_risk_management_signal(symbol, 'reduce_position',
                ...
                if current_data is not None and not current_data.empty and entry_price > 0:
                    ...
                    if current_price > 0:
                        ...
                        if price_change <= -self.stop_loss_pct:
                            # Stop loss triggered
                            self._generate_risk_management_signal(symbol, 'stop_loss',
                                                                f'Stop loss triggered: {price_change:.2%}')

                        elif price_change >= self.take_profit_pct:
                            # Take profit triggered
                            self._generate_risk_management_signal(symbol, 'take_profit',
                                                                f'Take profit triggered: {price_change:.2%}')
    ...
    def _check_portfolio_risk_limits(self):
        ...
        try:
            ...
            if risk_score > self.max_portfolio_risk:
                ...
                self._generate_portfolio_risk_signal('reduce_overall_risk',
    ...
    def get_signal_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            high_confidence = sum(1 for signal in latest_signals.values() if signal['confidence'] > 0.7)
