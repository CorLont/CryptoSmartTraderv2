    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not self.detected_specs["gpu_available"]:
            recommendations.append(
                "GPU not detected - install CUDA-compatible GPU for acceleration"
            )
...
class WorkstationHealthMonitor:
    ...
    def get_health_score(self) -> float:
        ...
        health_score = cpu_health * 0.3 + memory_health * 0.4 + disk_health * 0.3
...
def optimize_workstation() -> Dict[str, Any]:
    ...
    print(
        f"   Worker Processes: {summary['optimization_applied']['cpu_optimization']['worker_processes']}"
    )
    print(
        f"   Max Cache: {summary['optimization_applied']['memory_optimization']['max_cache_size_gb']} GB"
    )
    print(
        f"   GPU Batch Size: {summary['optimization_applied']['gpu_optimization']['max_batch_size']}"
    )
