...
class SHAPExplainer:
    ...
    def create_explainer(
        self, model, X_background: np.ndarray, feature_names: List[str], model_name: str = "default"
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"SHAP explainer created for {model_name} with {len(feature_names)} features"
            )
            return True
    ...
    def _create_fallback_explainer(
        self, model, X_background: np.ndarray, feature_names: List[str], model_name: str
    ) -> bool:
    ...
    def explain_prediction(
        self, X: np.ndarray, model_name: str = "default", top_k: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, sample in enumerate(X):
                if SHAP_AVAILABLE and model_name in self.explainers:
                    ...
                    explanation = self._explain_with_fallback(
                        sample, model_name, feature_names, top_k
                    )
    ...
    def _explain_with_shap(
        self, sample: np.ndarray, model_name: str, feature_names: List[str], top_k: int
    ) -> Dict[str, Any]:
    ...
    def _explain_with_fallback(
        ...
        try:
            ...
            return {"method": "error", "top_drivers": [], "error": str(e)}
...
class ExplainabilityEngine:
    ...
    def add_model_explainer(
        self, model, X_background: np.ndarray, feature_names: List[str], model_name: str
    ) -> bool:
        """Add explainer for a model"""

        return self.shap_explainer.create_explainer(model, X_background, feature_names, model_name)

    def explain_predictions(
        self, predictions_df: pd.DataFrame, features_df: pd.DataFrame, model_name: str = "ensemble"
    ) -> pd.DataFrame:
        ...
        try:
            ...
            feature_columns = [
                col for col in features_df.columns if col not in ["coin", "timestamp", "symbol"]
            ]
            ...
            explanations = self.shap_explainer.explain_prediction(X, model_name, top_k=5)
            ...
            self.logger.info(
                f"Added explanations to {len(predictions_with_explanations)} predictions"
            )
    ...
    def generate_explanation_summary(self, predictions_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "explanations_generated": len(
                    [d for d in predictions_df["explanation_details"] if d]
                ),
...
def add_explanations_to_predictions(
    predictions_df: pd.DataFrame, features_df: pd.DataFrame, model_name: str = "ensemble"
) -> pd.DataFrame:
