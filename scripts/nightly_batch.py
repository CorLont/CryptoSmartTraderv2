...
class NightlyBatchOrchestrator:
    ...
    async def run_complete_nightly_pipeline(
        ...
        self.logger.info(
            f"🌙 Starting complete nightly pipeline for {target_date.strftime('%Y-%m-%d')}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"✅ Complete nightly pipeline finished in {time.time() - pipeline_start:.2f}s"
            )
    ...
    def _update_system_status(self, metrics_results: Dict[str, Any]) -> bool:
        ...
        try:
            status = {
                ...
                "trading_status": metrics_results.get("go_nogo_decision", {}).get(
                    "status", "NO-GO"
                ),
        ...
        try:
            ...
            if status == "GO":
                ...
                recommendations.append(
                    f"⚠️ System degraded (score: {score:.1f}) - limit to paper trading"
                )
                recommendations.append("• Review component scores to identify improvement areas")
            else:
                recommendations.append(
                    f"❌ System unhealthy (score: {score:.1f}) - trading blocked"
                )
    ...
    async def _save_pipeline_results(
        self, pipeline_results: Dict[str, Any], target_date: datetime
    ) -> None:
...
async def main():
    ...
    if "phases" in pipeline_results:
        ...
        print(
            f"📈 METRICS: {'SUCCESS' if 'error' not in phases.get('metrics_collection', {}) else 'FAILED'}"
        )
