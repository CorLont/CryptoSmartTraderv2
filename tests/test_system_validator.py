...
def test_system_validator():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "system_validator", "core/system_validator.py"
        )
        ...
        logging_results = [
            r for r in report.validation_results if "logging" in r.check_name.lower()
        ]
        ...
        if report.timestamp and len(report.validation_results) > 0:
            print(
                f"   ✅ PASSED: Validation completed with {len(report.validation_results)} checks"
            )
        ...
        if len(found_categories) >= len(expected_categories) - 1:  # Allow for some variation
            ...
            print(
                f"   ⚠️ WARNING: Some validation categories missing: expected {expected_categories}, found {found_categories}"
            )
