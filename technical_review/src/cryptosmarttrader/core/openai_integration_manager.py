...
class OpenAIIntegrationManager:
    ...
    async def _process_single_news_item(self, news_item: Dict[str, Any], content: str) -> Optional[Dict[str, Any]]:
    ...
    async def analyze_market_sentiment(self, market_texts: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process all texts
            sentiment_tasks = [
                self.adapter.analyze_sentiment(text)
                for text in market_texts if text.strip()
            ]
            ...
            return {
                ...
                'market_relevance': sum(r.market_relevance for r in valid_results) / len(valid_results),
...
"""
            ...
            if self.adapter.client:
                response = self.adapter.client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "You are an expert market anomaly detector. Analyze data and provide structured JSON output."},
                        {"role": "user", "content": prompt}
                    ],
    ...
    def _get_health_recommendations(self, adapter_status: Dict[str, Any]) -> List[str]:
        ...
        fallback_rate = self.usage_stats['fallback_requests'] / max(self.usage_stats['total_requests'], 1)
