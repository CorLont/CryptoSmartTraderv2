...
class AlphaOpportunitiesDashboard:
    ...
    def render(self):
        ...
        st.markdown(
            "**STRICT CRITERIA: Alleen coins met 80%+ confidence EN 100%+ verwacht rendement (30 dagen)**"
        )
        ...
        if not self.pipeline:
            st.error(
                "Real-time pipeline niet beschikbaar. Kan geen betrouwbare opportunities tonen."
            )
            return
        ...
        with col1:
            st.markdown(
                "**Real-time analysis van ALLE Kraken cryptocurrencies - Zero tolerance voor dummy data**"
            )
        with col2:
    ...
    def _render_pipeline_status(self):
        ...
        try:
            ...
            with col1:
                status_indicator = (
                    "🟢 Running" if pipeline_status.get("pipeline_active", False) else "🔴 Stopped"
                )
    ...
    def _render_strict_alpha_opportunities(self):
        ...
        st.markdown(
            "**ALLEEN coins met 80%+ confidence EN 100%+ verwacht rendement (30d). Anders NIETS.**"
        )
        ...
        try:
            ...
            if not alpha_results:
                st.info(
                    "⏳ Pipeline analyseert nog... Wacht tot de eerste batch-analyse voltooid is."
                )
            ...
            if not strict_opportunities:
                st.warning(
                    "🚫 GEEN opportunities voldoen aan STRICT criteria (80%+ confidence + 100%+ return)"
                )
            ...
            for opp in strict_opportunities:
                ...
                table_data.append(
                    {
                        "Symbol": opp["symbol"],
                        "1H Return": f"{horizon_data.get('1H', {}).get('predicted_return', 0) * 100:.1f}%"
                        if "1H" in horizon_data
                        else "N/A",
                        "24H Return": f"{horizon_data.get('24H', {}).get('predicted_return', 0) * 100:.1f}%"
                        if "24H" in horizon_data
                        else "N/A",
            ...
            st.dataframe(
                ...
                column_config={
                    "30D Return": st.column_config.TextColumn(
                        "30D Return ⬇️", help="Expected return in 30 days (sorted high to low)"
                    ),
                    "7D Return": st.column_config.TextColumn(
                        "7D Return", help="Expected return in 7 days"
                    ),
                    "24H Return": st.column_config.TextColumn(
                        "24H Return", help="Expected return in 24 hours"
                    ),
                    "1H Return": st.column_config.TextColumn(
                        "1H Return", help="Expected return in 1 hour"
                    ),
                    "Confidence": st.column_config.TextColumn(
                        "Confidence", help="ML prediction confidence"
                    ),
            ...
            for i, opp in enumerate(strict_opportunities[:3], 1):
                with st.expander(
                    f"#{i} {opp['symbol']} - {opp.get('expected_return_30d', 0) * 100:.1f}% (30d) - {opp.get('confidence', 0) * 100:.1f}% confidence"
                ):
                    col1, col2 = st.columns(2)
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**Prediction Time:** {opp.get('prediction_timestamp', 'Unknown')}"
                        )
                        ...
                        if horizon_data:
                            ...
                            for horizon in ["1H", "24H", "7D", "30D"]:
                                if horizon in horizon_data:
                                    ...
                                    returns.append(
                                        horizon_data[horizon].get("predicted_return", 0) * 100
                                    )
    ...
    def _render_analysis_insights(self):
        ...
        try:
                ...
                if whale_results and "results" in whale_results:
                    ...
                    if high_activity_coins:
                        ...
                        for coin in high_activity_coins[:5]:
                            ...
                            st.write(
                                f"{flow_indicator} **{coin['symbol']}**: {coin['transactions']} large transactions"
                            )
                    else:
            ...
            if sentiment_results and "results" in sentiment_results:
                ...
                sentiment_scores = [
                    data.get("sentiment_score", 0.5) for data in sentiment_data.values()
                ]
    ...
    def _render_data_quality_verification(self):
        ...
        try:
            ...
            if quality_metrics:
                ...
                with col2:
                    ...
                    if failed_scraping > 0 or missing_data > 0:
                        st.warning(
                            f"Total {failed_scraping + missing_data} coins rejected due to incomplete data"
                        )
            ...
            st.info(
                "🔒 **ZERO TOLERANCE**: Coins met ontbrekende of invalid data worden volledig uitgesloten"
            )
            ...
            with st.spinner("Forcing complete pipeline update..."):
                ...
                if cache_manager and hasattr(cache_manager, "_cache"):
                    ...
                    for key in cache_manager._cache.keys():
                        if (
                            key.startswith("validated_")
                            or key.startswith("alpha_opportunities")
                            or "pipeline" in key
                        ):
        ...
        try:
            ...
            if ml_status:
                ...
                with col4:
                    ...
                    if training_times:
                        latest_training = (
                            max(training_times.values()) if training_times.values() else "Never"
                        )
                ...
                if training_config:
                    ...
                    with col1:
                        ...
                        st.write(
                            f"• Min samples: {training_config.get('min_training_samples', 0):,}"
                        )
                    ...
                    with col2:
                        ...
                        st.write(
                            f"• Confidence: {training_config.get('confidence_threshold', 0):.1%}"
                        )
                        st.write(
                            f"• Retrain MAE: {training_config.get('retrain_threshold_mae', 0):.1%}"
                        )
                        st.write(f"• Time horizons: 1H, 24H, 7D, 30D")
            else:
                st.info(
                    "Multi-horizon ML system status not available yet. System will initialize during first batch inference."
                )
