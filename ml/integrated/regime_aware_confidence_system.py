...
from ml.calibration.confidence_gate_calibrated import (
...
class RegimeGateConfig:
    ...
    uncertainty_scaling: Dict[MarketRegime, float] = field(
        default_factory=lambda: {
...
class RegimeAwareCalibrator:
    ...
    def fit_regime_calibrators(
        self, predictions_history: pd.DataFrame, regime_history: List[MarketRegime]
    ):
        ...
        for regime, data in regime_data.items():
            try:
                calibrator = ProbabilityCalibrator(
                    [CalibrationMethod.PLATT_SCALING, CalibrationMethod.ISOTONIC_REGRESSION]
                )
                ...
                self.logger.info(
                    f"Fitted calibrator for {regime.value} with {len(data['probabilities'])} samples"
                )
    ...
    def calibrate_for_regime(self, raw_confidence: float, regime: MarketRegime) -> float:
...
class RegimeAwareConfidenceSystem:
    ...
    def initialize_system(
        ...
        if feature_data is not None and target_data is not None:
            ...
            if np.sum(known_mask) > 100:
                ...
                self.regime_router.train_regime_models(
                    X_filtered, y_filtered, regime_labels_filtered
                )
        ...
        if historical_predictions is not None:
            ...
            if "regime" in historical_predictions.columns:
                ...
                self.regime_calibrator.fit_regime_calibrators(
                    historical_predictions, regime_history
                )
        ...
        if feature_data is not None and target_data is not None:
            ...
            try:
                ...
                self.uncertainty_quantifier = create_uncertainty_quantifier(
                    model_type="ensemble_mc_dropout", ensemble_size=3, mc_samples=50
                )

                self.uncertainty_quantifier.build_model(
                    input_size=feature_data.shape[1], hidden_sizes=[64, 32, 16]
                )

                # Use subset for quick training
                train_size = min(1000, len(feature_data))
                self.uncertainty_quantifier.train(
                    feature_data[:train_size], target_data[:train_size], epochs=30
                )
    ...
    def predict_with_regime_awareness(
        ...
        self.logger.info(
            f"Current regime: {self.current_regime.value} (confidence: {self.regime_confidence:.3f})"
        )
        ...
        for i, pred_dict in enumerate(predictions):
            try:
                ...
                regime_adjustment = self.regime_gate_config.regime_adjustments.get(
                ...
                if (
                    ...
                    try:
                        ...
                        uncertainty_scale = self.regime_gate_config.uncertainty_scaling.get(
                ...
                passes_regime_gate = (
                    regime_adjusted_confidence >= adjusted_threshold
                    and self.regime_confidence >= 0.6  # Minimum regime confidence
                    and total_uncertainty < 0.5  # Maximum uncertainty threshold
                )
                ...
                self.logger.error(
                    f"Failed to process prediction for {pred_dict.get('symbol', 'unknown')}: {e}"
                )
                continue
    ...
    def filter_regime_confident_predictions(
        ...
        for pred in regime_predictions:
            if (
                pred.passes_regime_gate
                and pred.regime_confidence >= min_regime_confidence
                and pred.total_uncertainty <= max_uncertainty
            ):
                filtered_predictions.append(pred)

        self.logger.info(
            f"Filtered {len(filtered_predictions)}/{len(regime_predictions)} predictions "
...
def create_integrated_confidence_system(
    base_confidence_threshold: float = 0.80, regime_adjustments: Optional[Dict[str, float]] = None
    ...
    regime_config = RegimeGateConfig(
        base_confidence_threshold=base_confidence_threshold, regime_adjustments=regime_adj_enum
    )
