...
from cryptosmarttrader.execution.execution_policy import (
    ...
    TimeInForce, OrderStatus, create_market_conditions
)
...
async def demonstrate_execution_system():
    ...
    print(f"   Spread: {good_conditions.spread_bps:.1f} bps, Volume: ${good_conditions.volume_1m:,.0f}")
    ...
    duplicate_params = ExecutionParams(
        ...
        side='buy',
        quantity=0.5,
        client_order_id='DEMO_DUPLICATE_ORDER'
    )
    ...
    print(f"   Large Order: {twap_params.quantity} BTC (~${twap_params.quantity * good_conditions.ask:,.0f})")
    ...
    print(f"   Iceberg Order: {iceberg_params.quantity} ETH (~${iceberg_params.quantity * eth_conditions.bid:,.0f})")
    ...
    print(f"   Total Executions: {final_stats['successful_executions']}/{final_stats['total_orders']}")
    print(f"   Success Rate: {final_stats['success_rate_percent']:.1f}%")
    print(f"   Average Performance: {final_stats['average_slippage_bps']:.1f} bps slippage, {final_stats['average_execution_time_ms']:.0f}ms")
