...
async def test_regime_aware_models():
    ...
    try:
        ...
        print(
            f"   Regime-specific: {len(regime_specific_preds)} samples in {pred_time_specific:.2f}s"
        )
        ...
        return (
            regime_specific_results.get("success", False)
            and regime_feature_results.get("success", False)
            and len(regime_specific_preds) > 0
            and len(regime_feature_preds) > 0
        )
