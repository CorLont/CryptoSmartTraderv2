        ...
        try:
            ...
            if self.metrics:
                self.metrics.record_operation(
                    operation, agent_name or "unknown", "success", duration
                )
...
class DailyLogManager:
    ...
    def log_health_metric(
        self, metric_name: str, value: Union[int, float], category: str = "system"
    ):
        ...
        self.logger.info(f"Health metric: {metric_name} = {value} ({category})", extra=metric_data)

    def log_trading_event(self, event_type: str, data: Dict[str, Any]):
        """Log trading event to daily logs"""

        event_data = {"timestamp": datetime.now().isoformat(), "event_type": event_type, **data}
    ...
    def log_confidence_gate_result(
        self, total_candidates: int, passed_count: int, pass_rate: float
    ):
        ...
        # Also log to main logger
        self.logger.info(
            f"Confidence gate: {passed_count}/{total_candidates} passed ({pass_rate:.1%})",
            extra=gate_data,
        )
