...
from cryptosmarttrader.execution import (
    LiquidityGate, SpreadMonitor, SlippageTracker, ExecutionFilter
)
...
def generate_realistic_market_data(n_symbols: int = 5) -> Dict[str, Any]:
    ...
    for i, symbol in enumerate(symbols):
        ...
        print(f"   {symbol}: ${mid_price:.2f}, {spread_bp:.1f}bp spread, ${depth:,.0f} depth, ${volume_1m:,.0f}/min volume")
...
def test_spread_monitoring():
    ...
    for i, opp in enumerate(opportunities[:3]):
        print(f"   {i+1}. {opp['symbol']}: {opp['current_spread_bp']:.1f}bp "
...
def test_slippage_tracking():
    ...
    for day in range(7):  # 7 days of history
        for _ in range(20):  # 20 trades per day
            ...
            # Record execution
            execution_time = datetime.now() - timedelta(days=7-day, hours=np.random.randint(0, 24))
