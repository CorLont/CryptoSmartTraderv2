...
class ReturnAttributionAnalyzer:
    ...
    def analyze_attribution(
        ...
        self.logger.info(
            "Starting return attribution analysis",
            period=period.value,
        ...
        self.logger.info(
            "Return attribution analysis complete",
            total_return_bps=total_return_bps,
    ...
    def _filter_returns_to_period(
        self, returns: pd.Series, start: datetime, end: datetime
    ) -> pd.Series:
    ...
    def _filter_executions_to_period(
        self, executions: List[ExecutionResult], start: datetime, end: datetime
    ) -> List[ExecutionResult]:
        """Filter executions to specified period."""
        return [exec for exec in executions if start <= exec.execution_time <= end]

    def _analyze_alpha_component(
        self,
        portfolio_returns: pd.Series,
        benchmark_returns: pd.Series,
        period_start: datetime,
        ...
        return AttributionComponent(
            ...
            contribution_pct=alpha_bps / (portfolio_returns.sum() * 10000)
            if portfolio_returns.sum() != 0
            else 0.0,
            ...
            metadata={
                ...
                "max_drawdown": excess_returns.cumsum()
                .expanding()
                .max()
                .sub(excess_returns.cumsum())
                .max(),
            },
        )

    def _analyze_fees_component(
        self,
    ...
    def _analyze_slippage_component(
        self,
        executions: List[ExecutionResult],
        total_return_bps: float,
        period_start: datetime,
        ...
        avg_slippage_bps = (
            float(-(total_slippage_cost / total_volume * 10000)) if total_volume > 0 else 0.0
        )
    ...
    def _analyze_timing_component(
        ...
        return AttributionComponent(
            ...
            contribution_pct=timing_cost_bps / portfolio_returns.sum() / 10000
            if portfolio_returns.sum() != 0
            else 0.0,
    ...
    def _analyze_sizing_component(
        ...
        sizing_impact_bps = (
            -(size_variance / (avg_order_size**2) * 100) if avg_order_size > 0 else 0.0
        )

        return AttributionComponent(
            name="Sizing",
            contribution_bps=sizing_impact_bps,
            contribution_pct=sizing_impact_bps / portfolio_returns.sum() / 10000
            if portfolio_returns.sum() != 0
            else 0.0,
            ...
            metadata={
                ...
                "size_coefficient_of_variation": np.std(order_sizes) / avg_order_size
                if avg_order_size > 0
                else 0.0,
    ...
    def _analyze_market_impact_component(
        self,
        executions: List[ExecutionResult],
        total_return_bps: float,
        period_start: datetime,
        ...
        large_orders = [
            exec for exec in executions if exec.executed_quantity * exec.avg_fill_price > 10000
        ]  # >$10k orders
        ...
        return AttributionComponent(
            ...
            metadata={
                ...
                "avg_large_order_size": np.mean(
                    [e.executed_quantity * e.avg_fill_price for e in large_orders]
                )
                if large_orders
                else 0.0,
    ...
    def _calculate_explained_variance(
        self, components: List[AttributionComponent], total_excess_return_bps: float
    ) -> float:
    ...
    def _calculate_execution_quality_score(self, executions: List[ExecutionResult]) -> float:
        ...
        fill_rate = np.mean(
            [
                exec.executed_quantity / exec.requested_quantity
                for exec in executions
    ...
    def _calculate_data_quality_score(self, executions: List[ExecutionResult]) -> float:
        ...
        complete_executions = len(
            [exec for exec in executions if exec.executed_quantity > 0 and exec.avg_fill_price > 0]
        )
    ...
    def _generate_optimization_opportunities(
        self, components: List[AttributionComponent]
    ) -> List[str]:
            ...
            if comp.contribution_bps < -10:  # Significant negative contribution
                if comp.name == "Fees":
                    opportunities.append(
                        f"High fee impact ({comp.contribution_bps:.1f} bps): Optimize maker/taker ratio"
                    )
                elif comp.name == "Slippage":
                    opportunities.append(
                        f"High slippage cost ({comp.contribution_bps:.1f} bps): Improve order routing"
                    )
                elif comp.name == "Timing":
                    opportunities.append(
                        f"Timing cost detected ({comp.contribution_bps:.1f} bps): Reduce execution latency"
                    )
                elif comp.name == "Market Impact":
                    opportunities.append(
                        f"Market impact cost ({comp.contribution_bps:.1f} bps): Break up large orders"
                    )
        ...
        if alpha_comp and alpha_comp.contribution_bps < 5:
            opportunities.append(
                "Low alpha generation: Review signal quality and model performance"
            )
    ...
    def _generate_execution_improvements(self, executions: List[ExecutionResult]) -> List[str]:
        ...
        if avg_latency > 500:
            improvements.append(
                f"High average latency ({avg_latency:.0f}ms): Optimize network connectivity"
            )
        ...
        # Fill rate analysis
        partial_fills = [
            exec for exec in executions if exec.executed_quantity < exec.requested_quantity
        ]
    ...
    def _generate_cost_reduction_suggestions(
        self, cost_components: List[AttributionComponent]
    ) -> List[str]:
        ...
        for comp in cost_components:
            if comp.contribution_bps < -5:  # Significant cost
                if comp.name == "Fees":
                    ...
                    if maker_ratio < 0.7:
                        suggestions.append(
                            f"Increase maker ratio (currently {maker_ratio:.1%}): Use limit orders"
                        )
                    ...
                    if avg_slippage > 15:
                        suggestions.append(
                            "High slippage: Use smaller order sizes and better timing"
                        )
                    ...
                    if large_orders > 0:
                        suggestions.append(
                            f"Break up {large_orders} large orders to reduce market impact"
                        )
    ...
    def get_attribution_summary(self, period_days: int = 7) -> Dict[str, Any]:
        ...
        recent_reports = [
            report
            for report in self.attribution_history
        ...
        return {
            ...
            "top_opportunity": self._get_top_cost_component(
                [float(avg_fees), float(avg_slippage), float(avg_timing)]
            ),
            "cost_breakdown_pct": {
                "fees": abs(avg_fees) / (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing)) * 100
                if (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing)) > 0
                else 0,
                "slippage": abs(avg_slippage)
                / (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing))
                * 100
                if (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing)) > 0
                else 0,
                "timing": abs(avg_timing)
                / (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing))
                * 100
                if (abs(avg_fees) + abs(avg_slippage) + abs(avg_timing)) > 0
                else 0,
    ...
    def save_attribution_report(
        self, report: ReturnAttributionReport, filepath: Optional[str] = None
    ) -> str:
        ...
        if filepath is None:
            ...
            filepath = str(
                reports_dir
                / f"attribution_report_{report.period_start.strftime('%Y%m%d')}_{report.period.value}.json"
            )
        ...
        for component_name in [
            ...
            if component_name in report_dict:
                ...
                comp["period_start"] = (
                    comp["period_start"].isoformat()
                    if isinstance(comp["period_start"], datetime)
                    else comp["period_start"]
                )
                comp["period_end"] = (
                    comp["period_end"].isoformat()
                    if isinstance(comp["period_end"], datetime)
                    else comp["period_end"]
                )
