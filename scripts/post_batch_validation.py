    ...
    async def _run_coverage_audit(self) -> None:
        ...
        try:
            ...
            cmd = [
                ...
                "--input",
                "data/batch_output/last_run_processed_symbols.json",
                "--quiet",
            ]
            ...
            if result.returncode == 0:
                ...
                self.validation_results["warnings"].append(
                    f"{validation_name}: coverage below target but acceptable"
                )
                ...
                self.validation_results["critical_issues"].append(
                    f"{validation_name}: coverage critically low"
                )
            ...
            self.validation_results["validations_failed"].append(validation_name)
            self.validation_results["critical_issues"].append(
                f"{validation_name}: timeout after 5 minutes"
            )
        except Exception as e:
            self.validation_results["validations_failed"].append(validation_name)
    ...
    async def _run_performance_evaluation(self) -> None:
        ...
        try:
            ...
            if not predictions_file.exists() or not prices_file.exists():
                self.validation_results["warnings"].append(
                    f"{validation_name}: insufficient historical data for evaluation"
                )
            ...
            if result.returncode == 0:
                ...
                self.validation_results["validations_passed"].append(validation_name)
                self.validation_results["warnings"].append(
                    f"{validation_name}: some metrics below target"
                )
                self.logger.warning(f"{validation_name} warning: performance issues")
            else:
                self.validation_results["validations_failed"].append(validation_name)
                self.validation_results["critical_issues"].append(
                    f"{validation_name}: performance targets not met"
                )
                self.logger.error(f"{validation_name} failed: {result.stderr}")
            ...
            self.validation_results["validations_failed"].append(validation_name)
            self.validation_results["critical_issues"].append(
                f"{validation_name}: timeout after 10 minutes"
            )
        except Exception as e:
            self.validation_results["validations_failed"].append(validation_name)
            self.validation_results["critical_issues"].append(f"{validation_name}: {str(e)}")
    ...
    async def _run_calibration_check(self) -> None:
        ...
        try:
            ...
            if not calibration_file.exists():
                self.validation_results["warnings"].append(
                    f"{validation_name}: no calibration data available"
                )
            ...
            if result.returncode == 0:
                ...
                self.validation_results["validations_passed"].append(validation_name)
                self.validation_results["warnings"].append(
                    f"{validation_name}: minor calibration issues"
                )
                self.logger.warning(f"{validation_name} warning: calibration drift")
            else:
                self.validation_results["validations_failed"].append(validation_name)
                self.validation_results["critical_issues"].append(
                    f"{validation_name}: significant calibration problems"
                )
                self.logger.error(f"{validation_name} failed: {result.stderr}")
    ...
    async def _run_health_score_calculation(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.validation_results["critical_issues"].append(
                    f"{validation_name}: system health too low"
                )
                self.logger.error(f"{validation_name} failed: {result.stderr}")
            ...
            self.validation_results["validations_failed"].append(validation_name)
            self.validation_results["critical_issues"].append(
                f"{validation_name}: timeout after 1 minute"
            )
        except Exception as e:
            self.validation_results["validations_failed"].append(validation_name)
            self.validation_results["critical_issues"].append(f"{validation_name}: {str(e)}")
...
def print_validation_summary(results: Dict[str, Any]) -> None:
    ...
    if "system_status" in results:
        status_icon = {"GO": "🟢", "WARNING": "🟡", "NO-GO": "🔴"}.get(
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Post-Batch Validation Pipeline")

    parser.add_argument(
        "--skip-coverage", action="store_true", help="Skip coverage audit validation"
    )

    parser.add_argument(
        "--skip-performance", action="store_true", help="Skip performance evaluation"
    )
