...
class AdaptiveModelSwitcher:
    ...
    def _initialize_regime_mappings(self):
        ...
        try:
            ...
            for regime, defaults in regime_defaults.items():
                ...
                backup_models = []
                backup_types = [ModelType.RANDOM_FOREST, ModelType.LINEAR, ModelType.GRADIENT_BOOSTING]
    ...
    def adapt_to_regime(self, data: pd.DataFrame, target_column: str = 'close') -> Dict[str, Any]:
        ...
        with self._lock:
            try:
                ...
                if regime_changed:
                    self.logger.info(f"Regime change detected: {self.current_regime.value} -> {detected_regime.value}")
                ...
                if detected_regime in self.regime_models:
                    ...
                    if self.config.enable_threshold_adaptation:
                        adapted_thresholds = self._adapt_thresholds(data, regime_mapping, regime_result)
                    ...
                    self.logger.warning(f"No model mapping found for regime: {detected_regime.value}")
                    return {
                        'success': False,
    ...
    def _adapt_features(self, data: pd.DataFrame, regime_mapping: RegimeModelMapping,
        ...
        try:
            ...
            engineered_data = self.feature_engineer.transform(data, target_column, regime_mapping.regime.value)
            ...
            if len(adapted_features) > max_features:
                ...
                other_features = [f for f in adapted_features if f not in priority_features][:max_features - len(priority_features)]
                adapted_features = priority_features + other_features

            self.logger.info(f"Adapted features for {regime_mapping.regime.value}: {len(adapted_features)} features")
    ...
    def _adapt_model(self, data: pd.DataFrame, regime_mapping: RegimeModelMapping,
        ...
        try:
            ...
            if switch_needed or self.current_model is None:
                ...
                self.logger.info(f"Switched to {best_model_config.model_type.value} for {regime_mapping.regime.value}")
                ...
                if self._should_retrain_model():
                    ...
                    trained_model = self._train_model(model, data, regime_mapping.primary_model, target_column)
                    return trained_model
    ...
    def _adapt_thresholds(self, data: pd.DataFrame, regime_mapping: RegimeModelMapping,
                         regime_result: RegimeDetectionResult) -> Dict[str, float]:
        ...
        try:
            ...
            for threshold_name, base_value in base_thresholds.items():
                ...
                if threshold_name == 'buy':
                    ...
                    adapted_value = max(self.config.buy_threshold_range[0],
                    ...
                    adapted_value = confidence_adjusted - volatility_adjustment
                    adapted_value = max(self.config.sell_threshold_range[0],
            ...
            self.logger.info(f"Adapted thresholds for {regime_mapping.regime.value}: {adapted_thresholds}")
    ...
    def _select_best_model(self, data: pd.DataFrame, regime_mapping: RegimeModelMapping,
                          target_column: str) -> ModelConfig:
    ...
    def _evaluate_model_candidate(self, data: pd.DataFrame, model_config: ModelConfig,
        ...
        try:
            ...
            # Prepare features
            available_features = [col for col in data.columns if col != target_column and col in model_config.features]
    ...
    def _create_model(self, model_config: ModelConfig) -> Any:
        ...
        try:
            ...
            if not HAS_SKLEARN and model_type in [ModelType.LINEAR, ModelType.RANDOM_FOREST, ModelType.GRADIENT_BOOSTING]:
            ...
            elif model_type in [ModelType.NEURAL_NETWORK, ModelType.LSTM, ModelType.GRU, ModelType.TRANSFORMER]:
    ...
    def _train_model(self, model: Any, data: pd.DataFrame, model_config: ModelConfig,
                    target_column: str) -> Any:
        ...
        try:
            ...
            if model_config.model_type in [ModelType.NEURAL_NETWORK, ModelType.LSTM, ModelType.GRU, ModelType.TRANSFORMER]:
                trained_model = self.deep_learning_engine.train_model(model, X, y, model_config.parameters)
    ...
    def evaluate_current_performance(self, data: pd.DataFrame, target_column: str) -> Dict[str, float]:
        ...
        try:
            ...
            if hasattr(self.current_model, 'predict'):
                ...
                if len(self.regime_performance[self.current_regime]) > 100:
                    self.regime_performance[self.current_regime] = self.regime_performance[self.current_regime][-100:]
    ...
    def get_current_signals(self, data: pd.DataFrame, target_column: str) -> Dict[str, Any]:
        ...
        try:
            if self.current_model is None or not self.current_features or not self.current_thresholds:
            ...
            if hasattr(self.current_model, 'predict'):
                ...
                if predicted_return >= buy_threshold:
                    ...
                    signal = 'HOLD'
                    confidence = 1.0 - (abs(predicted_return) / max(buy_threshold, abs(sell_threshold)))
                ...
                return {
                    ...
                    'regime': self.current_regime.value,
                    'model_type': self.regime_models[self.current_regime].primary_model.model_type.value if self.current_regime in self.regime_models else 'unknown'
                }
    ...
    def get_adaptation_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'current_model_type': self.regime_models[self.current_regime].primary_model.model_type.value if self.current_regime in self.regime_models else 'unknown',
                ...
                'recent_performance': dict(list(self.performance_history[-5:])) if self.performance_history else {},
...
def get_adaptive_model_switcher(config: Optional[AdaptiveSwitcherConfig] = None) -> AdaptiveModelSwitcher:
