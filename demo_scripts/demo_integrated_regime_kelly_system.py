...
def simulate_market_scenario():
    ...
    regime_labels = (['trend_up'] * 50 + ['mean_reversion'] * 50 + 
                    ['high_vol_chop'] * 50 + ['low_vol_drift'] * 50)
    
    market_data['true_regime'] = regime_labels
    
    print(f"📈 Phases Created:")
    print(f"   Phase 1 (0-49): Strong Uptrend - Total Return: {((prices[49]/prices[0])-1)*100:.1f}%")
    print(f"   Phase 2 (50-99): Range Bound - Volatility: {np.std(all_returns[50:100])*100:.1f}%")  
    print(f"   Phase 3 (100-149): High Vol Chop - Max DD: {(min(prices[100:150])/max(prices[100:150])-1)*100:.1f}%")
    print(f"   Phase 4 (150-199): Low Vol Recovery - Vol: {np.std(all_returns[150:200])*100:.1f}%")
...
def demonstrate_integrated_system():
    ...
    for regime in results_df['regime'].unique():
        ...
        print(f"     Avg Size: {avg_size:.1f}%" if not np.isnan(avg_size) else "     Avg Size: 0.0%")
    ...
    for i, result in enumerate(results_df.head(8).to_dict('records')):
        ...
        print(f"   {i+1}. {result['regime']}: {trade_emoji} {result['kelly_size']:.1f}% "
