...
from cryptosmarttrader.regime import (
    RegimeDetector,
    RegimeFeatures,
    RegimeClassifier,
    RegimeStrategies,
...
async def test_feature_calculation():
    ...
    try:
        ...
        features = feature_calc.calculate_all_features(
            market_data=market_data, dominance_data=dominance_data
        )
...
async def test_regime_classification():
    ...
    try:
        ...
        for i in range(50, len(btc_data), 10):  # Every 10 hours
            ...
            features = feature_calc.calculate_all_features(
                market_data=window_data, dominance_data=dominance_data
            )
...
async def test_full_regime_system():
    ...
    try:
        ...
        if regime_classification:
            ...
            logger.info(
                f"   Current Strategy: {analytics.get('strategy_summary', {}).get('strategy_type', 'unknown')}"
            )
