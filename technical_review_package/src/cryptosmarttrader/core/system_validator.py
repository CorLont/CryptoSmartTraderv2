...
class SystemValidator:
    ...
    def _load_config(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "optional_dependencies": ["cupy", "numba", "hvac", "prometheus_client", "respx"],
            ...
            # Directory structure
            "required_directories": ["core", "data", "logs", "models"],
            "recommended_directories": ["cache", "config", "tests", "backups"],
    ...
    def validate_system(self) -> SystemValidationReport:
        ...
        self.logger.info(
            f"System validation completed: {status_text} "
    ...
    def _validate_dependencies(self):
        ...
        if not critical_missing:
            ...
            self._add_result(
                ...
                recommendations=[
                    f"Install missing dependencies: pip install {' '.join(critical_missing)}"
                ],
            )
    ...
    def _validate_module_imports(self):
        ...
        for category, default_severity in module_categories:
            ...
            if not category_failures:
                ...
                severity = (
                    ValidationSeverity.WARNING
                    if len(category_failures) < len(modules) / 2
    ...
    def _validate_system_resources(self):
        ...
        try:
            ...
            try:
                ...
                self._add_result(
                    "disk_space", ValidationSeverity.WARNING, False, f"Cannot check disk space: {e}"
                )
    ...
    def _validate_configuration(self):
        ...
        try:
            ...
            # Check .env file
            if Path(".env").exists():
                self._add_result("env_file", ValidationSeverity.SUCCESS, True, ".env file exists")
            else:
                self._add_result(
                    "env_file",
                    ValidationSeverity.INFO,
    ...
    def _validate_logging_system(self):
        ...
        try:
            ...
            if Path("logs").exists():
                self._add_result(
                    "log_directory", ValidationSeverity.SUCCESS, True, "Log directory exists"
                )
            else:
                self._add_result(
                    "log_directory",
    ...
    def _generate_validation_report(self, start_time: datetime) -> SystemValidationReport:
        ...
        critical_issues = sum(
            ...
            if r.severity == ValidationSeverity.CRITICAL and not r.success
        )
        warning_issues = sum(
            1
            for r in self.validation_results
            if r.severity == ValidationSeverity.WARNING and not r.success
        )
