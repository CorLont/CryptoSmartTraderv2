...
from src.cryptosmarttrader.parity.execution_simulator import ExecutionResult, OrderSide, OrderType
...
def test_return_attribution_system():
    ...
    print(
        f"   Excess return: {(portfolio_returns.sum() - benchmark_returns.sum()) * 10000:.1f} bps"
    )
    ...
    for name, component in components:
        ...
        print(
            f"   {name}: {component.contribution_bps:+.1f} bps ({pct:+.1f}%) [conf: {confidence:.1%}]"
        )
