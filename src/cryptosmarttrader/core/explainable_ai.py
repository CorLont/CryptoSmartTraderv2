...
class PredictionExplainer:
    ...
    def explain_prediction(self, coin: str, prediction: float, horizon: str, 
                         features: Dict[str, float], model_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation['key_factors'] = self._identify_key_factors(features, explanation['feature_importance'])
    ...
    def _get_shap_explanation(self, features: Dict[str, float], model_name: str, horizon: str) -> Optional[Dict]:
    ...
    def _identify_key_factors(self, features: Dict[str, float], feature_importance: List[Dict]) -> List[str]:
    ...
    def _calculate_prediction_range(self, prediction: float, features: Dict[str, float]) -> Dict[str, float]:
        ...
        try:
            ...
            data_quality = sum(1 for v in features.values() if isinstance(v, (int, float)) and v != 0) / len(features)
            uncertainty = base_uncertainty * (2 - data_quality)  # Lower quality = higher uncertainty
    ...
    def _generate_human_explanation(self, coin: str, prediction: float, horizon: str, 
        ...
        try:
            ...
            # Add disclaimer
            explanation += f"Prediction confidence varies based on data quality and market conditions. "
    ...
    def _get_fallback_explanation(self, coin: str, prediction: float, horizon: str) -> Dict[str, Any]:
    ...
    def explain_portfolio_allocation(self, allocations: Dict[str, float], 
            ...
            for coin, allocation in sorted_positions[:5]:
                if coin in explanations:
                    ...
                    portfolio_explanation['top_positions'].append({
                        ...
                        'key_reason': exp.get('key_factors', ['Unknown'])[0] if exp.get('key_factors') else 'Unknown'
                    })
            
            # Calculate diversification score
            portfolio_explanation['diversification_score'] = self._calculate_diversification_score(allocations)
    ...
    def _generate_allocation_rationale(self, sorted_positions: List[Tuple], 
                                     explanations: Dict[str, Dict]) -> List[str]:
        ...
        try:
            ...
            if top_3_allocation > 0.7:
                rationale.append("High concentration in top 3 positions - higher risk/reward profile")
            ...
            if top_coin in explanations:
                ...
                if confidence > 0.8:
                    ...
                    rationale.append(f"Moderate confidence in {top_coin} suggests cautious position sizing")
            
            # Count high-confidence positions
            high_conf_positions = sum(1 for coin, _ in sorted_positions 
                                    if coin in explanations and explanations[coin].get('confidence', 0) > 0.7)
            
            if high_conf_positions >= 3:
                rationale.append("Multiple high-confidence opportunities support aggressive allocation")
