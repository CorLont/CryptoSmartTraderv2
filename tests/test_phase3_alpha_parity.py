...
from src.cryptosmarttrader.core.regime_detector import RegimeDetector, MarketRegime, RegimeMetrics
...
class TestStrategySwitcher:
    ...
    def test_cluster_limit_enforcement(self):
        ...
        assert (
            large_cap_total <= switcher.cluster_limits["large_cap"].max_weight + 0.01
        )  # Small tolerance
...
class TestBacktestParityAnalyzer:
    ...
    def test_execution_simulation(self):
        ...
        assert (
            live_execution.slippage_bps >= bt_execution.slippage_bps
        )  # Live should have more slippage
