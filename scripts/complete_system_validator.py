...
def check_models_trained():
    ...
    for horizon in horizons:
        ...
        print(f"  {status} {horizon}: {size_mb:.1f}MB" if exists else f"  {status} {horizon}: MISSING")
...
def check_strict_gate():
    ...
    try:
        ...
        if actual_pass == expected_pass:
            ...
            print(f"  ❌ Gate filtering: {actual_pass}/{len(test_data)} passed (expected {expected_pass})")
...
def check_data_hygiene():
    ...
    try:
        ...
        required_features = [c for c in feature_cols if any(req in c for req in ['rsi', 'volume', 'momentum'])]
        ...
        overall_ok = has_tz and on_hour and target_scales_ok and total_nans == 0 and not missing_required
        return overall_ok
...
def check_predictions_output():
    ...
    try:
        ...
        if conf_cols:
            ...
            print(f"    High conf (≥80%): {high_conf_count}/{len(df)} ({high_conf_count/len(df)*100:.1f}%)")
        ...
        print(f"  📈 Return range: {returns.min():.2f}% to {returns.max():.2f}% (span: {return_range:.2f}%)")
