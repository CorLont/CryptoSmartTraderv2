...
class MetaLabelingClassifier:
    ...
    def create_meta_features(
        ...
        features['volume_ratio'] = market_data['volume_24h'] / market_data['volume_24h'].rolling(7).mean()
        ...
        features['market_stress'] = features['volatility_20d'] > features['volatility_20d'].quantile(0.8)
...
class MetaLabelingSystem:
    ...
    def train_meta_labeling(
        ...
        evaluation = self._evaluate_meta_system(
            primary_signals, meta_features, barrier_events
        )
        
        training_result.update(evaluation)
        
        self.logger.info(f"Meta-labeling system trained: {evaluation['meta_precision']:.3f} precision")
        return training_result
    ...
    def apply_meta_filter(
        ...
        for idx, (signal_idx, signal_row) in enumerate(primary_signals.iterrows()):
            ...
            # Apply meta-filter
            if (meta_label.meta_label == 1 and 
                meta_label.meta_confidence >= self.min_meta_confidence):
    ...
    def _evaluate_meta_system(
        ...
        evaluation = {
            ...
            "avg_return_successful": np.mean([e.return_pct for e in successful_events]) if successful_events else 0,
            "avg_days_held": np.mean([e.days_held for e in barrier_events]) if barrier_events else 0,
            "profit_target_hit_rate": sum(1 for e in barrier_events if e.exit_reason == 'profit_target') / total_events if total_events > 0 else 0
        }
    ...
    def get_system_performance(self) -> Dict[str, Any]:
        ...
        filter_precision = (
            ...
            if self.performance_stats["total_signals"] > 0 else 0
        )
