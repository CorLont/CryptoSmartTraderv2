...
class SystemMetricsCollector(MetricsCollector):
    ...
    def collect(self) -> Dict[str, MetricValue]:
        ...
        if PSUTIL_AVAILABLE:
            ...
            metrics.update(
                {
                    "cpu_usage_percent": MetricValue(cpu_percent, now, {"type": "cpu"}),
                    "memory_usage_percent": MetricValue(memory.percent, now, {"type": "memory"}),
                    "memory_usage_mb": MetricValue(
                        memory.used / 1024 / 1024, now, {"type": "memory"}
                    ),
                    "disk_usage_percent": MetricValue(disk.percent, now, {"type": "disk"}),
        ...
        if self.gpu_available:
            try:
                ...
                for i, gpu in enumerate(gpus):
                    ...
                    metrics.update(
                        {
                            f"gpu_{i}_usage_percent": MetricValue(gpu.load * 100, now, gpu_labels),
                            f"gpu_{i}_memory_usage_percent": MetricValue(
                                gpu.memoryUtil * 100, now, gpu_labels
                            ),
                            f"gpu_{i}_memory_used_mb": MetricValue(gpu.memoryUsed, now, gpu_labels),
                            f"gpu_{i}_temperature_c": MetricValue(gpu.temperature, now, gpu_labels),
                        }
                    )
...
async def metrics_collection_loop(
    metrics_server: PrometheusMetricsServer, interval: float = 10.0
) -> None:
...
if __name__ == "__main__":
    ...
    async def test_prometheus_metrics():
        ...
        # Run collection loop
        collection_task = asyncio.create_task(metrics_collection_loop(metrics_server, interval=2.0))
