...
class EnsembleVotingAgent:
    ...
    def _create_ensemble_prediction(
        ...
        try:
            ...
            prediction_id = f"ensemble_{symbol}_{horizon.value}_{voting_method.value}_{int(time.time())}"
            ...
            ensemble_prediction = EnsemblePrediction(
                ...
                signal_to_noise_ratio=ensemble_result['confidence'] / (epistemic_uncertainty + 0.01),
    ...
    def _confidence_weighted_voting(self, predictions: List[ModelPrediction]) -> Dict[str, Any]:
        ...
        expected_accuracy = sum(p.historical_accuracy * p.confidence for p in predictions) / total_weight
    ...
    def _weighted_average_voting(self, predictions: List[ModelPrediction]) -> Dict[str, Any]:
        ...
        expected_accuracy = sum(p.historical_accuracy * model_weights[p.model_id] for p in predictions)
    ...
    def _cleanup_expired_predictions(self):
        ...
        with self._lock:
            ...
            self.ensemble_predictions = deque([
                pred for pred in self.ensemble_predictions
        ...
        with self._lock:
            if self.ensemble_predictions:
                ...
                successful_votes = sum(
                    1 for p in self.ensemble_predictions
                    if p.ensemble_confidence > 0.7 and p.model_agreement > 0.6
                )
                self.stats['successful_ensemble_votes'] = successful_votes
    
    def get_latest_predictions(self, symbol: str = None, limit: int = 10) -> List[EnsemblePrediction]:
    ...
    def get_high_confidence_predictions(self, min_confidence: float = 0.8) -> List[EnsemblePrediction]:
        """Get high confidence ensemble predictions"""
        
        with self._lock:
            return [
                pred for pred in self.ensemble_predictions
                if pred.ensemble_confidence >= min_confidence and
                pred.model_agreement >= 0.6
            ]
        ...
        with self._lock:
            ...
            if active_predictions > 0:
                ...
                best_predictions = sorted(
                    ...
                    key=lambda x: x.ensemble_confidence,
                    reverse=True
                )[:5]
