...
def generate_production_predictions():
    ...
    print(
        f"   Pass rate: {gate_results['total_passed'] / max(gate_results['total_candidates'], 1) * 100:.1f}%"
    )
    ...
    if not final_predictions:
        ...
        for horizon in horizons:
            ...
            if conf_col in predictions_df.columns:
                ...
                print(
                    f"   {horizon}: {high_conf}/{total} ({high_conf / total * 100:.1f}%) ≥80% confidence"
                )
    ...
    summary = {
        ...
        "gate_statistics": {
            ...
            "pass_rate_pct": gate_results["total_passed"]
            / max(gate_results["total_candidates"], 1)
            * 100,
        },
    }
