...
class UnifiedMetrics:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.equity_usd = Gauge(
            "portfolio_equity_usd", "Current portfolio equity in USD", registry=self.registry
        )
        ...
        self.position_count = Gauge(
            "portfolio_position_count", "Number of open positions", registry=self.registry
        )
        ...
        self.alerts_active_count = Gauge(
            "alerts_active_count", "Number of active alerts", ["severity"], registry=self.registry
        )
    ...
    def record_signal(self, agent_name: str, confidence: float, symbol: str = ""):
        ...
        if PROMETHEUS_AVAILABLE:
            self.signals_received.labels(
                agent_name=agent_name, confidence_bucket=confidence_bucket
            ).inc()
            self.signal_age_minutes.labels(agent_name=agent_name).set(0)

            if symbol:
                self.prediction_confidence.labels(model_name=agent_name, symbol=symbol).observe(
                    confidence
                )
    ...
    def update_exchange_connectivity(self, exchange_name: str, connected: bool):
        ...
        if PROMETHEUS_AVAILABLE:
            self.exchange_connectivity.labels(exchange_name=exchange_name).set(
    ...
    def update_data_quality(self, data_source: str, symbol: str, quality_score: float):
        ...
        if PROMETHEUS_AVAILABLE:
            self.data_quality_score.labels(data_source=data_source, symbol=symbol).set(
                quality_score
            )
        ...
        if quality_score < 0.8:
            self._fire_alert(
                "LowDataQuality", f"Data quality {quality_score:.0%} for {data_source}/{symbol}"
            )
        ...
        for rule in self.alert_rules:
            if rule.metric_name == "slippage_bps_p95":
                ...
                if self._evaluate_threshold(slippage_bps, rule.threshold, rule.operator):
                    self._fire_alert(
                        rule.name, f"Slippage {slippage_bps:.1f} bps exceeds {rule.threshold}"
                    )
        ...
        for rule in self.alert_rules:
            if rule.metric_name == "drawdown_pct":
                if self._evaluate_threshold(drawdown_pct, rule.threshold, rule.operator):
                    self._fire_alert(
                        rule.name, f"Drawdown {drawdown_pct:.1f}% exceeds {rule.threshold}%"
                    )
        ...
        for rule in self.alert_rules:
            if rule.metric_name == "signal_age_minutes":
                if self._evaluate_threshold(age_minutes, rule.threshold, rule.operator):
                    self._fire_alert(
                        rule.name, f"Signal from {agent_name} stale for {age_minutes:.1f} minutes"
                    )
