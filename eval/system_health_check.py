...
class SystemHealthChecker:
    ...
    def check_advanced_engines(self) -> bool:
            ...
            try:
                ...
                has_coordinator = any(
                    hasattr(module, attr) for attr in [
                        'coordinator', 'process', 'analyze', 'predict', 'detect'
                    ]
                )
        ...
        if success_rate >= 0.6:
            self.results.append(f"✅ Advanced engines status: {working_engines}/{len(advanced_engines)} operational")
            return True
        else:
            self.failed_tests += 1
            self.results.append(f"❌ Advanced engines insufficient: {working_engines}/{len(advanced_engines)} operational")
            return False
    ...
    def check_dependencies(self) -> bool:
        ...
        critical_deps = [
            ...
            "scikit-learn", "ccxt", "aiohttp", "pydantic"
        ]
