...
def generate_synthetic_data():
    ...
    for i, symbol in enumerate(symbols):
        ...
        price_data[symbol] = pd.DataFrame({"close": prices, "volume": volumes}, index=dates)
...
async def demonstrate_alpha_portfolio_system():
    ...
    for symbol in list(expected_returns.keys())[:5]:
        print(
            f"      {symbol}: Return {expected_returns[symbol]:.1%}, Vol {volatilities[symbol]:.1%}, Confidence {confidence_scores[symbol]:.2f}"
        )
    ...
    sorted_positions = sorted(
        portfolio_allocation.allocations.items(), key=lambda x: x[1].final_allocation, reverse=True
    )

    for symbol, sizing in sorted_positions[:6]:
        print(
            f"      {symbol}: {sizing.final_allocation:.1%} "
    ...
    for regime, market_vol, portfolio_sharpe in test_regimes:
        ...
        print(
            f"      {regime}: {dynamic_leverage:.2f}x leverage "
    ...
    print(
        f"      Portfolio Sharpe Change: {portfolio_allocation.portfolio_sharpe:.2f} → {rebalanced_allocation.portfolio_sharpe:.2f}"
    )
    ...
    for symbol in list(final_allocations.keys())[:5]:
        ...
        if abs(change) > 0.005:  # Show changes > 0.5%
            ...
            print(
                f"      {symbol}: {original:.1%} → {new_size:.1%} "
    ...
    print(
        f"      Concentration Score: {cluster_summary['diversification_metrics']['concentration_score']:.2f}"
    )
    print(
        f"      Max Single Allocation: {cluster_summary['diversification_metrics']['max_single_allocation']:.1%}"
    )
    ...
    if portfolio_allocation.portfolio_sharpe < 1.0:
        print(
            f"      📊 Consider increasing alpha generation - Sharpe ratio is {portfolio_allocation.portfolio_sharpe:.2f}"
        )
    ...
    print(
        f"   ✅ Market regime detection with {np.mean([s.confidence for s in regime_signals.values()]):.1%} avg confidence"
    )
    print(
        f"   ✅ Portfolio optimization with {portfolio_allocation.portfolio_sharpe:.2f} Sharpe ratio"
    )
    print(
        f"   ✅ Cluster risk management across {cluster_summary['diversification_metrics']['active_clusters']} clusters"
    )
    print(
        f"   ✅ Return attribution with {portfolio_attribution.performance_quality:.1%} quality score"
    )
