...
class ExecutionSimulator:
    ...
    def _generate_order_book(
        self, symbol: str, market_data: Optional[pd.DataFrame], timestamp: datetime
    ) -> OrderBook:
    ...
    def _simulate_latency(self) -> float:
        ...
        return max(self.config.min_latency_ms, min(self.config.max_latency_ms, latency))

    def _calculate_market_impact(self, symbol: str, quantity: float, daily_volume: float) -> float:
    ...
    def _simulate_queue_position(
        self, symbol: str, side: OrderSide, limit_price: float, order_book: OrderBook
    ) -> int:
    ...
    def _calculate_execution_probability(
        self, queue_position: int, latency_ms: float, spread_bps: float
    ) -> float:
    ...
    def _update_market_impact_history(self, symbol: str, impact_bps: float, timestamp: datetime):
        ...
        self.market_impact_history[symbol] = [
            (t, impact) for t, impact in self.market_impact_history[symbol] if t > cutoff_time
        ]
    ...
    def get_simulation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fee_rate_bps": (total_fees / (self.total_executed_volume * 50000) * 10000)
            if self.total_executed_volume > 0
            else 0.0,
        }
