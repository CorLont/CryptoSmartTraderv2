...
class FeatureEngineering:
    ...
    def engineer_features(
        ...
        if onchain_data is not None:
            ...
            features_df = features_df.merge(
                onchain_features, on=["timestamp", "symbol"], how="left"
            )
        ...
        self.logger.info(
            f"Feature engineering completed: {len(validated_features.columns)} features created"
        )
    ...
    def _create_technical_features(
        ...
        for symbol, symbol_data in price_data.groupby("symbol"):
            ...
            symbol_features = pd.DataFrame(
                {"timestamp": symbol_data["timestamp"], "symbol": symbol_data["symbol"]}
            )
            ...
            for indicator in ["rsi_14", "macd", "bb_position"]:
                metadata[indicator] = FeatureMetadata(indicator, "technical", 14, datetime.now())
    ...
    def _create_sentiment_features(
        ...
        for symbol, symbol_data in sentiment_data.groupby("symbol"):
            ...
            for feature in [
                ...
                metadata[feature] = FeatureMetadata(feature, "sentiment", 1, datetime.now())
    ...
    def _create_onchain_features(
        ...
        for symbol, symbol_data in onchain_data.groupby("symbol"):
            ...
            for feature in ["active_addresses", "transaction_volume", "whale_activity"]:
                for window in [24, 168]:
                    ...
                    symbol_features[momentum_feature] = (
                        symbol_features[feature] / symbol_features[feature].rolling(window).mean()
                        - 1
                    )
            ...
            for feature in ["active_addresses", "transaction_volume", "whale_activity"]:
                metadata[feature] = FeatureMetadata(feature, "onchain", 1, datetime.now())
    ...
    def _create_cross_asset_features(
        ...
        for symbol in market_data.columns:
            ...
            symbol_features = pd.DataFrame({"timestamp": symbol_timestamps, "symbol": symbol})
            ...
            if symbol in market_returns.columns:
                ...
                symbol_features["relative_strength"] = relative_strength.reindex(
                    symbol_timestamps
                ).values
                symbol_features["market_correlation"] = (
                    symbol_returns.rolling(168)
                    .corr(market_momentum)
                    .reindex(symbol_timestamps)
                    .values
                )
    ...
    def _create_interaction_features(
        ...
        if (
            "volatility_24h" in features_df.columns
            and "sentiment_volatility" in features_df.columns
        ):
    ...
    def _validate_features(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed_features:
            self.logger.warning(
                f"Removed {len(removed_features)} features due to excessive missing values"
            )
