...
from cryptosmarttrader.core.secrets_manager import create_secrets_manager, SecretType, SecretStatus
from cryptosmarttrader.core.log_sanitizer import (
    create_log_sanitizer,
    create_sanitized_logger,
    ExchangeComplianceMonitor,
)
...
async def demonstrate_security_compliance():
    ...
    secrets_manager = create_secrets_manager(secrets_path="demo_secrets", environment="development")
    ...
    retrieved_secret = secrets_manager.get_secret(secret_id="kraken_api_key", user_id="demo_user")
    ...
    for secret_info in secrets_list:
        print(
            f"      {secret_info['secret_id']}: {secret_info['secret_type']} "
    ...
    rotation_success = secrets_manager.rotate_secret(
        secret_id="kraken_api_key", new_secret_value=new_kraken_key, user_id="demo_user"
    )
    ...
    for secret_config in test_secrets:
        log_sanitizer.register_secret(
            secret=secret_config["secret_value"], identifier=secret_config["secret_id"]
        )
    ...
    revocation_success = secrets_manager.revoke_secret(
        secret_id="webhook_secret", user_id="security_admin"
    )
