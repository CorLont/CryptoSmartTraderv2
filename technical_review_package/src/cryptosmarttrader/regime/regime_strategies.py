...
class RegimeStrategies:
    ...
    def get_strategy_for_regime(
        ...
        adjusted_params = TradingParameters(
            ...
            * (2 - confidence_factor),  # Tighter stops when less confident
            take_profit_pct=base_params.take_profit_pct
            * confidence_factor,  # Smaller targets when less confident
            position_size_pct=base_params.position_size_pct
            * confidence_factor,  # Smaller positions when less confident
            max_hold_time=base_params.max_hold_time,
            rebalance_frequency=base_params.rebalance_frequency,
            no_trade=base_params.no_trade
            or confidence_factor < 0.6,  # Avoid trading if low confidence
    ...
    def should_enter_position(
        self, signal_strength: float, regime_params: TradingParameters
    ) -> Dict[str, Any]:
        """
    ...
    def should_exit_position(
        ...
        regime_params: TradingParameters,
    ) -> Dict[str, Any]:
        """
    ...
    def _regime_specific_entry_check(
        self, signal_strength: float, regime_params: TradingParameters
    ) -> Dict[str, Any]:
        ...
        if regime_params.strategy_type.startswith("momentum"):
            ...
            # Default case
            return {"allowed": True, "reason": "Default entry check passed", "risk_level": "medium"}
