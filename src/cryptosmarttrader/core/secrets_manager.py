...
class SecretsManager:
    ...
    def get_secret(
        self, secret_id: str, user_id: str = "system", required_permissions: List[str] = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if metadata.status not in [SecretStatus.ACTIVE, SecretStatus.ROTATING]:
                raise PermissionError(
                    f"Secret {secret_id} is not active (status: {metadata.status.value})"
                )
    ...
    def rotate_secret(self, secret_id: str, new_secret_value: str, user_id: str = "system") -> bool:
        """
        ...
        try:
            ...
            self._log_audit_event(
                ...
                user_id=user_id,
                success=True,
                details={"previous_status": old_status.value, "rotation_type": "manual"},
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            health_status = {
                ...
                "active_secrets": len(
                    [m for m in self.metadata_cache.values() if m.status == SecretStatus.ACTIVE]
                ),
    ...
    def _load_metadata(self):
        ...
        try:
            ...
            for secret_id, metadata_dict in data.items():
                ...
                metadata_dict["last_rotated"] = datetime.fromisoformat(
                    metadata_dict["last_rotated"]
                )
    ...
    def _validate_secret_value(self, secret_value: str, secret_type: SecretType) -> bool:
        ...
        if secret_type == SecretType.API_KEY:
            ...
            return any(
                secret_value.startswith(scheme)
