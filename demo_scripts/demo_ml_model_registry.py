...
from cryptosmarttrader.ml import (
    create_model_registry, create_walk_forward_trainer, create_drift_detector,
    ModelMetrics, ModelType, ModelStatus, TrainingConfig, RetrainingConfig, DriftConfig
)
...
async def demonstrate_ml_model_registry():
    ...
    if success:
        ...
        best_model.deployment_info['canary_start'] = (datetime.utcnow() - timedelta(hours=73)).isoformat()
    ...
    for model_info in all_models:
        print(f"      {model_info['model_id']} {model_info['version']}: "
    ...
    for i, result in enumerate(wf_results):
        print(f"      Fold {i+1}: Train acc {result.train_metrics.accuracy:.3f}, "
    ...
    drift_scenarios = [
        ...
        ("High Drift", extended_data.iloc[3000:3500] + np.random.normal(0, 1, (500, len(extended_data.columns))))  # Noisy data
    ]
    ...
    for scenario_name, drift_data in drift_scenarios:
        ...
        if alerts:
            ...
            for alert in alerts:
                print(f"         {alert.drift_type.value}: {alert.severity.value} "
    ...
    if rollback_success:
        ...
        if current_production:
            print(f"   Current production: {current_production['model_id']} {current_production['version']}")
    else:
    ...
    triggers = walk_forward_trainer.check_retraining_triggers(
        ...
        current_performance=poor_metrics
    )
    ...
    if triggers:
        ...
        if new_model_version:
            print(f"   ✅ New model trained: {new_model_version.model_id} {new_model_version.version}")
    ...
    print(comparison_df[['version', 'status', 'accuracy', 'sharpe_ratio', 'risk_budget_pct']].to_string(index=False))
    ...
    if 'total_alerts' in drift_summary:
        ...
        print(f"      Rollbacks: {rollback_stats['successful_rollbacks']}/{rollback_stats['total_rollbacks']} successful")
    else:
    ...
    print(f"   ✅ Model registry with {len(all_models)} models across {len(status_counts)} statuses")
