...
class TestTechnicalIndicators:
    ...
    def test_rsi_properties(self, prices):
        ...
        assert len([r for r in rsi[:13] if np.isnan(r)]) >= 13, "RSI should have NaN for initial period"
        
        # Property 3: Extreme price movements should push RSI towards extremes
        if len(valid_rsi) > 1:
            # Check if consistently rising prices increase RSI
            rising_prices = all(prices[i] <= prices[i+1] for i in range(len(prices)-5, len(prices)-1))
            if rising_prices and len(valid_rsi) >= 2:
                assert valid_rsi[-1] >= valid_rsi[-2] * 0.9, "RSI should increase with rising prices"
    ...
    def test_macd_properties(self, prices):
        ...
        for i in range(-5, 0):
            if not (np.isnan(ema12[i]) or np.isnan(ema26[i]) or np.isnan(macd_line[i])):
                ...
                assert abs(macd_line[i] - expected_macd) < 0.01, f"MACD calculation error at index {i}"
        ...
        for i in range(-5, 0):
            if not (np.isnan(macd_line[i]) or np.isnan(signal_line[i]) or np.isnan(histogram[i])):
                ...
                assert abs(histogram[i] - expected_hist) < 0.01, f"Histogram calculation error at index {i}"
        ...
        if macd_volatility > 0 and signal_volatility > 0:
            assert signal_volatility <= macd_volatility * 1.5, "Signal line should be smoother than MACD"
    ...
    def test_moving_average_properties(self, data):
        ...
        if len(data) > period * 2:
            ...
            if original_volatility > 0:
                assert ma_volatility <= original_volatility, "Moving average should reduce volatility"
...
class TestSignalLogic:
    ...
    def test_confidence_gating(self, confidence, threshold):
        ...
        assert actual_pass == should_pass, f"Confidence gate failed: {confidence} >= {threshold} should be {should_pass}"
    ...
    def test_position_sizing(self, price, signal_type, position_size):
        ...
        assert actual_size <= max_allowed, f"Position size {actual_size} exceeds maximum {max_allowed}"
