...
class PortfolioOptimizerAgent:
            ...
            if recent_optimizations:
                return {
                    ...
                    "average_portfolio_return": sum(
                        opt.get("expected_return", 0) for opt in recent_optimizations
                    )
                    / len(recent_optimizations),
                    "average_portfolio_risk": sum(
                        opt.get("portfolio_risk", 0) for opt in recent_optimizations
                    )
                    / len(recent_optimizations),
                    "last_optimization": recent_optimizations[-1] if recent_optimizations else None,
                }
            else:
                return {"total_optimizations": 0, "message": "No optimizations performed yet"}
    ...
    def _update_market_data(self):
        ...
        for symbol in self.asset_universe:
            ...
            self.asset_data[symbol] = {
                ...
                "sharpe_ratio": (np.mean(returns) * 365 - self.risk_free_rate)
                / (np.std(returns) * np.sqrt(365)),
    ...
    def _optimize_fractional_kelly(self) -> Optional[PortfolioOptimization]:
        ...
        optimization = PortfolioOptimization(
            ...
            / np.dot(weights, volatilities[: len(weights)]),
        ...
        try:
            ...
            if result.success:
                ...
                optimization = PortfolioOptimization(
                    ...
                    portfolio_cvar=abs(norm.ppf(0.05) * portfolio_volatility * np.sqrt(1 / 252))
                    * 1.3,
                    beta_to_market=0.8,
                    correlation_risk=np.mean(
                        np.abs(covariance_matrix[np.triu_indices_from(covariance_matrix, k=1)])
    ...
    def _update_performance_stats(self):
        ...
        with self._lock:
            ...
            if self.optimization_history:
                ...
                # Convergence rate
                successful = sum(
                    1 for opt in self.optimization_history if opt.optimization_status == "Success"
                )
                self.performance_stats["convergence_rate"] = successful / len(
