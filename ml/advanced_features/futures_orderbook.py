...
class FuturesDataAnalyzer:
    ...
    def calculate_open_interest_features(self, oi_data: pd.DataFrame, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # OI changes
            oi_change_24h = (current_oi - oi_data['open_interest'].iloc[-24]) / oi_data['open_interest'].iloc[-24] if len(oi_data) >= 24 else 0
            oi_volatility = oi_data['open_interest'].pct_change().rolling(24).std().iloc[-1]
            
            # Price-OI divergence
            price_change_24h = (price_data['close'].iloc[-1] - price_data['close'].iloc[-24]) / price_data['close'].iloc[-24] if len(price_data) >= 24 else 0
            ...
            features = {
                ...
                'oi_momentum': oi_data['open_interest'].diff().rolling(8).mean().iloc[-1] / current_oi if current_oi > 0 else 0
            }
    ...
    def calculate_basis_features(self, spot_price: float, futures_price: float, 
                               time_to_expiry_days: float) -> Dict[str, Any]:
...
class OrderBookAnalyzer:
    ...
    def analyze_orderbook_imbalance(self, orderbook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Volume imbalance
            volume_imbalance = (total_bid_volume - total_ask_volume) / (total_bid_volume + total_ask_volume + 1e-8)
            ...
            depth_weighted_imbalance = (weighted_bid_volume - weighted_ask_volume) / (weighted_bid_volume + weighted_ask_volume + 1e-8)
    ...
    def detect_spoofing(self, orderbook_snapshots: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Analyzing {len(orderbook_snapshots)} order book snapshots for spoofing")
            ...
            for i in range(1, len(orderbook_snapshots)):
                ...
                if spoof_pattern:
                    spoof_signals.append({
                        'timestamp': current_book.get('timestamp', datetime.utcnow().isoformat()),
    ...
    def _detect_spoof_pattern(self, prev_book: Dict[str, Any], curr_book: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            large_disappeared_bids = [price for price in disappeared_bids if prev_bids[price] > np.median(list(prev_bids.values()) + [1]) * 5]
            large_disappeared_asks = [price for price in disappeared_asks if prev_asks[price] > np.median(list(prev_asks.values()) + [1]) * 5]
            ...
            if appeared_bids:
                ...
                if max_bid in curr_bids and curr_bids[max_bid] > np.median(list(curr_bids.values()) + [1]) * 3:
                    return {
            ...
            if appeared_asks:
                ...
                if min_ask in curr_asks and curr_asks[min_ask] > np.median(list(curr_asks.values()) + [1]) * 3:
                    return {
                        'type': 'large_order_placement',
                        'side': 'ask',
                        'confidence': 0.6
...
class AdvancedTimingSignals:
    ...
    def _analyze_futures_signals(self, futures_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'funding_rates' in futures_data:
            funding_features = self.futures_analyzer.calculate_funding_features(futures_data['funding_rates'])
    ...
    def _analyze_orderbook_signals(self, orderbook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'orderbook_history' in orderbook_data:
            spoof_result = self.orderbook_analyzer.detect_spoofing(orderbook_data['orderbook_history'])
