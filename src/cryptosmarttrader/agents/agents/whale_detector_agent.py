...
class WhaleDetectorAgent:
                    ...
                    if whale_transaction:
                        with self._lock:
                            ...
                            if whale_transaction.whale_size == WhaleSize.MEGA_WHALE:
                                ...
                                self.logger.warning(
                                    f"MEGA WHALE detected: {symbol} ${whale_transaction.amount_usd:,.0f}"
                                )
    ...
    def _analyze_trade_for_whale_activity(
        ...
        activity_type = (
            WhaleActivityType.LARGE_BUY if trade["side"] == "buy" else WhaleActivityType.LARGE_SELL
        )

        # Calculate whale confidence (simplified heuristic)
        whale_confidence = min(
            1.0, (trade_value / self.whale_thresholds["small_whale_usd"]) * 0.2 + 0.5
        )
    ...
    def _detect_exchange_flows(self):
        ...
        for flow in simulated_flows:
            ...
            self.logger.info(
                f"Exchange flow detected: {flow['symbol']} {flow['flow_type']} ${flow['amount_usd']:,.0f}"
            )
    ...
    def _update_whale_metrics(self):
        ...
        with self._lock:
            ...
            recent_transactions = [
                tx for tx in self.whale_transactions if tx.timestamp > cutoff_time
            ]
    ...
    def _calculate_whale_metrics(
        self, symbol: str, transactions: List[WhaleTransaction]
    ) -> WhaleMetrics:
        ...
        # Volume metrics
        buy_transactions = [
            tx for tx in transactions if tx.activity_type == WhaleActivityType.LARGE_BUY
        ]
        sell_transactions = [
            tx for tx in transactions if tx.activity_type == WhaleActivityType.LARGE_SELL
        ]
        ...
        avg_transaction_size = (
            total_volume / large_transactions_24h if large_transactions_24h > 0 else 0
        )
        unusual_activity_detected = (
            large_transactions_24h > 10  # Many transactions
            or avg_transaction_size > self.whale_thresholds["large_whale_usd"]  # Very large average
            or abs(whale_sentiment_score) > 0.7  # Strong directional bias
        )
        ...
        for symbol, metrics in self.whale_metrics.items():
            ...
            if metrics.large_transactions_24h > 0:
                recent_large_tx = [
                    ...
                    if tx.symbol == symbol
                    and tx.timestamp > datetime.now() - timedelta(hours=1)
                    and tx.whale_size in [WhaleSize.LARGE_WHALE, WhaleSize.MEGA_WHALE]
                ]
    ...
    def get_whale_alerts(self, symbol: str = None, severity_min: int = 1) -> List[WhaleAlert]:
        ...
        with self._lock:
            alerts = [
                alert
                for alert in self.whale_alerts
    ...
    def get_whale_signals(self, min_confidence: float = 0.7) -> List[Dict[str, Any]]:
        ...
        for symbol, metrics in self.whale_metrics.items():
            ...
            if (
                metrics.accumulation_ratio > 0.7
                and metrics.large_transactions_24h >= 3
                and metrics.net_whale_flow > 1000000
            ):  # $1M+ net buying
                signals.append(
                    {
                        "symbol": symbol,
            ...
            # Strong distribution signal
                ...
                metrics.accumulation_ratio < 0.3
                and metrics.large_transactions_24h >= 3
                and metrics.net_whale_flow < -1000000
            ):  # $1M+ net selling
                signals.append(
