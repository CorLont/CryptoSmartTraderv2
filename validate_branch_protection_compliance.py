...
def validate_branch_protection():
    ...
    if codeowners_file.exists():
        ...
        print(
            f"   {'✅' if protected_paths >= 4 else '❌'} Critical paths protected ({protected_paths}/5)"
        )
    else:
    ...
    if protection_workflow.exists():
        ...
        compliance_results["required_checks"] = (
            has_security_check and has_test_checks and has_quality_check
        )
        compliance_results["review_requirements"] = (
            has_review_requirements and has_codeowner_reviews
        )

        print(f"   ✅ Branch protection workflow exists")
        print(f"   {'✅' if has_status_checks else '❌'} Required status checks configured")
        print(
            f"   {'✅' if compliance_results['required_checks'] else '❌'} All required checks present"
        )
        print(
            f"   {'✅' if compliance_results['review_requirements'] else '❌'} Code owner reviews required"
        )
    else:
    ...
    if pr_template.exists():
        ...
        print(
            f"   {'✅' if compliance_results['comprehensive_template'] else '❌'} Comprehensive sections ({sections_present}/5)"
        )
    ...
    if ci_workflow.exists():
        ...
        required_jobs = ["security", "test", "quality"]

        jobs_present = sum(
            1 for job in required_jobs if f"name: {job.title()}" in content or f"{job}:" in content
        )
        ...
        print(
            f"   {'✅' if compliance_results['required_jobs'] else '❌'} Required jobs present ({jobs_present}/3)"
        )
