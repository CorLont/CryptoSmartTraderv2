...
class CleanEnsembleVotingAgent:
    ...
    def generate_authentic_prediction(self, symbol: str, market_data: Dict[str, Any]):
        ...
        ensemble_prediction = self._create_authenticated_ensemble(symbol, authenticated_predictions)
    ...
    def _create_authenticated_ensemble(
        ...
        ensemble_direction = (
            "up" if ensemble_return > 0.5 else "down" if ensemble_return < -0.5 else "neutral"
        )
