...
class HealthMonitor:
    ...
    def check_health_degradation(self, lookback_minutes: int = 30) -> bool:
        ...
        recent_scores = [
            h.overall_score for h in self.health_history
...
class PositionManager:
    ...
    async def monitor_positions(self):
        ...
        while True:
            try:
                ...
                if self.risk_manager.check_kill_switch_conditions(self.current_positions, market_data):
    ...
    async def _check_emergency_conditions(self, health_score: HealthScore):
        ...
        if (health_score.overall_score < self.health_thresholds['auto_flatten'] and
            self.auto_flatten_enabled and not self.emergency_mode):
    ...
    async def _execute_emergency_flatten(self, reason: str):
        ...
        self.logger.critical(
            ...
            extra={
                ...
                'total_exposure': sum(abs(pos.market_value) for pos in self.current_positions.values())
            }
        )

        positions_before = {symbol: pos.market_value for symbol, pos in self.current_positions.items()}
        ...
        positions_after = {symbol: pos.market_value for symbol, pos in self.current_positions.items()}
        ...
        emergency_action = EmergencyAction(
            ...
            health_score=self.health_monitor.health_history[-1].overall_score if self.health_monitor.health_history else 0,
        ...
        for symbol, recommendation in recommendations.items():
            if recommendation['action'] in ['flatten', 'reduce']:
                ...
                if current_position:
                    ...
                    target_size = target_value / current_position.current_price if current_position.current_price > 0 else 0

                    order = PositionOrder(
                        order_id=f"auto_flatten_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                        symbol=symbol,
                        command=PositionCommand.REDUCE if recommendation['action'] == 'reduce' else PositionCommand.CLOSE,
    ...
    async def _execute_position_order(self, order: PositionOrder):
        ...
        try:
            ...
            if executed_order.status.value in ['filled', 'partially_filled']:
                ...
                order.status = 'failed'
                order.execution_result = {'error': f"Execution failed: {executed_order.status.value}"}
    ...
    def get_position_summary(self) -> Dict[str, Any]:
        ...
        current_health = self.health_monitor.health_history[-1] if self.health_monitor.health_history else None
        ...
        return {
            ...
            'last_emergency': self.emergency_actions[-1].timestamp.isoformat() if self.emergency_actions else None
        }
