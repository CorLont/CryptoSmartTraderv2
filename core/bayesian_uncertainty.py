...
class BayesianUncertaintyModel:
    ...
    def __init__(self, container):
        ...
        self.uncertainty_models = {"1h": None, "24h": None, "7d": None, "30d": None}
            ...
            if GP_AVAILABLE:
                ...
                for horizon in self.uncertainty_models.keys():
                    self.uncertainty_models[horizon] = RandomForestRegressor(
                        n_estimators=100, max_depth=10, random_state=42, bootstrap=True
                    )
    ...
    def estimate_prediction_uncertainty(
        self, features: np.ndarray, predictions: Dict[str, float], coin: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for horizon, prediction in predictions.items():
                if (
                    horizon in self.uncertainty_models
                    and self.uncertainty_models[horizon] is not None
                ):
    ...
    def _estimate_single_horizon_uncertainty(
        self, features: np.ndarray, prediction: float, horizon: str
    ) -> float:
    ...
    def _heuristic_uncertainty(self, prediction: float, horizon: str) -> float:
        ...
        # Horizon multipliers
        horizon_multipliers = {"1h": 1.0, "24h": 1.5, "7d": 2.0, "30d": 3.0}
    ...
    def _calculate_bayesian_confidence(
        self, prediction: float, uncertainty: float, horizon: str
    ) -> float:
        ...
        try:
            ...
            # Bayesian update
            posterior_confidence = self._bayesian_update(prior_confidence, likelihood, uncertainty)
    ...
    def _bayesian_update(self, prior: float, likelihood: float, uncertainty: float) -> float:
    ...
    def _get_historical_accuracy(self, horizon: str) -> float:
        ...
        default_accuracy = {"1h": 0.75, "24h": 0.70, "7d": 0.65, "30d": 0.60}
    ...
    def _calculate_overall_uncertainty(self, uncertainty_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            uncertainties = [
                data["uncertainty"] for data in horizons_data.values() if "uncertainty" in data
            ]

            # Collect confidence scores
            confidences = [
                data["bayesian_confidence"]
                for data in horizons_data.values()
    ...
    def _assess_prediction_quality(
        self, uncertainties: List[float], confidences: List[float]
    ) -> str:
    ...
    def train_uncertainty_model(
        self, training_data: List[Dict[str, Any]], horizon: str
    ) -> Dict[str, Any]:
    ...
    def _prepare_uncertainty_training_data(
        self, training_data: List[Dict[str, Any]], horizon: str
    ...
    def _evaluate_uncertainty_model(
        self, model: Any, X: np.ndarray, y: np.ndarray, horizon: str
    ) -> Dict[str, float]:
    ...
    def filter_predictions_by_uncertainty(
        ...
        for prediction in predictions:
            ...
            # Apply filters
            if mean_uncertainty <= max_uncertainty and mean_confidence >= min_confidence:
                # Add quality score
    ...
    def get_uncertainty_status(self) -> Dict[str, Any]:
        ...
        return {
            "models_initialized": sum(
                1 for model in self.uncertainty_models.values() if model is not None
            ),
            ...
            "calibration_data": {
                horizon: {
                    ...
                    "last_updated": data.get("last_updated", "").isoformat()
