...
from cryptosmarttrader.portfolio.kelly_sizing import (
    FractionalKellySizer, KellyParameters, KellySizingResult, KellyMode
)
from cryptosmarttrader.portfolio.correlation_manager import (
    CorrelationManager, CorrelationShockScenario, ClusterType, CorrelationLevel
)
...
class PortfolioManagementTester:
    ...
    def test_fractional_kelly_sizing(self) -> bool:
        ...
        try:
            ...
            # Test constraints
            constraints_applied = len(result.applied_caps) > 0 or len(high_vol_result.applied_caps) > 0
    ...
    def test_volatility_targeting(self) -> bool:
        ...
        try:
            ...
            for scenario_name, asset_vol, portfolio_vol, target_vol in scenarios:
                ...
                logger.info(f"{scenario_name}: {base_size:.3f} → {adjusted_size:.3f} (ratio: {vol_ratio:.2f})")
    ...
    def test_cluster_limits(self) -> bool:
        ...
        try:
            ...
            for cluster, info in cluster_summary.items():
                logger.info(f"  {cluster}: {info['weight']:.3f}/{info['max_weight']:.3f} weight, "
    ...
    def test_correlation_shock_stress(self) -> bool:
        ...
        try:
            ...
            for scenario_name, result in stress_results.items():
                ...
                logger.info(f"  Avg Correlation: {result.correlation_metrics['average_correlation']:.3f}")
            ...
            test_success = (
                0 < policy_breaches <= len(stress_scenarios) and
                stress_escalation and
                0.05 <= avg_max_drawdown <= 0.30
            )
    ...
    def test_integrated_portfolio_system(self) -> bool:
        ...
        try:
            ...
            # Decision logic
            position_approved = (
                kelly_result.is_valid and
                new_position_size > 0.001 and
                len(risk_flags) == 0
            )
    ...
    def run_comprehensive_tests(self):
        ...
        if 'correlation_stress' in self.test_results:
            ...
            logger.info(f"• Policy Breaches: {stress.get('policy_breaches', 0)}/{stress.get('scenarios_tested', 0)}")
        
        if 'cluster_limits' in self.test_results:
            cluster = self.test_results['cluster_limits']
            logger.info(f"• Cluster Violations Detected: {cluster.get('violations_detected', False)}")
