...
class AttributionResult:
    ...
    def cost_drag(self) -> float:
        ...
        cost_sources = [AttributionSource.FEES, AttributionSource.SLIPPAGE, AttributionSource.MARKET_IMPACT]
...
class ReturnAttributor:
    ...
    def attribute_returns(self,
        ...
        try:
            ...
            period_trades = trade_data[
                (trade_data['timestamp'] >= start_date) &
                (trade_data['timestamp'] <= end_date)
            ].copy()
            ...
            sharpe = np.mean(returns_daily) / np.std(returns_daily) * np.sqrt(252) if len(returns_daily) > 1 else 0
            ...
            logger.info(f"Attribution completed: {len(components)} components, {attribution_accuracy:.1%} accuracy")
    ...
    def _analyze_attribution_components(self,
                                      trade_data: pd.DataFrame,
                                      market_data: pd.DataFrame = None) -> List[AttributionComponent]:
    ...
    def _analyze_alpha_attribution(self,
                                  trade_data: pd.DataFrame,
                                  market_data: pd.DataFrame = None) -> Optional[AttributionComponent]:
        ...
        try:
            ...
            slippage = trade_data['slippage_cost'].sum() if 'slippage_cost' in trade_data.columns else 0
            ...
            if 'expected_return' in trade_data.columns:
                alpha_per_trade = trade_data['realized_pnl'] - trade_data.get('fees', 0) - trade_data.get('slippage_cost', 0)
    ...
    def _analyze_slippage_attribution(self, trade_data: pd.DataFrame) -> Optional[AttributionComponent]:
        ...
        try:
            ...
            if 'slippage_bps' in trade_data.columns:
                ...
                price_diff = (trade_data['actual_price'] - trade_data['expected_price']) / trade_data['expected_price']
            ...
            benchmark_slippage = np.mean([self.benchmark_costs["slippage_bps"].get(pt, 15) for pt in pair_types])
        ...
        try:
            ...
            return AttributionComponent(
                ...
                context={
                    ...
                    "timing_hit_rate": len([t for t in timing_impacts if t > 0]) / len(timing_impacts) if timing_impacts else 0
                }
            )
    ...
    def _analyze_sizing_attribution(self, trade_data: pd.DataFrame) -> Optional[AttributionComponent]:
        """Analyze position sizing impact"""
        try:
            if 'position_size' not in trade_data.columns or 'optimal_size' not in trade_data.columns:
                return None
            ...
            return AttributionComponent(
                ...
                trend="stable",
                context={
                    "avg_size_ratio": np.mean(trade_data['position_size'] / trade_data['optimal_size']),
                    "oversized_trades": len(trade_data[trade_data['position_size'] > trade_data['optimal_size']]),
                    "undersized_trades": len(trade_data[trade_data['position_size'] < trade_data['optimal_size']])
                }
            )
    ...
    def _analyze_market_impact_attribution(self, trade_data: pd.DataFrame) -> Optional[AttributionComponent]:
        ...
        try:
            ...
            return AttributionComponent(
                ...
                context={
                    ...
                    "impact_per_size": avg_impact / np.mean(trade_data.get('order_size', [1000])) if len(trade_data) > 0 else 0
                }
            )
    ...
    def _analyze_volatility_attribution(self,
                                      trade_data: pd.DataFrame,
                                      market_data: pd.DataFrame) -> Optional[AttributionComponent]:
    ...
    def _analyze_regime_attribution(self,
                                   trade_data: pd.DataFrame,
                                   market_data: pd.DataFrame) -> Optional[AttributionComponent]:
    ...
    def _attribute_by_time(self, trade_data: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        ...
        try:
            ...
            if 'timestamp' in trade_data.columns:
                ...
                trade_data_copy['day_of_week'] = pd.to_datetime(trade_data_copy['timestamp']).dt.day_name()
                trade_data_copy['month'] = pd.to_datetime(trade_data_copy['timestamp']).dt.month

                # Hourly attribution
                time_attribution['hourly'] = trade_data_copy.groupby('hour')['realized_pnl'].sum().to_dict()

                # Daily attribution
                time_attribution['daily'] = trade_data_copy.groupby('day_of_week')['realized_pnl'].sum().to_dict()

                # Monthly attribution
                time_attribution['monthly'] = trade_data_copy.groupby('month')['realized_pnl'].sum().to_dict()
    ...
    def _create_empty_attribution(self, start_date: datetime, end_date: datetime) -> AttributionResult:
    ...
    def get_attribution_trends(self, days_back: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            for source in AttributionSource:
                ...
                if source_values:
                    component_trends[source.value] = {
                        ...
                        "trend": "improving" if source_values[-1] > np.mean(source_values[:-1]) else "stable",
            ...
            return {
                ...
                "overall_attribution_quality": np.mean([r.attribution_accuracy for r in recent_results]),
                "top_contributors": sorted(component_trends.items(),
