...
class AdvancedLogger:
    ...
    def _create_component_logger(self, component: str):
        ...
        json_handler = TimedRotatingFileHandler(
            json_log_file, when="midnight", interval=1, backupCount=30, encoding="utf-8"
        )
        ...
        text_handler = TimedRotatingFileHandler(
            text_log_file, when="midnight", interval=1, backupCount=30, encoding="utf-8"
        )
    ...
    def log_performance(
        self, operation: str, execution_time: float, additional_metrics: Dict[str, Any] = None
    ):
    ...
    def log_user_action(
        self, action: str, user_id: str = None, session_data: Dict[str, Any] = None
    ):
        ...
        logger.info(f"User Action: {action}", extra={"data": extra_data, "user_id": user_id})
    ...
    def log_error(self, component: str, error: Exception, context: Dict[str, Any] = None):
        ...
        logger.error(f"Error in {component}: {error}", extra={"data": extra_data}, exc_info=True)

    def log_data_pipeline(
        self, stage: str, records_processed: int, success_rate: float, timing: float
    ):
...
def log_performance(
    operation: str, execution_time: float, additional_metrics: Dict[str, Any] = None
):
...
class PerformanceTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        get_advanced_logger().log_performance(
            self.operation_name, execution_time, {"success": exc_type is None}
        )
