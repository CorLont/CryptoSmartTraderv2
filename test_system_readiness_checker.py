...
def test_system_readiness_checker():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("readiness_checker", "core/system_readiness_checker.py")
        ...
        if consistent_naming:
            ...
            if "{horizon}_xgb.pkl" in xgb_pattern and "{horizon}_tree.pkl" in tree_pattern:
                print("   ✅ PASSED: XGBoost and tree patterns use consistent horizon_type.pkl format")
            else:
        ...
        if clean_issues:
            print(f"   ✅ PASSED: Issues list is clean ({len(model_readiness.issues)} issues, no empty strings)")
        ...
        if report.timestamp and len(report.components) >= 4:
            print(f"   ✅ PASSED: Complete readiness check with {len(report.components)} components")
        ...
        print(f"   - Overall score: {overall_score:.1f}% (threshold: {checker.config['readiness']['minimum_overall_score']})")
        ...
        expected_decision = (overall_score >= checker.config["readiness"]["minimum_overall_score"] and critical_ready)
