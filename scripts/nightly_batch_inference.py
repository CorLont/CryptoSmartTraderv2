...
from core.batch_inference_engine import (
    get_batch_inference_engine, BatchInferenceConfig, HorizonType, InferenceStatus
)
...
async def run_nightly_batch_inference(
    ...
    try:
        ...
        print(f"   Success Rate: {result.successful_predictions / result.total_coins * 100:.1f}%" if result.total_coins > 0 else "N/A")
        ...
        if result.predictions:
            ...
            for horizon, values in horizon_stats.items():
                print(f"   {horizon.value}: {len(values)} predictions (avg: {sum(values)/len(values):.3f})")
...
def print_usage_examples():
    ...
    print("  # Every 4 hours")
    print("  0 */4 * * * /usr/bin/python3 /path/to/scripts/nightly_batch_inference.py --mode 4hourly")
    print()
...
Examples:
  ...
  python scripts/nightly_batch_inference.py --batch-size 200    # Larger batches
    ...
    parser.add_argument(
        '--coins',
        nargs='+',
        help='Specific coins to process (default: all coins)'
    )
    
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Test configuration without running inference'
    )
