...
class SlippageModel:
    ...
    def calculate_slippage(
        ...
        if not levels:
            ...
            slippage_price = mid_price * (1 + slippage_rate if order_side == OrderSide.BUY else 1 - slippage_rate)
...
class OrderBookSimulator:
    ...
    def _update_liquidity_after_execution(
        self, 
        side: OrderSide, 
        execution_levels: List[Tuple[float, float]]
    ):
    ...
    def get_market_impact_estimate(
        self, 
        quantity: float, 
        side: OrderSide
    ) -> Dict[str, float]:
