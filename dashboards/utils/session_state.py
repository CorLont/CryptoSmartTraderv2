        ...
        for key, expected_types in type_checks.items():
            if key in st.session_state:
                ...
                if not isinstance(value, expected_types):
                    issues[key] = f"Invalid type for {key}: expected {expected_types}, got {type(value)}"
        ...
        for key, (min_val, max_val) in range_checks.items():
            if key in st.session_state:
                ...
                if isinstance(value, (int, float)) and not (min_val <= value <= max_val):
                    issues[key] = f"Value out of range for {key}: {value} not in [{min_val}, {max_val}]"
