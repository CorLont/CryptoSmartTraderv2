...
class EnhancedLogger:
    ...
    def log_data_quality(self, dataset: str, quality_score: float,
                        issues: list = None, record_count: int = None):
        ...
        if quality_score < 0.7:
            self.warning(f"Data quality issue in {dataset}: score {quality_score:.2f}", quality_data)
        else:
            self.info(f"Data quality check: {dataset} score {quality_score:.2f}", quality_data)

    def log_ml_metrics(self, model_name: str, metrics: Dict[str, float],
    ...
    def log_trade_signal(self, symbol: str, signal: str, confidence: float,
        ...
        self.info(f"Trade signal: {signal} for {symbol} (confidence: {confidence:.2f})", signal_data)

    def log_system_health(self, component: str, health_score: float,
        ...
        if health_score < 0.5:
            ...
            self.warning(f"System health degraded: {component} score {health_score:.2f}", health_data)
        else:
