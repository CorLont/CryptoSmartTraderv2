...
class RegimeFeatures:
    ...
    def calculate_volatility(self, prices: pd.Series, windows: List[int] = [5, 20, 60]) -> pd.DataFrame:
    ...
    def calculate_regime_indicators(self, prices: pd.Series, volume: Optional[pd.Series] = None) -> pd.DataFrame:
        ...
        try:
            ...
            for symbol in df['symbol'].unique():
                ...
                symbol_features = pd.concat([
                    returns_features, vol_features, trend_features, regime_features
                ], axis=1)
            ...
            if all_features:
                ...
                self.logger.info(f"Extracted regime features: {len(result_df)} samples, {len(result_df.columns)} features")
                return result_df
            else:
...
class HMMRegimeDetector:
    ...
    def prepare_hmm_features(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        feature_cols = [
            ...
            'trend_strength_10d', 'trend_strength_20d',
            'drawdown'
        ]
    ...
    def fit_hmm(self, features_df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Training HMM with {len(X_scaled)} samples, {X_scaled.shape[1]} features")
...
class RegimeDetector:
    ...
    def detect_regimes(self, price_data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(f"Regime detection completed: {len(result_df)} samples with {len(regime_columns)} regime labels")
    ...
    def get_regime_summary(self, regimes_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                'symbols': list(regimes_df['symbol'].unique()) if 'symbol' in regimes_df.columns else [],
                'time_range': {
                    'start': regimes_df['timestamp'].min().isoformat() if 'timestamp' in regimes_df.columns else None,
                    'end': regimes_df['timestamp'].max().isoformat() if 'timestamp' in regimes_df.columns else None
                }
            }

            # Regime distributions
            regime_columns = [col for col in regimes_df.columns if 'regime' in col.lower() and col != 'hmm_regime_prob']
