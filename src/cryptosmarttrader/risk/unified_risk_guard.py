...
class UnifiedRiskGuard:
    ...
    def _execute_unified_risk_gates(
        ...
        # Set appropriate reason based on decision
        if final_decision == RiskDecision.REDUCE_SIZE:
            reason = f"EXPOSURE_SIZE_REDUCED: {exposure_result.reason}"
        else:
    ...
    def _validate_data_quality(self, market_data: dict[str, Any] | None) -> RiskEvaluationResult:
    ...
    def _calculate_risk_score(self, order: StandardOrderRequest, market_data: dict[str, Any] | None) -> float:
