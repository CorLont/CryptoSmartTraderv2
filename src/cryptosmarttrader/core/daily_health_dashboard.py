...
class DailyHealthDashboard:
    ...
    def collect_confidence_gate_stats(self) -> Dict[str, Any]:
        ...
        if not gate_files:
            return {
                'status': 'no_gate_data',
                'message': 'No confidence gate data available'
            }
    ...
    def collect_coverage_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return coverage_data.get('metrics', {
                'total_coins': 0,
                'covered_coins': 0,
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if sys_health['cpu_usage'] > 80:
            recommendations.append("High CPU usage detected - consider optimizing workload distribution")
        if sys_health['memory_usage'] > 85:
            recommendations.append("High memory usage - review cache settings and process allocation")
        ...
        if risk['critical_alerts'] > 0:
            recommendations.append(f"Critical alerts detected ({risk['critical_alerts']}) - immediate attention required")
        ...
        if 'avg_accuracy' in model_perf and model_perf['avg_accuracy'] < 0.6:
            recommendations.append("Low model accuracy - consider retraining or feature engineering")
...
if __name__ == "__main__":
    ...
    print(f"   Overall Health: {report['overall_health']['overall_score']:.1f}% ({report['overall_health']['health_level']})")
