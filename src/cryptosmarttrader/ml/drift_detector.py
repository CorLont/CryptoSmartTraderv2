...
class DriftDetector:
    ...
    def detect_drift(self,
                    ...
                    current_performance: Optional[Dict[str, List[float]]] = None) -> List[DriftAlert]:
        """
        ...
        if current_performance and baseline_key in self.performance_baseline:
            perf_drift_alert = self._detect_performance_drift(
                ...
                current_performance, 
                model_id, version, detection_time
            )
            if perf_drift_alert:
    ...
    def monitor_continuous_drift(self,
                               ...
                               performance_stream: Optional[Dict[str, List[float]]] = None) -> bool:
        """
        ...
        if critical_alerts:
            ...
            if any(a.rollback_recommended and a.rollback_confidence > self.config.auto_rollback_threshold 
    ...
    def _detect_data_drift(self,
                          ...
                          model_id: str,
                          version: str,
                          detection_time: datetime) -> Optional[DriftAlert]:
    ...
    def _test_numerical_drift(self,
                             baseline_values: pd.Series,
                             current_values: pd.Series,
                             feature_name: str) -> Tuple[float, Dict[str, float]]:
    ...
    def _test_categorical_drift(self,
                               baseline_values: pd.Series,
                               current_values: pd.Series,
                               feature_name: str) -> Tuple[float, Dict[str, float]]:
        ...
        if stats and len(all_categories) > 1:
            try:
                ...
                if total_baseline > 0 and total_current > 0:
                    ...
                    chi2_stat = np.sum(((expected_current - expected_baseline) ** 2) / 
                                     (expected_baseline + 1e-8))
    ...
    def _detect_performance_drift(self,
        ...
        return DriftAlert(
            ...
            baseline_period=(detection_time - timedelta(days=self.config.baseline_window_days), 
                           detection_time - timedelta(days=self.config.detection_window_days)),
            current_period=(detection_time - timedelta(days=self.config.detection_window_days), 
                          detection_time),
            sample_sizes={
    ...
    def _detect_prediction_drift(self,
        ...
        try:
            ...
            baseline_sample = baseline_features.sample(min(1000, len(baseline_features)), 
                                                     random_state=42)
            current_sample = current_features.sample(min(1000, len(current_features)), 
                                                   random_state=42)
            ...
            statistical_tests = {
                'prediction_stats': {
                    'baseline_mean_prob': np.mean(baseline_pos_proba) if 'baseline_pos_proba' in locals() else 0,
                    'current_mean_prob': np.mean(current_pos_proba) if 'current_pos_proba' in locals() else 0,
    ...
    def _calculate_psi(self, baseline: pd.Series, current: pd.Series, buckets: int = 10) -> float:
        ...
        try:
            ...
            baseline_props = baseline_props.reindex(baseline_props.index.union(current_props.index), fill_value=0.001)
    ...
    def _update_feature_weights(self, model_id: str, version: str, data: pd.DataFrame):
        ...
        try:
            ...
            features = [col for col in data.columns if col != model_version.dataset_info.target_column]
    ...
    def _calculate_confidence_intervals(self, drift_scores: Dict[str, float]) -> Dict[str, Tuple[float, float]]:
    ...
    def _generate_rollback_reasons(self, drift_score: float, affected_features: List[str]) -> List[str]:
    ...
    def _generate_drift_actions(self, severity: DriftSeverity, affected_features: List[str]) -> List[str]:
    ...
    def _generate_performance_actions(self, severity: DriftSeverity, affected_metrics: List[str]) -> List[str]:
    ...
    def _evaluate_rollback_recommendation(self, alert: DriftAlert, model_id: str, version: str):
        ...
        if (alert.rollback_confidence > self.config.auto_rollback_threshold and 
    ...
    def _execute_automatic_rollback(self, model_id: str, version: str, alerts: List[DriftAlert]):
        ...
        try:
            # Execute rollback
            rollback_reason = f"Automatic rollback due to critical drift: {[a.drift_type.value for a in alerts]}"
            ...
            if success:
                ...
                rollback_record = {
                    ...
                    'alerts': [{'type': a.drift_type.value, 'severity': a.severity.value} for a in alerts],
                    'success': True
                }
    ...
    def get_drift_summary(self) -> Dict[str, Any]:
        ...
        # Recent alerts
        recent_alerts = [a for a in self.drift_history 
                        if (datetime.utcnow() - a.detection_time).days < 7]
        ...
        summary = {
            ...
            'drift_by_type': {
                drift_type: {
                    'count': len(alerts),
                    'avg_severity': np.mean([4 if a.severity == DriftSeverity.CRITICAL else 
                                           3 if a.severity == DriftSeverity.HIGH else
                                           2 if a.severity == DriftSeverity.MEDIUM else 1 
                                           for a in alerts])
                }
                for drift_type, alerts in drift_by_type.items()
            },
            'most_affected_features': sorted(feature_drift_count.items(), 
...
def create_drift_detector(model_registry: ModelRegistry, 
                         config: Optional[DriftConfig] = None) -> DriftDetector:
