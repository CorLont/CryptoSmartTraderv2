        ...
        try:
            ...
            if len(recent_failures) >= self.max_failed_attempts:
                self.log_security_event(
                    "LOCKOUT_TRIGGERED",
                    {
                        "identifier": self.hash_sensitive_data(identifier),
    ...
    def validate_api_access(self, api_key: str, source_ip: str = "unknown") -> bool:
        ...
        try:
            ...
            if not self.validate_input(api_key, "api_key"):
                self.log_security_event(
                    "INVALID_API_KEY_FORMAT", {"source_ip": source_ip}, "WARNING"
                )
                return False
            ...
            if self.is_locked_out(f"api_{source_ip}"):
                self.log_security_event(
                    "LOCKED_OUT_ACCESS_ATTEMPT", {"source_ip": source_ip}, "CRITICAL"
                )
                return False
            ...
            self.log_security_event(
                "API_VALIDATION_ERROR", {"source_ip": source_ip, "error": str(e)}, "ERROR"
            )
            return False
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "audit_log_size": self.audit_log_path.stat().st_size
                if self.audit_log_path.exists()
                else 0,
            }
