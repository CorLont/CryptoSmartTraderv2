...
class SentimentAgent:
    ...
    def __init__(self, config_manager=None):
        ...
        self.tracked_symbols = [
            "BTC", "ETH", "BNB", "XRP", "ADA", "SOL", "AVAX", "DOT", "MATIC", "LINK",
            "UNI", "AAVE", "COMP", "MKR", "SNX", "SUSHI", "1INCH", "CRV", "YFI", "ALCX"
        ]
        
        # Sentiment keywords
        self.sentiment_keywords = {
            "extremely_bullish": [
                "moon", "rocket", "explosive", "massive rally", "bull run", "breakthrough",
                "adoption", "partnership", "major upgrade", "game changer", "revolutionary"
            ],
            "bullish": [
                "bullish", "positive", "growth", "upgrade", "improvement", "optimistic",
                "rising", "surge", "pump", "gains", "rally", "uptrend"
            ],
            "bearish": [
                "bearish", "decline", "drop", "crash", "dump", "negative", "concern",
                "falling", "loss", "risk", "correction", "weakness"
            ],
            "extremely_bearish": [
                "collapse", "disaster", "crash", "panic", "catastrophic", "massive sell-off",
                "regulatory ban", "hack", "exploit", "rug pull", "scam"
            ]
        }
            ...
            try:
                ...
                for entry in feed.entries[:10]:  # Latest 10 articles per source
                    ...
                    for symbol in mentioned_symbols:
                        sentiment_score, confidence = self._analyze_text_sentiment(full_content, symbol)
    ...
    def _get_ai_sentiment_analysis(self, symbol: str, content: str) -> Optional[SentimentData]:
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert cryptocurrency sentiment analyst."},
            ...
            sentiment_data = SentimentData(
                ...
                sentiment_category=self._score_to_category(float(ai_result.get('sentiment_score', 0))),
    ...
    def _analyze_text_sentiment(self, text: str, symbol: str) -> Tuple[float, float]:
        ...
        try:
            ...
            combined_sentiment = (
                0.4 * textblob_sentiment +
                0.4 * keyword_sentiment +
                0.2 * symbol_context
            )
            ...
            confidence = min(0.9, max(0.3, 
                len(text) / 1000 + 
                (0.3 if any(kw in text.lower() for kw_list in self.sentiment_keywords.values() for kw in kw_list) else 0)
            ))
    ...
    def _analyze_symbol_context(self, text: str, symbol: str) -> float:
        ...
        # Positive contexts
        positive_contexts = [
            f"{symbol_lower} upgrade", f"{symbol_lower} partnership", 
            f"{symbol_lower} adoption", f"{symbol_lower} integration",
            f"{symbol_lower} bullish", f"{symbol_lower} rally"
        ]
        
        # Negative contexts
        negative_contexts = [
            f"{symbol_lower} hack", f"{symbol_lower} exploit",
            f"{symbol_lower} crash", f"{symbol_lower} dump",
            f"{symbol_lower} bearish", f"{symbol_lower} decline"
        ]
    ...
    def _estimate_impact_magnitude(self, content: str) -> float:
        ...
        high_impact_keywords = [
            "breaking", "major", "massive", "huge", "significant", "important",
            "partnership", "acquisition", "regulatory", "ban", "approval",
            "listing", "delisting", "hack", "exploit", "upgrade", "fork"
        ]
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        meaningful_words = [
            ...
            if len(word) > 3 and word not in ['this', 'that', 'with', 'from', 'they', 'will', 'have', 'been']
        ]
    ...
    def _calculate_sentiment_summary(self, symbol: str, sentiments: List[SentimentData]) -> SentimentSummary:
        ...
        social_sentiment = self._calculate_source_sentiment(sentiments, [NewsSource.TWITTER, NewsSource.REDDIT])
        official_sentiment = self._calculate_source_sentiment(sentiments, [NewsSource.OFFICIAL_ANNOUNCEMENTS])
        ...
        credibility_weighted_score = sum(credibility_weights) / len(credibility_weights) if credibility_weights else 0
        ...
        # Significant events
        significant_events = [
            s.content[:100] for s in sentiments 
            if s.impact_magnitude > 0.7 and s.confidence > 0.6
        ][:3]  # Top 3 events
    ...
    def _calculate_source_sentiment(self, sentiments: List[SentimentData], sources: List[NewsSource]) -> float:
    ...
    def _detect_sentiment_trend(self, sentiment_1h: float, sentiment_4h: float, sentiment_24h: float) -> str:
    ...
    def _detect_sentiment_shifts(self):
        ...
        for symbol, summary in self.sentiment_summaries.items():
            ...
            if summary.momentum_shift_detected and summary.confidence > 0.6:
                ...
                self.logger.info(f"Sentiment shift detected for {symbol}: {summary.sentiment_trend}")
            ...
            if abs(summary.overall_sentiment) > 0.6 and summary.confidence > 0.7:
                ...
                self.logger.info(f"High confidence {direction} sentiment for {symbol}: {summary.overall_sentiment:.3f}")
    ...
    def get_trending_sentiment(self, min_mentions: int = 5) -> List[Tuple[str, float]]:
        ...
        for symbol, summary in self.sentiment_summaries.items():
            if (summary.total_mentions >= min_mentions and 
                abs(summary.overall_sentiment) > 0.4 and
    ...
    def get_sentiment_signals(self, min_confidence: float = 0.6) -> List[Dict[str, Any]]:
        ...
        for symbol, summary in self.sentiment_summaries.items():
            ...
            if (summary.overall_sentiment > 0.5 and 
                summary.confidence > min_confidence and
            ...
            elif (summary.overall_sentiment < -0.5 and 
                  summary.confidence > min_confidence and
