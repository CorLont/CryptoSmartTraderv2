...
def test_system_monitor():
    ...
    try:
        ...
        try:
            ...
            if services:
                print(
                    f"   ✅ PASSED: Service availability check completed ({len(services)} services)"
                )
            ...
            if report.timestamp:
                ...
                if report.alerts:
                    threshold_related_alerts = [
                        alert for alert in report.alerts if "threshold" in alert.lower()
                    ]
        ...
        custom_config = {
            "alert_thresholds": {"cpu_percent": 90.0, "memory_percent": 95.0},
            "services": {
                "custom_service": {
                    "name": "Custom Service",
                    "host": "localhost",
        ...
        print(
            "- Threshold cleanup: Unused 'response_time' threshold removed, active thresholds validated"
        )
        print(
            "- Cross-platform compatibility: Load average disabled on Windows, graceful fallbacks"
        )
