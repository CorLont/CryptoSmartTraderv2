...
def example_market_data():
    ...
    for prediction in predictions['predictions'][:3]:
        ...
        print(f"   {confidence_emoji} {prediction['symbol']}: {prediction['direction']} "
...
def example_portfolio_management():
    ...
    for position in positions['positions'][:5]:
        ...
        print(f"   {pnl_emoji} {position['symbol']}: {position['quantity']} "
...
def example_trading_operations():
    ...
    for order in orders['orders']:
        ...
        print(f"   {status_emoji} {order['symbol']} {order['side']}: "
...
def example_security_operations():
    ...
    for event in audit_events['events']:
        ...
        print(f"   {event_emoji} {event['event_type']}: {event['user_id']} "
              f"({event['timestamp']})")
...
async def example_async_operations():
    ...
    market_tasks = [
        client.get(f"/api/v1/market/pair/{symbol}") 
    ...
    for symbol, data in zip(symbols, market_results):
        ...
        print(f"   {change_emoji} {symbol}: ${data['price']:,.2f} "
