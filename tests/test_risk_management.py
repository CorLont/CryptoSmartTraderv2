...
from src.cryptosmarttrader.risk.risk_guard import (
    RiskGuard, RiskLevel, TradingMode, RiskLimits, RiskMonitor
)
...
class TestRiskGuard:
    ...
    def test_position_size_limits(self, risk_guard):
        ...
        large_position_dict = {f'COIN{i}-USD': 0.01 for i in range(60)}  # 60 positions when limit is 50
        risk_guard.update_portfolio_state(
            equity=100000.0,
            positions=large_position_dict
        )
        assert risk_guard.current_risk_level == RiskLevel.WARNING
    ...
    def test_risk_status_reporting(self, risk_guard, mock_portfolio_state):
        ...
        required_fields = [
            'risk_level', 'trading_mode', 'kill_switch_active',
            'daily_pnl', 'total_drawdown', 'current_equity',
            'position_count', 'last_update', 'limits', 'recent_events'
        ]
...
class TestRiskScenarios:
    ...
    def test_data_feed_failure_scenario(self, risk_system):
        ...
        for _ in range(20):
            risk_guard.update_data_quality(datetime.now(), False, 10000.0)  # All failures, high latency
...
def simulate_trading_day(risk_guard: RiskGuard, scenarios: list):
    ...
    for hour, scenario in enumerate(scenarios):
        ...
        # Log hourly status
        status = risk_guard.get_risk_status()
        print(f"Hour {hour}: Risk Level {status['risk_level']}, "
