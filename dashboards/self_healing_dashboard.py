...
class SelfHealingDashboard:
    ...
    def _render_system_overview_tab(self):
        ...
        with col3:
            ...
            st.metric("Disabled Components", disabled_count, delta="⚠️" if disabled_count > 0 else "✅")
        
        with col4:
            recent_alerts = len([a for a in status.get('recent_alerts', []) 
        ...
        if performance_summary:
            ...
            for comp_name, comp_data in performance_summary.items():
                ...
                components_data.append({
                    ...
                    'Failures': failures,
                    'Last Disabled': comp_data.get('last_disabled', 'Never')[:19] if comp_data.get('last_disabled') else 'Never',
                    'Reason': comp_data.get('disable_reason', '').replace('_', ' ').title() if comp_data.get('disable_reason') else 'N/A'
                })
        ...
        if black_swan_count > 0:
            ...
            risk_level = "HIGH" if black_swan_count > 3 else "MEDIUM" if black_swan_count > 1 else "LOW"
            risk_color = "red" if risk_level == "HIGH" else "orange" if risk_level == "MEDIUM" else "green"
            
            st.markdown(f"**Current Risk Level:** :{risk_color}[{risk_level}] ({black_swan_count} indicators)")
            
            if black_swan_count > 2:
                st.warning("⚠️ Multiple black swan indicators detected. System is in protective mode.")
    ...
    def _render_alerts_tab(self):
        ...
        if alert_level_filter != "All":
            filtered_alerts = [a for a in recent_alerts if a.get('level', '').lower() == alert_level_filter.lower()]
        else:
    ...
    def _render_performance_tab(self):
        ...
        if recent_metrics:
            ...
            if not metrics_df.empty:
                ...
                st.dataframe(
                    display_df[['timestamp', 'metric_name', 'value', 'threshold', 'is_anomaly']].rename(columns={
                        'timestamp': 'Time',
                        'metric_name': 'Metric',
                        'value': 'Value',
                        'threshold': 'Threshold',
        ...
        with col2:
            metric_value = st.number_input("Value", min_value=0.0, max_value=1.0, value=0.8, step=0.01)
        
        with col3:
            metric_threshold = st.number_input("Threshold", min_value=0.0, max_value=1.0, value=0.6, step=0.01)
    ...
    def _render_component_control_tab(self):
        ...
        for idx, (comp_name, comp_data) in enumerate(performance_summary.items()):
            with cols[idx % 3]:
                ...
                with st.container():
                    ...
                    st.metric("Performance", f"{performance_score:.1f}%", delta=f"{failures} failures")
                    
                    # Control buttons
                    button_col1, button_col2 = st.columns(2)
                    
                    with button_col1:
                        if st.button(f"{'Disable' if enabled else 'Enable'}", key=f"toggle_{comp_name}"):
                            if enabled:
        ...
        with sim_col2:
            ...
            if st.button("🔒 Simulate Security Alert", use_container_width=True):
                report_security_alert("api_anomaly", AlertLevel.CRITICAL, "Simulated API security breach")
    ...
    def _render_configuration_tab(self):
        ...
        with st.expander("🔧 Auto-Disable Settings"):
            ...
            with settings_cols[0]:
                st.metric("Failure Threshold", auto_settings.get("consecutive_failure_threshold", 3))
                st.metric("Performance Degradation", f"{auto_settings.get('performance_degradation_threshold', 0.3):.1%}")
        ...
        with st.expander("🌪️ Black Swan Detection"):
            ...
            with bs_cols[1]:
                ...
                st.metric("Correlation Breakdown", f"{black_swan.get('correlation_breakdown', 0.3):.1f}")
        
        # Configuration editing
        st.subheader("✏️ Edit Configuration")
        
        st.info("Configuration editing will be available in the full system. Current values are displayed above for monitoring.")
    ...
    def _plot_system_health_trend(self):
        ...
        try:
            ...
            np.random.seed(42)
            health_trend = 100 - np.random.exponential(2, len(dates))  # Mostly high, occasional drops
