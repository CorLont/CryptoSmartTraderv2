...
class EnhancedProbabilityCalibrator:
    ...
    def fit_calibration(self,
                       ...
                       validation_split: float = 0.3) -> Dict[str, float]:
    ...
    def _calculate_reliability_at_confidence(self,
                                           probabilities: np.ndarray,
                                           true_labels: np.ndarray,
...
class UncertaintyQuantifier:
    ...
    def calculate_mc_dropout_uncertainty(self,
                                       x: torch.Tensor,
    ...
    def calculate_prediction_variance(self,
                                    ...
                                    window_size: int = 50) -> torch.Tensor:
...
class EnhancedConfidenceGate:
    ...
    def calibrate_system(self,
                        ...
                        true_outcomes: np.ndarray) -> Dict[str, float]:
        """Calibrate the confidence gate system"""

        # Fit probability calibration
        calibration_results = self.calibrator.fit_calibration(
            model_predictions, true_outcomes
        )
    ...
    def apply_gate(self,
        ...
        gate_stats = {
            ...
            'confidence_passed_mean': calibrated_probs[passed_indices].mean() if len(passed_indices) > 0 else 0,
...
def create_calibrated_confidence_system(model: nn.Module,
                                      training_data: Dict[str, torch.Tensor]) -> EnhancedConfidenceGate:
...
if __name__ == "__main__":
    ...
    print(f"   Passed: {len(gate_results['passed_gate'])}/{gate_results['gate_stats']['total_candidates']}")
