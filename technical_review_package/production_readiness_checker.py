...
class ProductionReadinessChecker:
    ...
    def run_comprehensive_check(self) -> Dict[str, Any]:
        ...
        overall_results["production_ready"] = (
            confidence_score >= 85 and len(self.critical_failures) == 0
        )
    ...
    def _check_ml_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result and result.get("confidence", 0) > 0:
                results.append(
                    f"✅ ML Predictions: {result['confidence']:.0%} confidence generated"
                )
                score += 20
            else:
            ...
            results.append(
                f"✅ Regime Detection: {regime.value if hasattr(regime, 'value') else regime}"
            )
            score += 20
    ...
    def _check_performance_metrics(self) -> Dict[str, Any]:
        ...
        if python_version.major == 3 and python_version.minor >= 11:
            ...
            results.append(
                f"⚠️ Python: {python_version.major}.{python_version.minor} (may have issues)"
            )
            score += 5
        ...
        if imports_ok >= 3:
            results.append(
                f"✅ Dependencies: {imports_ok}/{len(critical_imports)} critical modules available"
            )
        else:
            results.append(
                f"❌ Dependencies: Only {imports_ok}/{len(critical_imports)} modules available"
            )
