...
class PerformanceDashboard:
    ...
    def _render_error_monitoring(self):
        ...
        if strategies:
            for strategy in strategies:
                st.write(f"✅ {strategy.__name__ if hasattr(strategy, '__name__') else str(strategy)}")
        else:
    ...
    def _render_rate_limiting(self):
        ...
        selected_key = st.selectbox(
            "Selecteer service:",
            options=list(all_status.keys()),
            key="rate_limit_select"
        )
