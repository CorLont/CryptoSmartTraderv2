...
async def test_parallel_scraping():
    ...
    orchestrator = await get_scraping_orchestrator(no_fallback_mode=False)  # Allow fallback for testing
    ...
    try:
        ...
        print(f"   Execution time: {execution_time:.2f}s ({'✅ PASS' if execution_time < 60 else '❌ FAIL'})")
        ...
        for symbol, result in results.items():
            ...
            print(f"   {symbol}: {result.total_results} results, "
        ...
        print(f"   {'✅' if parallel_pass else '❌'} 10 parallel sources completed < 60s: {execution_time:.2f}s")
        if parallel_pass:
            criteria_met += 1
        
        # 2. Daily logs contain scraping reports
        logs_pass = len(scraping_reports) > 0 and any("scraping_report" in f.name for f in daily_log_dir.glob("*.json"))
