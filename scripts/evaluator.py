...
def comprehensive_evaluation(
    ...
    logger.info(f"Evaluation completed: P@{precision_k}={metrics['metrics']['precision_at_k']:.3f}, "
...
def print_evaluation_summary(results: Dict[str, Any]) -> None:
    ...
    print(f"   {hr_icon} Hit Rate (≥{results['confidence_threshold']:.0%}): {hit_rate_val:.1%} (target: ≥55%)")
    ...
    print(f"   Positive predictions: {quality['positive_predictions']:,} ({quality['positive_predictions']/results['sample_size']:.1%})")
    print(f"   Positive outcomes: {quality['positive_outcomes']:,} ({quality['positive_outcomes']/results['sample_size']:.1%})")
    print()
...
Examples:
  ...
  python scripts/evaluator.py --predictions pred.csv --prices prices.csv --horizon 168 --confidence 0.85
    ...
    parser.add_argument(
        '--prices',
        required=True,
        help='Path to prices CSV file (hourly OHLC data)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='K value for Precision@K metric (default: 5)'
    )
