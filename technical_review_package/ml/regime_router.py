...
class MarketRegimeRouter:
    ...
    def extract_regime_features(self, price_data: pd.DataFrame) -> pd.DataFrame:
        ...
        price_data["trend_strength"] = (price_data["close"] - price_data["sma_20"]) / price_data[
            "sma_20"
        ]

        # Mean reversion indicator
        price_data["mean_reversion"] = (price_data["close"] - price_data["sma_20"]) / price_data[
            "volatility_20d"
        ]
    ...
    def fit_regime_detector(self, features: pd.DataFrame) -> None:
        ...
        self.regime_model = GaussianMixture(
            n_components=self.n_regimes, covariance_type="full", random_state=42, max_iter=200
        )
    ...
    def predict_regime(self, current_features: pd.DataFrame) -> dict:
        ...
        return {
            ...
            "probabilities": {self.regime_names[i]: prob for i, prob in enumerate(latest_probs)},
        }

    def train_regime_experts(
        self, features: pd.DataFrame, targets: pd.DataFrame, regimes: np.ndarray
    ) -> None:
        ...
        for regime_id in range(self.n_regimes):
            ...
            if regime_mask.sum() < 50:  # Need minimum samples
                logger.warning(
                    f"Insufficient data for regime {regime_id}: {regime_mask.sum()} samples"
                )
                continue
            ...
            logger.info(
                f"Trained expert for regime {self.regime_names[regime_id]} on {len(regime_features)} samples"
            )
...
def create_regime_aware_predictions(
    price_data: pd.DataFrame, target_data: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for i in range(len(regime_features)):
        ...
        try:
            ...
            results.append(
                {
                    ...
                    if hasattr(price_data.index, "__getitem__")
                    else i,
            ...
            logger.warning(f"Error in regime_router.py: {e}")
            continue
