...
class VolatilityTargetingKelly:
    ...
    def __init__(self,
        ...
        self.logger.info("VolatilityTargetingKelly initialized",
    ...
    def calculate_position_size(self,
                              ...
                              expected_return: float,
                              current_price: float,
                              portfolio_value: float) -> SizingResult:
        """
        ...
        with self._lock:
            try:
                ...
                self.logger.error("Position sizing calculation failed",
                                symbol=symbol,
                                error=str(e),
    ...
    def _calculate_kelly_fraction(self,
                                ...
                                signal_confidence: float) -> Dict[str, float]:
    ...
    def _apply_volatility_targeting(self,
                                  symbol: str,
                                  asset_metrics: AssetMetrics,
                                  base_size: float) -> Dict[str, float]:
    ...
    def _apply_caps_and_limits(self,
                             symbol: str,
                             base_size: float,
        ...
        if cluster_id and cluster_id in self.cluster_metrics:
            ...
            if total_cluster_exposure > max_cluster_value:
                ...
                if available_cluster_space > 0:
                    final_size = (available_cluster_space / asset_metrics.current_price) * np.sign(final_size)
                else:
                    final_size = 0.0
        ...
        if position_leverage > self.max_leverage:
            final_size = (portfolio_value * self.max_leverage / asset_metrics.current_price) * np.sign(final_size)
    ...
    def _update_cluster_metrics(self) -> None:
        ...
        for cluster_id, symbols in cluster_groups.items():
            ...
            cluster_correlations = cluster_corr.values[np.triu_indices_from(cluster_corr.values, k=1)]
    ...
    def get_sizing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'caps_applied_pct': np.mean([r.asset_cap_applied or r.cluster_cap_applied for r in recent_results]) * 100,
            'target_volatility': self.target_volatility,
            'kelly_fraction': self.kelly_fraction,
            'max_asset_exposure_pct': self.max_asset_exposure_pct,
            'max_cluster_exposure_pct': self.max_cluster_exposure_pct,
