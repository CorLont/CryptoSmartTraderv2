...
class OrderIdempotencyTester:
    ...
    async def test_duplicate_fill_prevention(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            'success': (execution_result.was_successful and 
                       no_duplicate_exchange_orders and 
                       submission_attempts <= execution_result.total_attempts),
            'details': {
                'submission_attempts': submission_attempts,
                'execution_attempts': execution_result.total_attempts,
                'exchange_order_id': execution_result.exchange_order_id,
                'final_status': execution_result.final_status.value,
                'order_status_in_engine': order_status.status.value if order_status else 'not_found'
            }
        }
    ...
    async def test_retry_idempotency(self) -> Dict[str, Any]:
        ...
        # Validate results
        all_orders_unique = len(set(r.exchange_order_id for r in execution_results if r.exchange_order_id)) == len([r for r in execution_results if r.exchange_order_id])
    ...
    async def test_concurrent_execution(self) -> Dict[str, Any]:
        ...
        orders = [
            self.create_test_order(f"PAIR{i}/USD", i + 1.0)
            for i in range(10)
        ]
        ...
        no_id_collisions = len(unique_exchange_ids) == len([r for r in successful_results if r.exchange_order_id])
    ...
    async def run_comprehensive_test_suite(self) -> Dict[str, Any]:
        ...
        suite_result = {
            ...
            'validation_summary': {
                'order_deduplication_working': any(r.get('test_name') == 'basic_deduplication' and r.get('success') for r in test_results),
                'timeout_recovery_working': any(r.get('test_name') == 'timeout_recovery' and r.get('success') for r in test_results),
                'duplicate_prevention_working': any(r.get('test_name') == 'duplicate_fill_prevention' and r.get('success') for r in test_results),
                'retry_idempotency_working': any(r.get('test_name') == 'retry_idempotency' and r.get('success') for r in test_results),
                'concurrent_safety_working': any(r.get('test_name') == 'concurrent_execution' and r.get('success') for r in test_results)
            }
        }
