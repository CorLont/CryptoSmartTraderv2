...
async def demonstrate_backtest_live_parity():
    ...
    print(
        f"      Tracking Error: {abs(backtest_data['return'] - live_data['return']) * 10000:.1f} bps"
    )
    ...
    for day in range(7):
        ...
        print(
            f"      Day {day + 1}: {daily_metrics.status.value} ({daily_metrics.tracking_error_bps:.1f} bps)"
        )
    ...
    if daily_report["drift_alerts"]:
        ...
        for alert in daily_report["drift_alerts"]:
            print(
                f"      • {alert['type']}: {alert['magnitude']:.2f} magnitude ({alert['confidence']:.1%} confidence)"
            )
    ...
    for i in range(3):
        ...
        print(
            f"         Order {i + 1}: Queue Position {queue_result.queue_position}, "
    ...
    latest_parity = (
        parity_analyzer.parity_history[-1] if parity_analyzer.parity_history else parity_metrics
    )
    ...
    te_grade = (
        "A"
        if latest_parity.tracking_error_bps < 10
        else "B"
        if latest_parity.tracking_error_bps < 20
        else "C"
    )
    corr_grade = (
        "A" if latest_parity.correlation > 0.8 else "B" if latest_parity.correlation > 0.6 else "C"
    )
    hit_grade = (
        "A" if latest_parity.hit_rate > 0.7 else "B" if latest_parity.hit_rate > 0.5 else "C"
    )
    conf_grade = (
        "A"
        if latest_parity.confidence_score > 0.8
        else "B"
        if latest_parity.confidence_score > 0.6
        else "C"
    )
    ...
    print(
        f"   ✅ Component attribution analysis across {len(latest_parity.component_attribution)} components"
    )
