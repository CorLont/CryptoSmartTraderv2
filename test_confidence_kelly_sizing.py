...
def generate_test_data(n_samples: int = 200) -> Dict[str, np.ndarray]:
    ...
    loss_returns = -np.random.lognormal(mean=np.log(1.5), sigma=0.3, size=n_samples)  # ~1.5% avg loss
...
def test_probability_calibration():
    ...
    print(f"   Confidence-Outcome Correlation: {np.corrcoef(data['raw_confidences'], data['outcomes'])[0,1]:.3f}")
    ...
    if "error" not in calibration_results:
        ...
        print(f"   Calibration Improvement: {abs(np.mean(calibrated_probs) - np.mean(test_outcomes)) < abs(np.mean(test_confidences) - np.mean(test_outcomes))}")
...
def test_kelly_sizing():
    ...
    for mode in modes:
        ...
        for scenario in test_scenarios:
            ...
            print(f"   {scenario['name']}: {result['position_size']:.1f}% "
...
def test_confidence_weighted_sizing():
    ...
    if "error" not in calibration_results:
        ...
        for i, result in enumerate(sizing_results[:5]):
            print(f"   Signal {i+1}: {result.position_size:.1f}% "
...
def test_kelly_optimization():
    ...
    if "error" not in optimization_results:
        ...
        for fraction, metrics in optimization_results['all_results'].items():
            print(f"   f={fraction:.2f}: Sharpe={metrics['sharpe_ratio']:.3f}, "
