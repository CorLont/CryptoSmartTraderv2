...
def strict_toplist(
    ...
    if df.empty:
        logger.warning(
            f"NO COINS meet confidence threshold {min_conf:.1%} - returning empty result"
        )
...
def multi_horizon_strict_filter(
    pred_df: pd.DataFrame, min_conf: float = 0.80, horizons: Optional[Dict[str, str]] = None
...
def format_strict_output(df: pd.DataFrame, top_n: int = 10) -> str:
    ...
    for i, row in df.head(top_n).iterrows():
        ...
        output.append(
            f"{i + 1:2d}. {coin:<12} | Return: {pred_return:+6.1%} | Conf: {confidence:.1%}"
        )
...
def save_strict_results(
    results: Dict[str, pd.DataFrame], output_dir: str = "data/strict_filter"
) -> None:
...
Examples:
  ...
  python scripts/strict_filter.py --input predictions.csv --horizon 24h
    ...
    parser.add_argument(
        "--top", type=int, default=10, help="Number of top recommendations to display (default: 10)"
    )

    parser.add_argument(
        "--output", help="Output directory for results (default: data/strict_filter)"
    )
