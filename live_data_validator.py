...
class LiveDataValidator:
    ...
    def __init__(self):
        ...
        if not self.api_key or not self.secret:
            raise ValueError(
                "KRAKEN_API_KEY and KRAKEN_SECRET must be set for live data validation"
            )
    ...
    def generate_quality_report(
        self, validation_results: Dict[str, DataQualityMetrics]
    ) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print(
            f"Zero-Tolerance Policy: {'✅ PASSED' if report['zero_tolerance_passed'] else '❌ FAILED'}"
        )
