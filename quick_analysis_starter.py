...
def load_and_analyze():
    ...
    if len(high_conf) > 0:
        ...
        strong_buys = high_conf[
            (high_conf["expected_return_pct"] > 5) & (high_conf["max_confidence"] > 0.85)
        ...
        for idx, row in strong_buys.head(10).iterrows():
            ...
            print(
                f"  {whale_indicator} {row['coin']:<8} | Return: {row['expected_return_pct']:>6.2f}% | Conf: {row['max_confidence']:.3f} | Sentiment: {row['sentiment_label']}"
            )
    ...
    if len(whale_coins) > 0:
        ...
        for idx, row in whale_sorted.head(5).iterrows():
            print(
                f"  {row['coin']:<8} | Whale Score: {row['whale_score']:>6.2f} | Return: {row['expected_return_pct']:>6.2f}%"
            )
    ...
    if len(strong_buys) > 0:
        ...
        print(
            f"🥇 Top Pick: {top_pick['coin']} - {top_pick['expected_return_pct']:.2f}% return (confidence: {top_pick['max_confidence']:.3f})"
        )

    if len(whale_coins) > 0:
        whale_pick = whale_coins.sort_values("expected_return_pct", ascending=False).iloc[0]
        print(
            f"🐋 Whale Pick: {whale_pick['coin']} - {whale_pick['expected_return_pct']:.2f}% return (whale score: {whale_pick['whale_score']:.2f})"
        )
    ...
    summary = {
        ...
        "top_recommendation": {
            ...
            "expected_return": strong_buys.iloc[0]["expected_return_pct"]
            if len(strong_buys) > 0
            else None,
