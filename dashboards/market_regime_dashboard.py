...
class MarketRegimeDashboard:
    ...
    def _render_sidebar(self):
        ...
        selected_period = st.sidebar.selectbox(
            "Analysis Period", options=list(data_periods.keys()), index=1
        )
        ...
        selected_method = st.sidebar.selectbox(
            "Detection Method", options=detection_methods, index=0
        )
        ...
        selected_strategy = st.sidebar.selectbox(
            "Adaptation Strategy", options=adaptation_strategies, index=1
        )
    ...
    def _render_regime_detection_tab(self):
        ...
        with col1:
            ...
            if data is not None and len(data) > 50:
                ...
                price_change = (
                    data["close"].pct_change().iloc[-1] * 100 if "close" in data.columns else 0
                )
                volatility = (
                    data["close"].pct_change().std() * 100 if "close" in data.columns else 0
                )
                            ...
                            with result_col2:
                                ...
                                if regime_result.regime_probability:
                                    ...
                                    regime_names = [
                                        ...
                                        for r in regime_result.regime_probability.keys()
                                    ]
                        ...
                        try:
                            ...
                            if result.get("success"):
                                ...
                                with adaptation_col1:
                                    st.write(
                                        f"**Regime:** {result['regime'].replace('_', ' ').title()}"
                                    )
                                    st.write(
                                        f"**Model Type:** {result['model_type'].replace('_', ' ').title()}"
                                    )
                                ...
                                st.error(
                                    f"❌ Adaptation failed: {result.get('error', 'Unknown error')}"
                                )
            ...
            if st.button("📊 Generate Trading Signals", use_container_width=True):
                if data is not None and len(data) > 20:
                    with st.spinner("Generating trading signals..."):
                        try:
                            enhanced_data = self._enhance_data_for_adaptation(data)
                            signals = self.adaptive_switcher.get_current_signals(
                                enhanced_data, "close"
                            )
        ...
        if data is not None and len(data) > 100:
            ...
            if st.button("📈 Evaluate Current Performance", use_container_width=True):
                with st.spinner("Evaluating model performance..."):
                    try:
                        enhanced_data = self._enhance_data_for_adaptation(data)
                        metrics = self.adaptive_switcher.evaluate_current_performance(
                            enhanced_data, "close"
                        )
                        ...
                        if metrics:
                            ...
                            st.warning(
                                "No performance metrics available. Model may not be trained."
                            )
        ...
        for regime, mapping in regime_mappings.items():
            with st.expander(f"🌍 {regime.value.replace('_', ' ').title()} Configuration"):
                ...
                with config_col1:
                    ...
                    st.write(
                        f"- Type: {mapping.primary_model.model_type.value.replace('_', ' ').title()}"
                    )
    ...
    def _render_realtime_monitoring_tab(self):
        ...
        with col1:
            ...
            if data is not None and len(data) > 50:
                try:
                    ...
                    st.write(
                        f"**Last Updated:** {regime_result.detection_timestamp.strftime('%H:%M:%S')}"
                    )
        ...
        with col3:
            ...
            health_color = (
                "green" if health_score > 0.7 else "orange" if health_score > 0.3 else "red"
            )
            ...
            st.markdown(
                f"**Status:** <span style='color:{health_color}'>{'🟢 Healthy' if health_score > 0.7 else '🟡 Partial' if health_score > 0.3 else '🔴 Issues'}</span>",
                unsafe_allow_html=True,
            )
        ...
        try:
            ...
            if data is not None and len(data) > 50:
                with st.spinner("Detecting current market regime..."):
                    regime_result = detect_current_regime(data, "close")
                    st.success(
                        f"Current regime: {regime_result.regime.value.replace('_', ' ').title()}"
                    )
                    st.rerun()
            else:
        ...
        try:
            ...
            if data is not None and len(data) > 50:
                with st.spinner("Forcing model adaptation..."):
                    ...
                    if result.get("success"):
                        st.success(
                            f"Adaptation successful: {result['model_type']} model for {result['regime']} regime"
                        )
                    else:
