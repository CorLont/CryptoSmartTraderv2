...
class TestAutomatedFeatureEngineering:
    ...
    def test_cross_feature_generation(self, sample_data):
        ...
        cross_features = engineer.synthesizer.generate_cross_features(sample_data, target_column)

        assert len(cross_features.columns) > len(sample_data.columns)

        # Check for interaction features
        interaction_features = [
            col for col in cross_features.columns if "_x_" in col or "_div_" in col
        ]
    ...
    def test_polynomial_features(self, sample_data):
        ...
        transform_cols = [
            ...
            if any(t in col for t in ["log", "sqrt", "reciprocal"])
        ]
    ...
    def test_feature_importance_analysis(self, sample_data):
        ...
        importance_results = engineer.shap_analyzer.analyze_feature_importance(features, target)
    ...
    def test_shap_regime_analyzer(self, sample_data):
        ...
        results = analyzer.analyze_regime_specific_importance(sample_data, target_column, "regime")
    ...
    def test_end_to_end_feature_engineering(self, sample_data):
        ...
        engineered_data = engineer_features_for_coin(sample_data, target_column, "regime")
    ...
    def test_feature_adaptation_triggers(self, sample_data):
        ...
        for trigger in triggers:
            adapted_features = adapter.adapt_features(sample_data, target_column, trigger)
            assert isinstance(adapted_features, list)
    ...
    def test_memory_management(self, sample_data):
        ...
        config = FeatureEngineeringConfig(max_features_per_iteration=5, max_total_features=20)
        engineer = AutomatedFeatureEngineer(config)
...
if __name__ == "__main__":
    ...
    try:
        ...
        engineered_data = engineer.fit(sample_data, "price").transform(sample_data, "price")
        print(
            f"✓ Generated {len(engineered_data.columns)} features from {len(sample_data.columns)} original features"
        )
        ...
        adapter = get_live_adaptation_engine()
        adapted_features = adapter.adapt_features(
            sample_data, "price", AdaptationTrigger.REGIME_CHANGE
        )
