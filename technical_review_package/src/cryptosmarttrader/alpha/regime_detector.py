...
class RegimeDetector:
    ...
    def detect_regime(
        self, price_data: pd.Series, volume_data: Optional[pd.Series] = None
    ) -> RegimeSignal:
        """
        ...
        if len(price_data) < self.lookback_periods:
            raise ValueError(
                f"Insufficient data: need {self.lookback_periods}, got {len(price_data)}"
            )
        ...
        if volume_data is not None:
            breadth = self._calculate_market_breadth(
                price_data.iloc[-self.lookback_periods :],
    ...
    def _calculate_momentum_indicators(self, price_data: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            return {"momentum_10d": 0.0, "momentum_20d": 0.0, "rsi": 50.0, "macd_signal": 0.0}

    def _calculate_market_breadth(
        self, price_data: pd.Series, volume_data: pd.Series
    ) -> Dict[str, float]:
        ...
        try:
            ...
            return {"vwap_momentum": 0.0, "volume_trend": 0.0, "price_volume_divergence": 5.0}
...
def create_regime_detector(
    lookback_periods: int = 100, min_confidence: float = 0.6
) -> RegimeDetector:
