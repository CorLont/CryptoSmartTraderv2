...
class ListingDetectionAgent:
    ...
    def __init__(self, config_manager=None):
        ...
        self.exchange_announcement_sources = {
            ...
            ExchangeSource.KUCOIN: [
                "https://www.kucoin.com/news/en-new-listings"
            ]
        }
        ...
        self.listing_keywords = {
            'confirmed_listing': [
                "will list", "listing", "now supports", "added support for",
                "new trading pair", "trading begins", "deposits open",
                "withdrawals enabled", "trading enabled"
            ],
            'pre_listing': [
                "deposit-only mode", "deposits enabled", "trading coming soon",
                "listing soon", "will support", "under evaluation"
            ],
            'rumor_indicators': [
                "potential listing", "considering", "evaluation",
                "community request", "high demand"
            ]
        }
    ...
    def _initialize_exchange_clients(self):
        ...
        try:
            ...
            self.logger.info(f"Initialized {len(self.exchange_clients)} exchanges for listing detection")
    ...
    def _parse_rss_announcements(self, exchange: ExchangeSource, rss_url: str) -> List[ListingDetection]:
            ...
            for entry in feed.entries[:20]:  # Check latest 20 entries
                ...
                if listing_info:
                    detection = ListingDetection(
                        ...
                        institutional_interest=0.6 if exchange in [ExchangeSource.COINBASE, ExchangeSource.KRAKEN] else 0.4,
    ...
    def _scrape_announcement_page(self, exchange: ExchangeSource, page_url: str) -> List[ListingDetection]:
            ...
            try:
                ...
                for pair in new_pairs:
                    if self._is_relevant_pair(pair):
                        ...
                        listing_type = ListingType.NEW_TOKEN if is_new_token else ListingType.NEW_PAIR
    ...
    def _get_ai_listing_analysis(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert cryptocurrency listing analyst."},
    ...
    def _is_known_token(self, symbol: str) -> bool:
        ...
        known_tokens = {
            ...
            'MATIC', 'LINK', 'UNI', 'AAVE', 'COMP', 'MKR', 'SNX'
        }
    ...
    def _is_new_detection(self, detection: ListingDetection) -> bool:
        ...
        for existing in self.listing_detections:
            if (existing.symbol == detection.symbol and 
                existing.exchange == detection.exchange and
                abs((existing.timestamp - detection.timestamp).total_seconds()) < 3600):  # 1 hour window
                return False
        return True
    ...
    def _analyze_listing_opportunities(self):
        ...
        recent_detections = [
            d for d in self.listing_detections 
            if d.timestamp > datetime.now() - timedelta(hours=24) and
        ...
        for symbol, detections in symbol_detections.items():
            ...
            if opportunity:
                with self._lock:
                    ...
                    self.logger.info(f"LISTING OPPORTUNITY: {symbol} - {opportunity.expected_return:.1f}% expected return")
    
    def _create_listing_opportunity(self, symbol: str, detections: List[ListingDetection]) -> Optional[ListingOpportunity]:
    ...
    def _cleanup_old_detections(self):
        ...
        with self._lock:
            ...
            self.listing_detections = deque([
                d for d in self.listing_detections if d.timestamp > cutoff_time
            ], maxlen=5000)
    ...
    def get_active_opportunities(self, min_confidence: float = 0.6) -> List[ListingOpportunity]:
        ...
        with self._lock:
            return [
                opp for opp in self.listing_opportunities
                if opp.confidence >= min_confidence and
                opp.optimal_entry_time > datetime.now() - timedelta(hours=48)
            ]
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sources_monitored': sum(len(sources) for sources in self.exchange_announcement_sources.values()),
