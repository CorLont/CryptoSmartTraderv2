...
class ComprehensiveMarketScanner:
                ...
                self.scan_statistics.update(
                    {
                        ...
                        "active_coins_analyzed": len(
                            [c for c in self.coin_metadata.values() if c.get("active", False)]
                        ),
                    }
                )

                self.logger.info(
                    f"Full market scan completed in {scan_duration:.1f}s - {len(self.discovered_coins)} coins analyzed"
                )
    ...
    def _analyze_all_timeframes(self):
        ...
        try:
            ...
            for batch_num in range(total_batches):
                ...
                self.logger.info(
                    f"Processing batch {batch_num + 1}/{total_batches} ({len(batch_coins)} coins)"
                )
            ...
            self.logger.info(
                f"Completed analysis of {len(active_coins)} coins across {len(self.timeframes)} timeframes"
            )
    ...
    def _analyze_coin_batch_parallel(self, coin_batch: List[str]):
        ...
        try:
            with ThreadPoolExecutor(
                max_workers=self.scanning_config["analysis_threads"]
            ) as executor:
    ...
    def _calculate_comprehensive_indicators(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["price_change_24h_pct"] = (
                ((df["close"].iloc[-1] - df["close"].iloc[-24]) / df["close"].iloc[-24]) * 100
                if len(df) >= 24
                else 0
            )
            ...
            if "volume_sma" in gpu_indicators and len(gpu_indicators["volume_sma"]) > 0:
                ...
                analysis["volume_ratio"] = df["volume"].iloc[-1] / max(
                    volume_sma, 1e-10
                )  # Avoid division by zero
            else:
                analysis["volume_sma_10"] = df["volume"].rolling(10).mean().iloc[-1]
                analysis["volume_ratio"] = df["volume"].iloc[-1] / max(
                    analysis["volume_sma_10"], 1e-10
                )

            # Moving averages from GPU calculations
            if gpu_indicators:
                analysis["sma_20"] = (
                    gpu_indicators["sma_20"][-1]
                    if len(gpu_indicators.get("sma_20", [])) > 0
                    else df["close"].rolling(20).mean().iloc[-1]
                )
                analysis["sma_50"] = (
                    gpu_indicators["sma_50"][-1]
                    if len(gpu_indicators.get("sma_50", [])) > 0 and len(df) >= 50
                    else None
                )
                analysis["ema_12"] = (
                    gpu_indicators["ema_12"][-1]
                    if len(gpu_indicators.get("ema_12", [])) > 0
                    else df["close"].ewm(span=12).mean().iloc[-1]
                )
                analysis["ema_26"] = (
                    gpu_indicators["ema_26"][-1]
                    if len(gpu_indicators.get("ema_26", [])) > 0
                    else df["close"].ewm(span=26).mean().iloc[-1]
                )
            else:
            ...
            if len(recent_closes) > 1:
                ...
                analysis["trend_strength"] = (
                    np.corrcoef(time_series, recent_closes)[0, 1]
                    if len(time_series) == len(recent_closes)
                    else 0.0
                )
    ...
    def _evaluate_trading_opportunity(self, symbol: str, timeframe: str, analysis: Dict[str, Any]):
        ...
        try:
            ...
            if opportunity_score >= 3:
                ...
                self.logger.info(
                    f"Trading opportunity detected: {symbol} {timeframe} (score: {opportunity_score})"
                )
    ...
    def get_comprehensive_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis["timeframe_analysis"]:
                analysis["aggregated_signals"] = self._aggregate_signals(
            ...
            analysis["opportunities"] = [
                opp for opp in opportunities if opp.get("symbol") == symbol
            ]
