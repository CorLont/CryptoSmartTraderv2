...
def create_synthetic_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing_features:
        ...
        logger.warning(
            "Using zero defaults for missing features - may impact synthetic target quality"
        )
...
def validate_synthetic_targets(df: pd.DataFrame) -> bool:
    ...
    expected_targets = []
    for horizon in ["1h", "24h", "168h", "720h"]:
        expected_targets.extend([f"target_return_{horizon}", f"target_direction_{horizon}"])
