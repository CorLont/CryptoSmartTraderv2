...
class NewsSpeedAgent:
    ...
    def __init__(self, config_manager=None):
        ...
        self.critical_keywords = [
            ...
            'regulation', 'ban', 'illegal', 'investigation',
            'sec', 'cftc', 'fed', 'central bank',
            'adoption', 'partnership', 'integration',
            'listing', 'delisting', 'exchange'
        ]
        
        self.bullish_keywords = [
            'adoption', 'partnership', 'integration', 'investment',
            'bullish', 'positive', 'growth', 'surge', 'rally',
            'breakthrough', 'milestone', 'approval', 'green light'
        ]
        
        self.bearish_keywords = [
            'hack', 'exploit', 'crash', 'plunge', 'dump',
            'bearish', 'negative', 'decline', 'fall', 'drop',
            'ban', 'illegal', 'investigation', 'concern'
        ]
    ...
    def start(self):
        ...
        if not self.active:
            self.active = True
            self.monitoring_thread = threading.Thread(target=self._start_async_monitoring, daemon=True)
                ...
                for source, config in self.news_sources.items():
                    if config.get('rss_url'):
                        task = asyncio.create_task(
                            self._monitor_rss_feed(source, config)
                        )
    ...
    async def _process_rss_entry(self, source: NewsSource, entry: Any, config: Dict):
        ...
        try:
            ...
            sentiment_score, sentiment_confidence = self._fast_sentiment_analysis(title + ' ' + content)
    ...
    async def _process_news_queue(self):
        ...
        while self.active:
            try:
                ...
                news_event = await asyncio.wait_for(
                    self.news_queue.get(), 
                    timeout=1.0
                )
        ...
        while self.active:
            try:
                ...
                with self._lock:
                    ...
                    old_events = [
                        event_id for event_id, event in self.processed_news.items()
    ...
    def get_speed_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if recent_signals:
                avg_generation_time = sum(s.generated_in_ms for s in recent_signals) / len(recent_signals)
