...
class HumanInLoopDashboard:
    ...
    def render(self):
        ...
        st.markdown("Active learning, feedback collection, and model improvement through human expertise")
            ...
            try:
                ...
                with st.expander("🔍 Generated Insights"):
                    ...
                    with col2:
                        ...
                        st.metric("Confidence Gap", f"{insights.get('confidence_calibration', {}).get('confidence_gap', 0):.1%}")
    ...
    def _render_active_learning(self):
        ...
        **Active Learning Process:**
        ...
        for i, query in enumerate(queries):
            with st.expander(f"Query {i+1}: {query['question']} (Priority: {query['priority']}/10)"):
        ...
        if not queries:
            st.info("🎉 No pending queries at the moment. The system is confident in current predictions!")
    ...
    def _render_feedback_analytics(self):
        ...
        with col3:
            recent_good = (feedback_summary["quality_trends"]["Excellent"] + 
        ...
        timeline_df = pd.DataFrame({
            'Date': dates,
            'Feedback Count': daily_feedback,
            'Avg Quality Score': quality_scores
        })
    ...
    def _render_calibration_metrics(self):
        ...
        **Calibration Analysis:**
        ...
        cal_df = pd.DataFrame({
            ...
            'Calibration Error': [f"{abs(c-a):.3f}" for c, a in zip(model_confidence, actual_accuracy)]
        })
