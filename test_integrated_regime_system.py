...
def test_integrated_regime_system():
    ...
    try:
        ...
        system = create_integrated_confidence_system(
            ...
            regime_adjustments={"bull_market": 0.0, "bear_market": 0.1, "high_volatility": 0.15},
        )
        ...
        regime_predictions = system.predict_with_regime_awareness(
            predictions=predictions, current_market_data=market_data, feature_data=feature_data
        )
        ...
        filtered_predictions = system.filter_regime_confident_predictions(
            regime_predictions, min_regime_confidence=0.6, max_uncertainty=0.4
        )

        pass_rate = len(filtered_predictions) / len(regime_predictions)
        print(
            f"Filtered predictions: {len(filtered_predictions)}/{len(regime_predictions)} ({pass_rate:.1%})"
        )
        ...
        print(
            f"  Fallback calibrator fitted: {calibration.get('fallback_calibrator_fitted', False)}"
        )
        print(
            f"  Uncertainty quantifier available: {calibration.get('uncertainty_quantifier_available', False)}"
        )
