...
class RegimeStrategyTester:
    ...
    def test_regime_detection_accuracy(self) -> bool:
        ...
        try:
            ...
            for regime, stats in regime_accuracies.items():
                ...
                logger.info(
                    f"{regime.value} accuracy: {regime_accuracy:.2%} ({stats['correct']}/{stats['total']})"
                )
            ...
            self.test_results["regime_detection_accuracy"] = {
                ...
                "regime_accuracies": {
                    r.value: s["correct"] / s["total"] for r, s in regime_accuracies.items()
                },
            }
        ...
        try:
            ...
            for i in range(100, len(test_data), 20):
                ...
                if switch_success and current_strategy:
                    ...
                    if (
                        last_strategy
                        and last_regime == classification.regime
                        and current_strategy.regime == last_strategy.regime
                    ):
    ...
    def test_regime_specific_parameters(self) -> bool:
        ...
        try:
            ...
            for regime, params in regime_params.items():
                logger.info(
                    f"{regime}: Size={params['position_size']:.2f}, "
    ...
    def test_performance_tracking(self) -> bool:
        ...
        try:
            ...
            tracked_regimes = sum(
                1 for stats in performance_summary.values() if stats["trades"] > 0
            )
    ...
    def run_comprehensive_tests(self):
        ...
        for test_name, _ in tests:
            result = (
                "✅ PASSED"
                if self.test_results.get(test_name.lower().replace(" ", "_"), {}).get(
                    "success", False
                )
                else "❌ FAILED"
            )
