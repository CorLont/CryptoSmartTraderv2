...
class AutomatedBackupSystem:
    ...
    def restore_from_backup(
        self, session_id: str, categories: List[str] = None, target_directory: str = None
    ) -> Dict[str, Any]:
        """Restore system from backup"""
        try:
            # Find backup manifest
            manifest_file = (
                self.backup_config["backup_root"] / "manifests" / f"manifest_{session_id}.json"
            )
    ...
    def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for category in self.backup_categories:
                ...
                category_stats = {"backup_count": 0, "total_size": 0, "latest_backup": None}
                ...
                for backup_item in category_dir.iterdir():
                    if backup_item.is_file() or backup_item.is_dir():
                        ...
                        if (
                            not category_stats["latest_backup"]
                            or mtime > category_stats["latest_backup"]
                        ):
...
def main():
    ...
    if args.action == "backup":
        ...
        result = backup_system.restore_from_backup(
            args.session_id, args.categories, args.target_dir
        )
        print(json.dumps(result, indent=2, default=str))
