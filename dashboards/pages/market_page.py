...
def render_market_controls():
    ...
    with col3:
        ...
        selected_exchange = st.selectbox("Exchange", exchanges, key="market_exchange_filter")
...
def render_market_metrics():
    ...
    with col1:
        st.metric(
            "Total Market Cap", f"${total_market_cap / 1e9:.2f}B", delta=f"{avg_change_24h:+.2f}%"
        )

    with col2:
        st.metric("24h Volume", f"${total_volume_24h / 1e9:.2f}B", delta=None)
    ...
    with col4:
        st.metric("Gainers", gainers, delta=f"vs {losers} losers")
    ...
    with col5:
        ...
        st.metric("Fear & Greed", f"{fear_greed} ({fear_greed_label})", delta=None)
...
def render_market_table():
    ...
    with col1:
        sort_by = st.selectbox(
            "Sort by", ["market_cap", "price", "change_24h", "volume_24h"], key="table_sort_by"
        )
...
def render_market_analysis():
    ...
    with col1:
        ...
        if not market_data.empty:
            ...
            fig_pie = px.pie(
                market_data, values="market_cap", names="symbol", title="Market Cap Distribution"
            )
...
def generate_market_insights(market_data: pd.DataFrame) -> list:
    ...
    # Volume analysis
    high_volume_count = len(
        market_data[market_data["volume_24h"] > market_data["volume_24h"].median() * 1.5]
    )
