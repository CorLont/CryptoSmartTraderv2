...
class EarlyMoverSystem:
    ...
    def _validate_risk_parameters(self, signal: AlphaSignal) -> bool:
        ...
        if signal.expected_return > 200 and signal.confidence < 0.8:  # 200%+ return needs 80%+ confidence
            return False
        
        # Check time horizon consistency
        if signal.strength == SignalStrength.EXTREME and signal.time_horizon == TimeHorizon.LONG_TERM:
    ...
    def _create_alpha_opportunity(self, symbol: str, signals: List[AlphaSignal]) -> Optional[AlphaOpportunity]:
        ...
        if (opportunity.risk_adjusted_return >= 50 and  # 50%+ expected return
            opportunity.total_confidence >= 0.7 and      # 70%+ confidence
            opportunity.signal_diversity >= 2):          # Multiple confirming sources
            return opportunity
    ...
    def _execute_priority_opportunities(self, opportunities: List[AlphaOpportunity]):
        ...
        sorted_opportunities = sorted(
            opportunities,
            key=lambda x: x.risk_adjusted_return * x.total_confidence,
            reverse=True
        )
        ...
        for opp in sorted_opportunities[:5]:  # Execute top 5 opportunities
            ...
            if self._check_portfolio_constraints(opp):
                ...
                self.logger.info(f"ALPHA OPPORTUNITY: {opp.symbol} - {opp.expected_return:.1f}% expected return")
                self.logger.info(f"Confidence: {opp.total_confidence:.1f}, Kelly size: {opp.kelly_optimal_size:.1%}")
    ...
    def _cleanup_expired_signals(self):
        ...
        with self._lock:
            ...
            expired_opportunities = [
                opp_id for opp_id, opp in self.alpha_opportunities.items()
