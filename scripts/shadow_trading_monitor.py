...
from core.shadow_trading_engine import (
    get_shadow_trading_engine, SoakPeriodConfig, TradingMode, SoakStatus
)
...
async def run_shadow_trading_monitor(interval_minutes: int = 60):
    ...
    while True:
        try:
            ...
            print(f"   Duration: {status['soak_period']['current_duration_days']}/{status['soak_period']['target_duration_days']} days")
            print(f"   Progress: {status['soak_period']['progress_percentage']:.1f}%")
            print(f"   Trades: {status['trading_statistics']['filled_trades']}/{status['trading_statistics']['minimum_required_trades']}")
            print(f"   Portfolio Value: ${status['portfolio_status']['portfolio_value']:,.2f}")
            
            # Show market regimes
            regimes = status['market_exposure']['regimes_encountered']
            if regimes:
                print(f"   Market Regimes: {', '.join(regimes)} ({len(regimes)}/{status['market_exposure']['regimes_required']})")
            
            # Check if ready for validation
            if (status['soak_period']['current_duration_days'] >= shadow_engine.config.minimum_duration_days and
                status['trading_statistics']['filled_trades'] >= shadow_engine.config.minimum_trades):
                ...
                if validation_result.status == SoakStatus.PASSED:
                    ...
                    print(f"   Authorization Date: {validation_result.end_date.strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                days_remaining = max(0, shadow_engine.config.minimum_duration_days - status['soak_period']['current_duration_days'])
                trades_remaining = max(0, shadow_engine.config.minimum_trades - status['trading_statistics']['filled_trades'])
...
async def run_daily_soak_report():
    ...
    print(f"   Live Trading Authorized: {'✅ YES' if status['live_trading_authorized'] else '❌ NO'}")
    print()
    ...
    print(f"   Regimes Encountered: {len(market['regimes_encountered'])}/{market['regimes_required']}")
    ...
    # Recent trades
    recent_trades = [t for t in shadow_engine.shadow_trades if 
                    (datetime.now() - t.entry_time).days <= 7]
    
    if recent_trades:
        print(f"📋 RECENT TRADES (Last 7 days): {len(recent_trades)}")
        for trade in recent_trades[-5:]:  # Show last 5
            print(f"   {trade.entry_time.strftime('%m-%d')} {trade.symbol} {trade.side} "
