    ...
    try:
        ...
        async def run_tests():
            ...
            if macd_result.get("macd") and macd_result["macd"]["uses_ema"]:
                ...
                print(f"   Fast/Slow/Signal periods: {macd_data['fast_period']}/{macd_data['slow_period']}/{macd_data['signal_period']}")
                
                # Verify EMA calculation is different from SMA approximation
                agent_sma = TechnicalAnalysisAgent(config={"indicators": {"macd": {"use_ema": False}}})
            ...
            if rsi_result.get("rsi"):
                ...
                agent_simple = TechnicalAnalysisAgent(config={"indicators": {"rsi": {"use_wilder_smoothing": False}}})
            ...
            if result.symbol == "TEST/USD" and result.indicators:
                ...
                missing_indicators = [ind for ind in expected_indicators if ind not in present_indicators]
            ...
            valid_signals = all(
                signal.signal_type in ['buy', 'sell', 'neutral'] and
                0 <= signal.strength <= 1 and
                signal.indicator in ['rsi', 'macd', 'bollinger', 'sma']
                for signal in signals
            )
        ...
        if success:
            ...
            print("- Data integrity: No synthetic fallback data, proper validation and error handling")
...
if __name__ == "__main__":
    success = asyncio.run(test_ta_agent())
