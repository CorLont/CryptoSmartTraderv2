...
from cryptosmarttrader.execution.backtest_live_parity import (
    ExecutionSimulator,
    BacktestLiveParityTracker,
)
...
class EnterpriseSafetySystemTester:
    ...
    def test_risk_limits_system(self) -> bool:
        ...
        try:
            ...
            allowed, violations = risk_manager.check_order_against_limits(
                "BTC/USD", 5000.0, 100000.0
            )
    ...
    def test_kill_switch_system(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Kill switch test - Trigger: ✅, Recovery: {'✅' if recovery_success else '❌'}"
            )
            return True
    ...
    def test_order_deduplication(self) -> bool:
        ...
        try:
            ...
            order_record = deduplicator.register_order(
                order_id, "BTC/USD", "buy", "limit", 0.1, 50000.0
            )
    ...
    def test_circuit_breaker_system(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Circuit breaker test - State: {status['state']}, Failures: {failure_count}"
            )
            return True
    ...
    def test_environment_manager(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Environment test - Env: {current_env.value}, Scaling: {scaled_size / base_size:.2%}"
            )
            return True
    ...
    def test_prometheus_metrics(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Metrics test - Counters: {summary['counters']}, Gauges: {summary['gauges']}"
            )
            return True
    ...
    def test_execution_simulator(self) -> bool:
        ...
        try:
            ...
            if (
                len(parity_tracker.backtest_executions) > 0
                and len(parity_tracker.live_executions) > 0
            ):
                ...
                if report.get("status") == "insufficient_data":
                    ...
                    logger.info(
                        f"Parity score: {report.get('parity_score', {}).get('overall_score', 0):.1f}"
                    )
            ...
            logger.info(
                f"Execution test - Slippage: {slippage_total:.1f}bps, Fill: {fill_rate:.1f}%"
            )
            return True
    ...
    def test_security_manager(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Security test - Secrets: {status['total_secrets']}, Health: {status['security_health']}"
            )
            return True
    ...
    def test_health_checker(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Health test - Status: {summary['status']}, Components: {summary['total_components']}"
            )
            return True
            ...
            def risk_breach_callback(limit, old_status, new_status):
                if new_status.value == "EMERGENCY":
                    kill_switch.manual_trigger(
                        f"Risk limit breach: {limit.limit_id}", "risk_manager"
                    )
