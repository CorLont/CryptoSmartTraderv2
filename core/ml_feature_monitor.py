...
class FeatureLeakageDetector:
    ...
    def _detect_perfect_correlations(
        self, features_df: pd.DataFrame, target: np.ndarray
    ) -> List[LeakageViolation]:
    ...
    def _detect_lookahead_bias(
        ...
        suspicious_patterns = ["future_", "next_", "lead_", "forward_", "ahead_"]
        ...
        for column in features_df.columns:
            try:
                ...
                self.baseline_stats[column] = {
                    ...
                    "correlation_with_target": np.corrcoef(feature_values, target)[0, 1]
                    if len(target) == len(feature_values)
                    else 0,
...
class SHAPFeatureAnalyzer:
    ...
    def analyze_feature_importance(
        self, model: Any, features_df: pd.DataFrame, background_samples: int = 100
        ...
        try:
            ...
            if hasattr(model, "predict"):
                ...
                background = features_df.sample(
                    min(background_samples, len(features_df)), random_state=42
                )
                ...
                for i, feature_name in enumerate(features_df.columns):
                    feature_shap_values = (
                        shap_values[:, i] if len(shap_values.shape) > 1 else shap_values
                    )
            ...
            self.logger.info(
                ...
                extra={
                    ...
                    "features_to_remove": len(
                        [f for f, r in analysis_results["recommendations"].items() if r == "remove"]
                    ),
                    "features_to_monitor": len(
                        [
                            f
                            for f, r in analysis_results["recommendations"].items()
                            if r == "monitor"
                        ]
                    ),
                },
        ...
        if self.last_analysis:
            summary["recent_issues"] = {
                ...
                "features_needing_attention": len(
                    [
                        f
                        for f, r in self.last_analysis.get("recommendations", {}).items()
