...
class AutomatedFeatureEngineeringDashboard:
    ...
    def _render_sidebar(self):
        ...
        st.session_state.fe_config = FeatureEngineeringConfig(
            max_features_per_iteration=max_features,
    ...
    def _render_feature_engineering_tab(self):
                        ...
                        try:
                            ...
                            if engineered_data is not None:
                                ...
                                if len(engineered_data.columns) <= 20:  # Limit for visualization
                                    corr_matrix = engineered_data.select_dtypes(include=[np.number]).corr()
        ...
        with col2:
            ...
            st.metric("Total Features Generated", importance_summary.get('total_features_generated', 0))
        ...
        if discovery_summary.get('regime_feature_mapping'):
            ...
            if regime_data:
                ...
                fig = px.bar(
                    ...
                    x='Regime',
                    y='Feature Count',
                    title="Feature Count by Market Regime"
                )
                st.plotly_chart(fig, use_container_width=True)
            ...
            if st.button("🧠 Run SHAP Analysis", use_container_width=True):
                with st.spinner("Running SHAP analysis..."):
                    try:
                        ...
                        target_column = 'close' if 'close' in enhanced_data.columns else enhanced_data.columns[0]
                        ...
                        if regime_results:
                            st.success(f"✅ SHAP analysis completed for {len(regime_results)} regimes")
                            ...
                            st.warning("No SHAP results available. This might be due to insufficient data or missing SHAP library.")
    ...
    def _render_live_adaptation_tab(self):
        ...
        stability_color = "green" if regime_stability > 5 else "orange" if regime_stability > 2 else "red"
        st.markdown(f"**Stability Score:** <span style='color:{stability_color}'>{regime_stability}</span>", 
    ...
    def _render_performance_monitor_tab(self):
        ...
        with col2:
            ...
            st.metric("Active Features", discovery_summary.get('active_features', 0))
            st.metric("Discovery Iteration", discovery_summary.get('discovery_iteration', 0))
            st.metric("Avg Performance", f"{discovery_summary.get('avg_feature_performance', 0):.3f}")
        ...
        fig.add_trace(
            go.Scatter(x=performance_data['timestamp'], y=performance_data['feature_count'], name='Features'),
            row=1, col=1
        )
        
        fig.add_trace(
            go.Scatter(x=performance_data['timestamp'], y=performance_data['adaptation_score'], name='Adaptation'),
            row=2, col=1
        )
        
        fig.add_trace(
            go.Scatter(x=performance_data['timestamp'], y=performance_data['discovery_rate'], name='Discovery'),
            row=3, col=1
        )
        ...
        with health_col1:
            ...
            fe_health = "🟢 Healthy" if fe_summary.get('total_features_generated', 0) > 0 else "🔴 Issues"
            st.markdown(f"**Feature Engineering:** {fe_health}")
        
        with health_col2:
            # Discovery health
            discovery_health = "🟢 Active" if discovery_summary.get('active_features', 0) > 0 else "🟡 Idle"
            st.markdown(f"**Feature Discovery:** {discovery_health}")
        
        with health_col3:
            # Adaptation health
            adaptation_health = "🟢 Ready" if adaptation_summary.get('rollback_enabled', False) else "🟡 Limited"
    ...
    def _load_market_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            data = self.data_manager.get_historical_data(
                coin, timeframe, start_time, end_time
            )
        ...
        try:
            ...
            if data is not None:
                with st.spinner("Running feature engineering..."):
                    ...
                    if engineered_data is not None:
                        st.success(f"✅ Feature engineering completed: {len(engineered_data.columns)} features")
                        st.rerun()
                    else:
        ...
        try:
            ...
            if data is not None:
                with st.spinner("Forcing feature adaptation..."):
                    ...
                    st.success(f"✅ Adaptation completed: {len(adapted_features)} features selected")
                    st.rerun()
            else:
        ...
        try:
            ...
            if data is not None:
                ...
                with st.spinner(f"Triggering adaptation: {trigger_type}..."):
                    ...
                    adapted_features = self.adaptation_engine.adapt_features(data, target_column, trigger)
