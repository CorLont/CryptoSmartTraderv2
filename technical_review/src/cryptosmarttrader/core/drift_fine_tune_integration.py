...
class DriftFineTuneIntegration:
    ...
    async def _run_monitoring_cycle(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Monitoring cycle completed: {len(drift_alerts)} alerts, "
    ...
    async def _process_drift_alerts(self, drift_alerts: List[DriftAlert]) -> None:
        ...
        for alert in drift_alerts:
            try:
                self.logger.warning(f"Processing drift alert: {alert.drift_type} for {alert.component}")
                ...
                if self.config['auto_fine_tune_enabled'] and alert.component in self.config['health_check_components']:
    ...
    async def _schedule_drift_response(self, alert: DriftAlert) -> None:
        ...
        if response_key not in self.pending_drift_responses:
            ...
            self.logger.info(f"Scheduled drift response for {alert.component} in "
    ...
    async def _calculate_system_health(self) -> float:
        ...
        try:
            ...
            weighted_score = sum(score * weights.get(component, 0)
    ...
    def _on_trading_status_change(self, change: TradingStatusChange) -> None:
        ...
        self.logger.info(f"Trading status changed: {change.previous_mode.value} -> "
        ...
        if (change.new_mode.value in ['paper', 'disabled'] and
            change.reason in [DisableReason.LOW_HEALTH_SCORE, DisableReason.PERFORMANCE_DEGRADATION]):
            ...
            for component in self.config['health_check_components']:
                try:
                    job_id = self.fine_tune_scheduler.create_fine_tune_job(
                        model_name=component,
                        trigger_reason="health_degradation",
                        priority="high"
                    )

                    self.logger.info(f"Created urgent fine-tune job {job_id} for {component} "
    ...
    def record_component_metrics(self, component: str, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if 'accuracy' in metrics:
                self.drift_system.record_performance_metrics(component, 'accuracy', metrics['accuracy'])

            if 'latency' in metrics:
                self.drift_system.record_performance_metrics(component, 'latency', metrics['latency'])
    ...
    def add_training_data(self, model_name: str, features, targets, importance_weights=None) -> None:
        """Add training data for fine-tuning"""

        try:
            self.fine_tune_scheduler.add_training_data(model_name, features, targets, importance_weights)
            self.logger.debug(f"Added training data for {model_name}")

        except Exception as e:
            self.logger.error(f"Failed to add training data for {model_name}: {e}")

    def manual_trigger_fine_tune(self, model_name: str, reason: str, priority: str = "medium") -> str:
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'system_uptime': (datetime.now() - self.integration_metrics['system_start_time']).total_seconds()
        }
...
if __name__ == "__main__":
    async def test_drift_fine_tune_integration():
        ...
        for i in range(5):
            ...
            print(f"   Recorded metrics batch {i+1}: error_rate={metrics['error_rate']:.3f}, "
        ...
        print(f"   Fine-tune jobs triggered: {status['integration_metrics']['fine_tune_jobs_triggered']}")
        print(f"   Auto-disables triggered: {status['integration_metrics']['auto_disables_triggered']}")
        ...
        return (status['integration_metrics']['drift_alerts_total'] > 0 or
                status['integration_metrics']['fine_tune_jobs_triggered'] > 0)
