...
from core.coverage_audit_system import (
    get_kraken_coverage_auditor,
    get_data_completeness_validator,
    CoverageStatus,
...
async def run_daily_coverage_audit(exchange: str = "kraken", include_completeness: bool = True):
    ...
    try:
        ...
        print(
            f"   Audit Duration: {coverage_report.performance_metrics.get('audit_duration_minutes', 0):.1f} minutes"
        )
        print()
        ...
        if coverage_report.ticker_changes:
            ...
            high_impact_changes = [
                c for c in coverage_report.ticker_changes if c.impact_score > 0.5
            ]
            if high_impact_changes:
                print(f"\n   HIGH IMPACT CHANGES:")
                for change in high_impact_changes[:10]:  # Show top 10
                    print(
                        f"   - {change.symbol}: {change.change_type.value} (impact: {change.impact_score:.2f})"
                    )
            print()
        ...
        if coverage_report.data_quality_issues:
            ...
            critical_issues = [
                i for i in coverage_report.data_quality_issues if i.get("severity") == "critical"
            ]
            if critical_issues:
                print(f"\n   CRITICAL ISSUES:")
        ...
        if include_completeness:
            ...
            if coin_data:
                ...
                print(
                    f"   Pass Rate: {completeness_result.passed_coins / completeness_result.total_coins_evaluated * 100:.1f}%"
                )
                print(f"   Average Completeness: {completeness_result.average_completeness:.1%}")
                print(
                    f"   Processing Time: {completeness_result.processing_latency_minutes:.1f} minutes"
                )
                print()
                ...
                if completeness_result.processing_latency_minutes > 30:
                    print(
                        f"⚠️  WARNING: Processing time {completeness_result.processing_latency_minutes:.1f} min > 30 min budget"
                    )

                if completeness_result.excluded_coins > (
                    completeness_result.total_coins_evaluated * 0.02
                ):
                    print(
                        f"⚠️  WARNING: {completeness_result.excluded_coins} excluded coins > 2% threshold"
                    )
        ...
        await save_daily_summary(
            coverage_report, completeness_result if include_completeness else None
        )
...
async def save_daily_summary(coverage_report, completeness_result=None):
    ...
    summary = {
        ...
        "coverage_audit": {
            ...
            "audit_duration_minutes": coverage_report.performance_metrics.get(
    ...
    if completeness_result:
        summary["completeness_validation"] = {
            ...
            "pass_rate": completeness_result.passed_coins
            / completeness_result.total_coins_evaluated
            * 100
            if completeness_result.total_coins_evaluated > 0
            else 0,
...
async def run_quick_coverage_check(exchange: str = "kraken"):
    ...
    try:
        if exchange.lower() == "kraken":
            ...
            coverage_percentage = (
                (len(covered_tickers) / len(tradeable_tickers)) * 100 if tradeable_tickers else 0
            )
...
Examples:
  ...
  python scripts/daily_coverage_audit.py quick              # Quick check
    ...
    parser.add_argument("command", choices=["audit", "quick"], help="Command to execute")

    parser.add_argument("--exchange", default="kraken", help="Exchange to audit (default: kraken)")

    parser.add_argument(
        "--no-completeness", action="store_true", help="Skip data completeness validation"
    )
