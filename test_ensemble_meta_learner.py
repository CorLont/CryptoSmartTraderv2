...
from cryptosmarttrader.ensemble import (
    TechnicalAnalysisModel, SentimentModel, RegimeModel,
    MetaLearner, EnsembleConfig, 
    AlphaBlender, BlendingStrategy, BlendingConfig,
    SignalDecayManager, DecayConfig, DecayFunction
)
...
def generate_synthetic_market_data(n_periods: int = 200) -> Dict[str, Any]:
    ...
    print(f"   Sentiment scores: Social={sentiment_data['social_sentiment']:.2f}, "
...
def test_base_models():
    ...
    print(f"   Consensus direction: {max(set([p.direction for p in predictions.values()]), key=list([p.direction for p in predictions.values()]).count)}")
...
def test_meta_learner():
    ...
    if 'error' not in training_results:
        ...
        print(f"   CV AUC: {training_results['cv_auc_mean']:.3f} ± {training_results['cv_auc_std']:.3f}")
    ...
    for i in range(5):
        ...
        if base_predictions:
            ...
            ensemble_pred = EnsemblePrediction(
                ...
                base_predictions=base_predictions,
                model_weights={name: 1.0/len(base_predictions) for name in base_predictions.keys()},
