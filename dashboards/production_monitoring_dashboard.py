...
def get_agent_performance_metrics():
    ...
    try:
        ...
        metrics['security_manager'] = {
            'status': 'operational' if security_health.get('total_secrets_cached', 0) > 0 else 'degraded',
...
def main():
    ...
    st.set_page_config(
        page_title="CryptoSmartTrader - Production Monitoring",
        page_icon="📊",
        layout="wide"
    )
    ...
    with col4:
        error_count = sum(m.get('total_errors', 0) for m in agent_metrics.values() if 'total_errors' in m)
        st.metric(
            "Total Errors",
            f"{error_count}",
            delta="Last 24h"
    ...
    for agent_name, metrics in agent_metrics.items():
        with st.expander(f"{agent_name.replace('_', ' ').title()} - {metrics.get('status', 'unknown').upper()}"):
            col1, col2 = st.columns(2)
    ...
    failed_agents = [name for name, metrics in agent_metrics.items() if metrics.get('status') == 'error']
    if failed_agents:
        alerts.append(("MEDIUM", f"Agents in error state: {', '.join(failed_agents)}"))
    
    # Check for high error rates
    recent_errors = sum(m.get('errors_last_hour', 0) for m in agent_metrics.values() if 'errors_last_hour' in m)
    if recent_errors > 10:
