...
def confidence_binning_analysis(
    ...
    binning_results = df.groupby(confidence_bins, observed=False).agg({
        'realized': [
            ('count', 'count'),
...
def detailed_calibration_analysis(
    df: pd.DataFrame,
    conf_col: str = 'conf_720h'
) -> Dict[str, Any]:
    """
    ...
    if not standard_results.empty:
        ...
        ece = (standard_results['realized_count'] * standard_results['calibration_error']).sum() / standard_results['realized_count'].sum()
    ...
    calibration_analysis = {
        ...
        "standard_binning": standard_results.to_dict('records') if not standard_results.empty else [],
        "fine_binning": fine_results.to_dict('records') if not fine_results.empty else []
    }
    
    logger.info(f"Calibration analysis completed: ECE={reliability_metrics['expected_calibration_error']:.3f}")
...
def save_calibration_results(
    ...
    output_dir: str = "logs/calibration"
) -> str:
...
def print_calibration_summary(results: Dict[str, Any]) -> None:
    ...
    print(f"   Confidence range: {overall['confidence_range']['min']:.1%} - {overall['confidence_range']['max']:.1%}")
    ...
    if not np.isnan(reliability["expected_calibration_error"]):
        ...
        print(f"   Well-calibrated bins: {reliability['well_calibrated_bins']}/{reliability['bins_with_data']}")
    print()
    ...
    if standard_binning:
        ...
        for bin_result in standard_binning:
            if bin_result['realized_count'] > 0:
                print(f"{bin_result['confidence_range']:<12} "
...
Examples:
  ...
  python scripts/calibration.py --input data.csv --bins 0.8 0.85 0.9 0.95 1.0
    ...
    parser.add_argument(
        '--input',
        required=True,
        help='Path to merged predictions+reality CSV file'
    )
    
    parser.add_argument(
        '--confidence',
        default='conf_720h',
        help='Confidence column name (default: conf_720h)'
    )
