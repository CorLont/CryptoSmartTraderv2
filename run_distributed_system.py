    ...
    async def _display_startup_status(self):
        ...
        try:
            ...
            for agent_name, agent_info in agents.items():
                ...
                print(f"      Workers: {config['worker_threads']}, Memory limit: {config['max_memory_mb']} MB")
                print(f"      Uptime: {metrics['uptime_seconds']:.0f}s, Restarts: {metrics['restart_count']}")
...
def main():
    ...
    parser.add_argument('--agents', nargs='+', 
                       choices=['sentiment_agent', 'technical_agent', 'whale_agent', 
                               'ml_agent', 'backtest_agent', 'trade_executor'],
                       help='Specific agents to run (default: all)')
    parser.add_argument('--monitoring-only', action='store_true',
                       help='Run only monitoring server')
