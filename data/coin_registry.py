...
class CoinRegistry:
    ...
    def get_coins_by_rank(self, max_rank: int = 100, active_only: bool = True) -> List[Dict[str, Any]]:
    ...
    def _resolve_symbol(self, symbol: str) -> Optional[str]:
        ...
        for coin_id, mappings in self.coin_mappings.items():
            alt_symbols = mappings.get('alternative_symbols', [])
            if symbol in alt_symbols or symbol.upper() in alt_symbols or symbol.lower() in alt_symbols:
                return coin_id
            ...
            with self._lock:
                ...
                for coin_id, coin_data in self.coins.items():
                    if not isinstance(coin_data, dict):
                        validation_results['errors'].append(f"Coin {coin_id} data is not a dictionary")
                        continue
                    
                    required_fields = ['symbol', 'name', 'market_cap_rank']
                    for field in required_fields:
                        if field not in coin_data:
                            validation_results['errors'].append(f"Coin {coin_id} missing required field: {field}")
                    
                    # Check symbol consistency
                    if coin_data.get('symbol', '').upper() != coin_id:
                        validation_results['warnings'].append(f"Coin {coin_id} has inconsistent symbol: {coin_data.get('symbol')}")
                
                # Check mappings consistency
                for coin_id, mappings in self.coin_mappings.items():
                    if coin_id not in self.coins:
                        validation_results['warnings'].append(f"Mapping exists for non-existent coin: {coin_id}")
                ...
                for exchange, mappings in self.exchange_mappings.items():
                    for exchange_symbol, coin_id in mappings.items():
                        if coin_id not in self.coins:
                            validation_results['warnings'].append(f"Exchange mapping {exchange}:{exchange_symbol} points to non-existent coin: {coin_id}")
    ...
    def cleanup_registry(self, remove_inactive: bool = False, 
                        remove_old_days: int = 365) -> Dict[str, int]:
        """Clean up registry by removing old or inactive entries"""
        cleanup_stats = {
            'coins_removed': 0,
            'mappings_cleaned': 0,
            'exchange_mappings_cleaned': 0
        }
