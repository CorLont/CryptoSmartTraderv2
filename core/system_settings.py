...
class MLSettings(BaseSystemSettings):
    ...
    model_types: List[str] = Field(
        default=["xgboost", "neural", "ensemble"],
...
class DataSettings(BaseSystemSettings):
    ...
    data_sources: List[str] = Field(
        default=["kraken", "binance", "coinbase"],
        description="Enabled data sources"
    )
    
    # Data retention
    raw_data_retention_days: int = Field(default=30, description="Raw data retention in days")
    processed_data_retention_days: int = Field(default=90, description="Processed data retention in days")
