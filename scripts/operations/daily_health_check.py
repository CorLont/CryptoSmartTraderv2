...
class DailyHealthChecker:
    ...
    def check_application_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                app_results["health_score"] = health_data.get("application_health", {}).get(
                    "health_score", 0
                )
                app_results["trading_status"] = health_data.get("application_health", {}).get(
            ...
            if log_file.exists():
                ...
                if recent_errors > 10:
                    self.results["warnings"].append(
                        f"High error count: {recent_errors} errors in logs"
                    )
            else:
        ...
        try:
            ...
            for dir_name in cache_dirs:
                ...
                if dir_path.exists():
                    ...
                    newest_file = max(
                        dir_path.rglob("*"), key=lambda x: x.stat().st_mtime, default=None
                    )
                    if newest_file:
                        ...
                        if age_hours > 24:
                            self.results["warnings"].append(
                                f"{dir_name} data is {age_hours:.1f} hours old"
                            )
    ...
    def check_process_health(self) -> Dict[str, Any]:
        ...
        if len(running_processes) < 3:
            self.results["warnings"].append(
                f"Expected 3+ processes, found {len(running_processes)}"
            )
    ...
    def save_results(self):
        ...
        with open(summary_file, "a") as f:
            f.write(
                f"{self.timestamp.isoformat()},{self.results['overall_health']},{len(self.results['warnings'])},{len(self.results['errors'])}\n"
            )
