...
class KellyCalculator:
    ...
    def calculate_kelly_fraction(
        ...
        if expected_return > 0:
            ...
            # Kelly fraction
            kelly_f = (win_probability * win_amount - (1 - win_probability) * loss_amount) / win_amount
        else:
...
class CorrelationManager:
    ...
    def update_correlations(self, returns_data: pd.DataFrame):
        ...
        if len(returns_data) < self.correlation_window:
            self.logger.warning(f"Insufficient data for correlation calculation: {len(returns_data)} < {self.correlation_window}")
            self.correlation_matrix = pd.DataFrame()
            return

        # Calculate rolling correlation matrix
        self.correlation_matrix = returns_data.rolling(
    ...
    def calculate_correlation_penalty(
        ...
        if not existing_positions or self.correlation_matrix is None or self.correlation_matrix.empty:
...
class UncertaintyAwarePortfolioSizer:
    ...
    def calculate_position_size(
        ...
        if confidence < self.min_confidence:
            final_size = 0.0
            reasoning = f"Below minimum confidence threshold ({confidence:.3f} < {self.min_confidence})"
        elif final_size == 0:
    ...
    def optimize_portfolio_allocation(
        ...
        for opp in opportunities:
            ...
            rec = self.calculate_position_size(
                ...
                volume_24h, market_cap, bid_ask_spread
            )
    ...
    def _calculate_portfolio_metrics(
        ...
        if len(weights) == 0 or np.sum(weights) == 0:
            return PortfolioRiskMetrics(
                total_exposure=0, concentration_risk=0, correlation_risk=0,
                uncertainty_risk=0, liquidity_risk=0, var_95=0,
        ...
        portfolio_variance = np.sum((weights * volatilities) ** 2) + \
                           2 * avg_correlation * np.sum(np.outer(weights * volatilities, weights * volatilities))
