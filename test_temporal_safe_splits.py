...
def test_temporal_safe_splits():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "temporal_splits", "core/temporal_safe_splits.py"
        )
        ...
        config = SplitConfig(
            strategy=SplitStrategy.ROLLING_WINDOW, train_size=5, test_size=2, gap_hours=1.0
        )
        ...
        test_data = pd.DataFrame({"timestamp": dates, "value": np.random.randn(1000)})
        ...
        if len(purged_splits) > 0:
            ...
            if len(train_gaps) > 0:
                print(
                    f"   ✅ PASSED: Purging implemented - found {len(train_gaps)} gaps in training indices"
                )
                print(
                    f"   Purge buffer metadata: {first_split.metadata.get('purge_buffer_hours', 'missing')}"
                )
            else:
        ...
        for i, case_data in enumerate(edge_cases):
            try:
                config = SplitConfig(
                    strategy=SplitStrategy.ROLLING_WINDOW, train_size=2, test_size=1, gap_hours=0.1
                )
        ...
        for strategy in strategies_to_test:
            config = SplitConfig(
                strategy=strategy, train_size=20, test_size=5, gap_hours=1.0, max_splits=3
            )
