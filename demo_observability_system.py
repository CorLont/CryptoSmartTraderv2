...
from cryptosmarttrader.observability import (
    MetricsCollector, AlertManager, get_metrics_collector, 
    create_alert_manager, AlertSeverity
)
...
async def demonstrate_observability_system():
    ...
    for i in range(15):
        ...
        # Record order sent
        request_id = metrics_collector.start_request("execution", "place_order", symbol=symbol, side=side)
        ...
        if random.random() > 0.05:  # 95% success rate
            ...
            price = random.uniform(30000, 60000) if symbol == 'BTC-USD' else random.uniform(2000, 4000)
            slippage_bps = random.uniform(1.0, 25.0)
            latency = random.uniform(0.05, 0.5)
            
            metrics_collector.record_order_filled(symbol, side, quantity, price, slippage_bps, latency)
            metrics_collector.end_request(request_id, success=True, filled_quantity=quantity)
            
            print(f"      Order {i+1}: {side} {quantity:.2f} {symbol} @ ${price:,.2f} - {slippage_bps:.1f} bps slippage")
        else:
            # Order error
            error_type = random.choice(['insufficient_balance', 'market_closed', 'rate_limit'])
            metrics_collector.record_order_error(symbol, error_type, f"Simulated {error_type} error")
    ...
    for day in range(10):
        ...
        print(f"      Day {day+1}: Equity ${current_equity:,.2f}, Drawdown {drawdown_pct:.1f}%, PnL ${daily_pnl:,.2f}")
    ...
    signal_sources = ['technical_analysis', 'sentiment_analysis', 'regime_detector', 'whale_tracker']
    ...
    for i in range(20):
        ...
        metrics_collector.record_signal_received(
            source, signal_type, 
            symbol=symbol, confidence=confidence, strength=random.uniform(0.3, 1.0)
        )
        
        print(f"      Signal {i+1}: {source} -> {signal_type} for {symbol} (confidence: {confidence:.2f})")
    ...
    for i in range(25):
        ...
        print(f"      API {i+1}: {method} {endpoint} -> {status_code} ({duration*1000:.0f}ms) {status_text}")
    ...
    print(f"      Orders: {metrics_summary['orders']['total_sent']} sent, {metrics_summary['orders']['total_filled']} filled")
    ...
    print(f"      Last signal: {metrics_summary['system']['minutes_since_last_signal']:.1f} minutes ago")
    ...
    if active_alerts:
        ...
        for alert in active_alerts:
            print(f"      🚨 {alert['name']} ({alert['severity']}) - triggered {alert['trigger_count']} times")
    else:
    ...
    req1 = metrics_collector.start_request("market_analysis", "analyze_opportunity", symbol="BTC-USD")
    workflow_requests.append(req1)
    await asyncio.sleep(0.1)
    
    # Start risk assessment request  
    req2 = metrics_collector.start_request("risk_management", "assess_risk", symbol="BTC-USD", parent_request=req1)
    workflow_requests.append(req2)
    await asyncio.sleep(0.05)
    
    # Start execution request
    req3 = metrics_collector.start_request("execution", "place_order", symbol="BTC-USD", parent_request=req2)
    ...
    for alert in final_alerts:
        severity_emoji = {"warning": "⚠️", "critical": "🔴", "emergency": "🚨"}.get(alert['severity'], "ℹ️")
    ...
    print(f"      Fill Rate: {(final_summary['orders']['total_filled'] / max(1, final_summary['orders']['total_sent']) * 100):.1f}%")
    print(f"      Error Rate: {(final_summary['orders']['total_errors'] / max(1, final_summary['orders']['total_sent']) * 100):.1f}%")
    ...
    print(f"      Critical/Emergency: {alert_summary['severity_distribution']['critical'] + alert_summary['severity_distribution']['emergency']}")
