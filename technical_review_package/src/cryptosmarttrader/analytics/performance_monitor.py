...
class PerformanceMonitor:
    ...
    def __init__(self, monitoring_frequency_seconds: int = 60, alert_cooldown_minutes: int = 15):
    ...
    def _update_baselines(self):
        ...
        try:
            ...
            if self.slippage_buffer:
                recent_slippage = [
                    value for timestamp, value in self.slippage_buffer if timestamp >= cutoff_time
                ]
                if recent_slippage:
            ...
            if self.fee_buffer:
                recent_fees = [
                    value for timestamp, value in self.fee_buffer if timestamp >= cutoff_time
                ]
                if recent_fees:
            ...
            if self.execution_buffer:
                recent_execution = [
                    value for timestamp, value in self.execution_buffer if timestamp >= cutoff_time
                ]
                if recent_execution:
            ...
            if self.performance_buffer:
                ...
                if recent_performance:
                    ...
                    alpha_values = [
                        metrics.get("alpha_bps", 0) for _, metrics in recent_performance
                    ]
                    if alpha_values:
                    ...
                    return_values = [
                        metrics.get("return_bps", 0) for _, metrics in recent_performance
                    ]
                    if return_values:
        ...
        try:
            ...
            if "slippage_bps" in current_metrics and "slippage_bps" in self.baselines:
                ...
                if baseline_slippage > 0:
                    ...
                    if deviation_pct > self.thresholds["slippage_drift_pct"]:
                        alert = self._create_slippage_alert(
                            current_slippage, baseline_slippage, deviation_pct
                        )
            ...
            if "alpha_bps" in current_metrics and "alpha_bps" in self.baselines:
                ...
                if baseline_alpha > 0:
                    ...
                    if decay_pct > self.thresholds["alpha_decay_pct"]:
                        alert = self._create_alpha_decay_alert(
                            current_alpha, baseline_alpha, decay_pct
                        )
                        if self._should_create_alert(alert.alert_type):
                            alerts.append(alert)
            ...
            if "execution_quality" in current_metrics and "execution_quality" in self.baselines:
                ...
                if quality_drop > self.thresholds["execution_quality_drop"]:
                    alert = self._create_execution_quality_alert(
                        current_quality, baseline_quality, quality_drop
                    )
                    if self._should_create_alert(alert.alert_type):
                        alerts.append(alert)
    ...
    def _create_slippage_alert(
        self, current: float, baseline: float, deviation: float
    ) -> PerformanceAlert:
    ...
    def _create_alpha_decay_alert(
        self, current: float, baseline: float, decay: float
    ) -> PerformanceAlert:
    ...
    def _create_fee_spike_alert(
        self, current: float, baseline: float, spike: float
    ) -> PerformanceAlert:
    ...
    def _create_execution_quality_alert(
        self, current: float, baseline: float, drop: float
    ) -> PerformanceAlert:
    ...
    def _create_drawdown_alert(self, current_drawdown: float) -> PerformanceAlert:
        ...
        return PerformanceAlert(
            ...
            deviation_pct=(
                (current_drawdown - self.thresholds["drawdown_warning_pct"])
                / self.thresholds["drawdown_warning_pct"]
                * 100
            ),
            time_window="Current",
            recommendations=[
    ...
    def _create_latency_alert(self, current_latency: float) -> PerformanceAlert:
        ...
        return PerformanceAlert(
            ...
            deviation_pct=(
                (current_latency - self.thresholds["latency_spike_ms"])
                / self.thresholds["latency_spike_ms"]
                * 100
            ),
            time_window="Current",
            recommendations=[
    ...
    def _create_error_rate_alert(self, current_error_rate: float) -> PerformanceAlert:
        ...
        return PerformanceAlert(
            ...
            deviation_pct=(
                (current_error_rate - self.thresholds["error_rate_pct"])
        ...
        try:
            for alert in self.alert_history:
                if alert.alert_id == alert_id:
                    ...
                    logger.info(
                        f"Alert {alert_id} resolved by {user} (MTTR: {alert.mttr_minutes:.1f} min)"
                    )
                    return True
    ...
    def get_alert_metrics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            mttr_minutes = (
                np.mean([alert.mttr_minutes for alert in resolved_alerts]) if resolved_alerts else 0
            )
