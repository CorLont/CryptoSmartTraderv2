...
class SystemMonitor:
        ...
        try:
            ...
            if data_dir.exists():
                app_metrics["data_files"] = {
                    "analysis_files": len(list((data_dir / "analysis").glob("*.json"))) if (data_dir / "analysis").exists() else 0,
                    "social_files": len(list((data_dir / "social").glob("*.json"))) if (data_dir / "social").exists() else 0,
                    "metrics_files": len(list((data_dir / "metrics").glob("*.json"))) if (data_dir / "metrics").exists() else 0
                }
    ...
    def _log_metrics(self, metrics: Dict[str, Any], alerts: List[Dict[str, Any]]):
        ...
        try:
            ...
            dashboard_accessible = app_metrics.get("services", {}).get("dashboard_accessible", False)
