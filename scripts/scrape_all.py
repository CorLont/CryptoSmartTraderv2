        ...
        try:
            ...
            if symbol in historical_data:
                ...
                if len(hist_df) > 1:
                    ...
                    features['momentum_3d'] = (hist_df['close'].iloc[-1] / hist_df['close'].iloc[-4] - 1) if len(hist_df) >= 4 else 0
                    features['momentum_7d'] = (hist_df['close'].iloc[-1] / hist_df['close'].iloc[-8] - 1) if len(hist_df) >= 8 else 0
                    
                    # Volume features
                    features['volume_trend_7d'] = hist_df['volume'].tail(7).mean() / hist_df['volume'].mean() if hist_df['volume'].mean() > 0 else 1
            ...
            features['liquidity_score'] = min(ticker['baseVolume'] / 1000000, 1.0) if ticker['baseVolume'] else 0
...
def main():
    ...
    top_symbols = sorted(
        ticker_data.items(),
        key=lambda x: x[1]['baseVolume'] or 0,
        reverse=True
    )[:50]  # Top 50 by volume
    ...
    logger.info(f"Average volume: ${features_df['volume_24h'].mean():,.0f}" if 'volume_24h' in features_df.columns else "Volume data unavailable")
