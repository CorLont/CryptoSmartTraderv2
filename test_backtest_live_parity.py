...
from cryptosmarttrader.execution.execution_simulator import (
    ExecutionSimulator, OrderSide, OrderType, FillType, ExecutionQuality
)
from cryptosmarttrader.execution.tracking_error_monitor import (
    TrackingErrorMonitor, TrackingErrorComponent
)
...
class BacktestLiveParityTester:
    ...
    def test_execution_simulator_components(self) -> bool:
        ...
        try:
            ...
            market_success = (
                market_result.filled_size > 0 and
                len(market_result.fills) > 0 and
                market_result.total_fees > 0 and
                market_result.slippage_bps >= 0
            )
            ...
            limit_success = (
                limit_result.order_type == OrderType.LIMIT and
                limit_result.slippage_bps >= 0
            )
            ...
            fee_accuracy = (
                abs(maker_fee - expected_maker_fee) < 1.0 and
                abs(taker_fee - expected_taker_fee) < 1.0
            )
            ...
            self.test_results['execution_simulator'] = {
                ...
                'avg_slippage_bps': (market_result.slippage_bps + large_order_result.slippage_bps) / 2
            }
        ...
        try:
            ...
            for i in range(50):
                ...
                self.tracking_error_monitor.record_paper_trade(
                    ...
                    timestamp=datetime.now() - timedelta(minutes=random.randint(1, 10080))  # Within 7 days
                )
            ...
            stats_success = (
                stats_7d.get('total_trades', 0) >= 40 and  # Most trades recorded
                stats_7d.get('mean_abs_tracking_error_bps', 0) > 0 and
                stats_7d.get('std_tracking_error_bps', 0) >= 0
            )
            
            logger.info(f"Tracking error statistics (7 days):")
            logger.info(f"  Total trades: {stats_7d.get('total_trades', 0)}")
            logger.info(f"  Mean tracking error: {stats_7d.get('mean_tracking_error_bps', 0):.2f} bps")
            logger.info(f"  Mean absolute TE: {stats_7d.get('mean_abs_tracking_error_bps', 0):.2f} bps")
            logger.info(f"  Std deviation: {stats_7d.get('std_tracking_error_bps', 0):.2f} bps")
            logger.info(f"  Max absolute TE: {stats_7d.get('max_abs_tracking_error_bps', 0):.2f} bps")
            logger.info(f"  P95 absolute TE: {stats_7d.get('p95_abs_tracking_error_bps', 0):.2f} bps")
            
            # Test component attribution
            attribution = self.tracking_error_monitor.get_component_attribution(days_back=7)
            
            attribution_success = (
                attribution.get('status') != 'no_data' and
                len(attribution.get('component_breakdown', {})) > 0
            )
            
            if attribution_success:
                logger.info("Component attribution:")
                for component, stats in attribution['component_breakdown'].items():
                    logger.info(f"  {component}: {stats['mean_bps']:.1f} bps "
            ...
            report_success = (
                report.total_trades > 0 and
                report.mean_tracking_error_bps is not None and
                report.threshold_compliance_rate >= 0
            )
        ...
        try:
            ...
            for day in range(7):
                ...
                for trade in range(num_trades):
                    ...
                    price_drift = scenario["base_price"] * scenario["volatility"] * random.gauss(0, 1)
                    ...
                    if live_result.filled_size > 0:
                        ...
                        live_total = live_result.average_price * live_result.filled_size + live_result.total_fees
                ...
                if day_trades:
                    ...
                    logger.info(f"Day {day+1} average tracking error: {daily_avg_te:.1f} bps ({len(day_trades)} trades)")
            ...
            if ab_comparisons:
                ...
                trades_within_threshold = sum(1 for te in all_tracking_errors if te <= target_threshold)
                compliance_rate = trades_within_threshold / len(all_tracking_errors)
                
                # Daily consistency check
                daily_te_std = np.std(daily_tracking_errors) if len(daily_tracking_errors) > 1 else 0
    ...
    def run_comprehensive_tests(self):
        ...
        if 'execution_simulator' in self.test_results:
            ...
            logger.info(f"• Execution Quality: {exec_sim.get('market_execution_quality', 'unknown')}")
