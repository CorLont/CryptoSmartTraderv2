    ...
    def display_log_dashboard(self):
        ...
        with col2:
            ...
            if perf_summary['operations']:
                slowest_op = max(perf_summary['operations'].items(), 
                               key=lambda x: x[1]['avg_time'])
                st.metric("Slowest Operation", 
                         f"{slowest_op[0]} ({slowest_op[1]['avg_time']:.2f}s)")
        ...
        with col3:
            ...
            if error_summary['by_component']:
                top_error_component = max(error_summary['by_component'].items(),
                                        key=lambda x: x[1])
                st.metric("Most Errors", 
                         f"{top_error_component[0]} ({top_error_component[1]})")
        ...
        view_type = st.selectbox(
            "Select view:",
            ["Predictions", "Performance", "Errors", "API Calls", "User Actions"]
        )
        ...
        for log in logs:
            if 'data' in log:
                data.append({
                    ...
                    'Expected Returns': str(log['data'].get('prediction_data', {}).get('expected_returns', {})),
                    'Sentiment': log['data'].get('prediction_data', {}).get('sentiment_score', 0),
                    'Whale Detected': log['data'].get('prediction_data', {}).get('whale_detected', False)
                })
...
if __name__ == "__main__":
    ...
    st.set_page_config(
        page_title="CryptoSmartTrader Log Viewer",
        page_icon="📊",
        layout="wide"
    )
