...
class ProductionPredictionGenerator:
        ...
        for coin_data in market_data:
            ...
            for horizon in self.horizons:
                if horizon in models:
                    try:
                        # DISABLED: No artificial predictions - requires real ML model implementation
                        logger.warning(
                            f"Skipping {coin} - no trained ML model available for {horizon}"
                        )
                        continue
                        ...
                        # DISABLED: No fallback artificial data allowed
                        logger.error(
                            f"Model prediction failed for {coin} at horizon {horizon}: {e}"
                        )
                        continue
                    ...
                    # DISABLED: No predictions without trained models
                    logger.warning(f"Model {horizon} not available for {coin} - skipping")
                    continue

            # DISABLED: No artificial sentiment or whale data
            # Requires real sentiment analysis from news/social media APIs
            # Requires real whale detection from blockchain transaction monitoring
            logger.warning(f"Skipping {coin} - no real sentiment/whale analysis available")
            continue

            # DISABLED: No artificial meta-labeling - requires real label quality assessment

            # DISABLED: All artificial data generation blocked
            logger.error(f"Skipping {coin} - no real ML pipeline available")
            continue
    ...
    def apply_80_percent_gate(self, predictions):
        ...
        logger.info(
            f"80% confidence gate: {len(filtered_predictions)}/{len(predictions)} predictions passed"
        )
        ...
        if len(filtered_predictions) == 0:
            ...
            empty_gate_log = {
                ...
                "max_confidence_seen": max(
                    [
                        max([pred.get(f"confidence_{h}", 0) for h in self.horizons])
                        for pred in predictions
                    ]
                )
                if predictions
