...
class AlertManager:
    ...
    def suppress_alert(self, rule_name: str, duration_minutes: int = 60):
        ...
        self.metrics_collector.logger.info(f"Alert suppressed: {rule_name} for {duration_minutes} minutes")
    ...
    def _handle_alert_trigger(self, rule: AlertRule, current_time: datetime, context: Dict[str, Any]):
    ...
    def _handle_alert_clear(self, rule_name: str, current_time: datetime):
        ...
        if rule_name in self.alert_states:
            ...
            # Log alert cleared
            self.metrics_collector.logger.info(f"Alert cleared: {rule_name}", extra={
                'extra_fields': {
                    'alert_name': rule_name,
                    'duration_minutes': (current_time - state.first_triggered).total_seconds() / 60,
    ...
    def _escalate_alert(self, state: AlertState, rule: AlertRule):
        ...
        self.metrics_collector.logger.error(f"Alert escalated: {state.rule_name}", extra={
            'extra_fields': {
    ...
    def _send_alert_notification(self, rule: AlertRule, state: AlertState):
        """Send alert notification."""
        self.metrics_collector.logger.warning(f"ALERT: {rule.name}", extra={
            'extra_fields': {
